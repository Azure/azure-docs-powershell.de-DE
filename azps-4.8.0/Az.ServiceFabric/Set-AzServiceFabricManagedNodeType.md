...
## Beschreibung
Legt Knotentyp-Ressourceneigenschaften fest oder führt reimage-Aktionen für bestimmte NDES des Knotentyps mit dem Parameter-reimage aus. Bei einem reimgae-Vorgang werden die Service Fabric-Knoten deaktiviert, bevor Sie die VMs erneut abbilden und Sie wieder aktivieren, sobald Sie wieder verfügbar sind. Wenn dies für primäre Knotentypen erfolgt, kann es eine Weile dauern, da es möglicherweise nicht alle Knoten zur gleichen Zeit umbildet. Use-ForceReimage erzwingen Sie den Vorgang selbst dann, wenn Service Fabric die Knoten nicht deaktivieren, aber mit Vorsicht verwenden kann, da dies zu Datenverlusten führen kann, wenn statusbehaftete Arbeitslasten auf dem Knoten ausgeführt werden.
...
Aktualisieren Sie die Anzahl der Instanzen des Knotentyps.
...
Aktualisieren der Platzierungs-properites des Knotentyps Dadurch werden ältere Placement-properites, falls vorhanden, überschrieben.
...
Umbilden von Knoten 0 und 3 auf dem Knotentyp
...
Aktualisieren Sie die Anzahl der Instanzen des Knotentyps mit Piping.

## Parameter

### -ApplicationEndPort
Anwendungsendpunkt eines Portbereichs
...
Anwendungsstart Port eines Bereichs von Ports.
...
### -Kapazität
Kapazitätskategorien, die auf die Knoten im Knotentyp als Schlüssel-Wert-Paare angewendet wurden, verwendet der Clusterressourcen-Manager diese Tags, um zu verstehen, wie viel Ressourcen ein Knoten besitzt. Beim Aktualisieren werden die aktuellen Werte außer Kraft gesetzt.
...
### -EphemeralEndPort
Ephemerer Endpunkt Port eines Bereichs von Ports.
...
### -EphemeralStartPort
Ephemerer Start-Port eines Bereichs von Ports.
...
### -ForceReimage
Wenn Sie dieses Flag verwenden, wird die Entfernung erzwungen, auch wenn Service Fabric die Knoten nicht deaktivieren kann.
Verwenden Sie mit bedacht, da dies zu Datenverlusten führen kann, wenn auf dem Knotenstatus behaftete Arbeitslasten ausgeführt werden.
...
### -PlacementProperty
Platzierungs Tags, die auf Knoten im Knotentyp als Schlüssel-Wert-Paare angewendet werden, können verwendet werden, um anzugeben, wo bestimmte Dienste (Arbeitsauslastung) ausgeführt werden sollen. Beim Aktualisieren werden die aktuellen Werte außer Kraft gesetzt.
...
### -Reimage
Geben Sie an, dass Knoten auf dem Knotentyp erneut abbilden sollen.
