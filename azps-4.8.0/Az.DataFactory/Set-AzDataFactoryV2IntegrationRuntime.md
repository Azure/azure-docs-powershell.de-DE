...
## Beschreibung
Das Set-AzDataFactoryV2IntegrationRuntime-Cmdlet aktualisiert eine Integrationslaufzeit mit bestimmten Parametern.

## Beispiele

### Beispiel 1: Update-Integrationslauf Zeit Beschreibung.
```
...
### Beispiel 2: Freigeben der selbst gehosteten Integrationslaufzeit
```
PS C:\> Set-AzDataFactoryV2IntegrationRuntime -ResourceGroupName 'rg-test-dfv2' -DataFactoryName 'test-df-eu2' -Name 'test-selfhost-ir' `
...
Das Cmdlet fügt die ADF hinzu, um die Common Integration Runtime zu verwenden. Bei Verwendung `-SharedIntegrationRuntimeResourceId` des Parameters `-Type` muss auch der Wert angegeben werden. Beachten Sie, dass der Data Factory die Berechtigung erteilt werden muss, die Integrationslaufzeit zu verwenden, bevor Cmdlet ausgeführt wird.

### Beispiel 3: Konfigurieren Sie Self-Hosted IR als Proxy für Azure-SSIS-IR in ADF.
```
...
Das Cmdlet Update Azure-SSIS-Integrationslaufzeit, um die selbst gehostete Integrationslaufzeit als Datenproxy zu verwenden.
...
Die Anmeldeinformationen für den Katalogdaten Bank Administrator der Integrationslaufzeit.
...
### -CatalogPricingTier
Die Preisstufe der Katalogdatenbank der Integrationslaufzeit.
...
### -CatalogServerEndpoint
Der Katalogdatenbankserver-Endpunkt der Integrationslaufzeit.
...
### -DataFlowComputeType
Compute-Typ des Datenfluss Clusters, der den Datenfluss Auftrag ausführt.
...
### -DataFlowCoreCount
Kernanzahl des Datenfluss Clusters, der den Datenfluss Auftrag ausführt.
...
### -DataFlowTimeToLive
Zeit zum Leben (in Minuten) Einstellung des Datenfluss Clusters, der den Datenfluss Auftrag ausführt.
...
Der Name der Self-Hosted-Integrationslaufzeit, der als Proxy verwendet wird
...
Der Namen des Azure BLOB-Speicher-verknüpften Diensts, der auf den Stagingdaten-Speicher verweist, der beim Verschieben von Daten zwischen Self-Hosted und Azure-SSIS-Integrationslaufzeit verwendet werden soll
...
### -DataProxyStagingPath
Der Pfad im Staging-Datenspeicher, der beim Verschieben von Daten zwischen Self-Hosted-und Azure-SSIS-Integrations Runtime verwendet werden soll, wird ein Standardcontainer verwendet, wenn nicht angegeben.
...
### -Edition
Die Edition für die SSIS-Integrationslaufzeit, die Standard oder Enterprise sein kann, Standard ist Standard, wenn Sie nicht angegeben wird.
...
### -LicenseType
Der Lizenztyp, den Sie für die SSIS-IR auswählen möchten. Es gibt zwei Typen: LicenseIncluded oder Grundpr.. Wenn Sie für die Azure Hybrid use Benefit (AHUB)-Preise qualifiziert sind, wählen Sie Grundpr. aus. Wenn dies nicht der Fall ist, wählen Sie LicenseIncluded aus.
...
Maximale parallele Ausführungsanzahl pro Knoten für eine verwaltete dedizierte Integrationslaufzeit.
...
### -Knoten
Die Knoten Größe der Integrationslaufzeit.
...
### -SetupScriptContainerSasUri
Der SAS-URI des Azure-BLOB-Containers, der das benutzerdefinierte Setupskript enthält.
...
### -Subnetz
Der Name des Subnets in der VNet.
...
### -Typ
Der Typ der Integrationslaufzeit.
