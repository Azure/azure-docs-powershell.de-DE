...
Wenn zwei gelöschte apps mit dem gleichen Namen (ContosoApp) vorhanden sind, erhalten Sie Details zu den Websites und Wiederherstellen der APP mit dem Namen ContosoRestore mit der APP unserer Wahl, indem Sie Restore mit ID aufrufen.
...
Wenn zwei gelöschte apps mit dem gleichen Namen (ContosoApp) vorhanden sind, erhalten Sie Details zu den Websites und zur Wiederherstellung der APP mit dem Namen ContosoRestore mit der APP unserer Wahl, indem Sie RESTORE with Inputobject (Deletedsite)-Details aufrufen. 
