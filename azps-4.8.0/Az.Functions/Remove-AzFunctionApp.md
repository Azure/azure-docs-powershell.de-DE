...
### Beispiel 1: Rufen Sie eine Funktions-APP nach Namen ab, und löschen Sie Sie.
```powershell
PS C:\> Get-AzFunctionApp -Name MyAppName -ResourceGroupName MyResourceGroupName | Remove-AzFunctionApp -Force
```

Dieser Befehl ruft eine Funktions-APP mit dem Namen ab und löscht sie.

### Beispiel 2: Löschen einer Funktions-APP mit dem Namen
```powershell
PS C:\> Remove-AzFunctionApp -Name MyAppName -ResourceGroupName MyResourceGroupName -Force
```

Dieser Befehl löscht eine Funktions-APP nach Namen.
...
### -Force
Erzwingt, dass das Cmdlet die Funktions-APP entfernt, ohne die Bestätigung zu bestätigen.
