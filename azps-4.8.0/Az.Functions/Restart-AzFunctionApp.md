...
### Beispiel 1: Abrufen einer Funktions-APP mit dem Namen und erneutes Starten.
```powershell
PS C:\> Get-AzFunctionApp -Name MyAppName -ResourceGroupName MyResourceGroupName | Restart-AzFunctionApp -Force
```

Dieser Befehl ruft eine Funktions-APP mit dem Namen ab und startet Sie neu.

### Beispiel 2: Starten Sie eine Funktions-APP mit dem Namen erneut.
```powershell
PS C:\> Restart-AzFunctionApp -Name MyAppName -ResourceGroupName MyResourceGroupName -Force
```

Mit diesem Befehl wird eine Funktions-APP mit dem Namen neu gestartet.
...
### -Force
Erzwingt das Cmdlet, die Funktions-APP ohne Aufforderung zur Bestätigung neu zu starten.
