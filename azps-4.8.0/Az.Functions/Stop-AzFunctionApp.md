...
### Beispiel 1: Rufen Sie eine Funktions-APP mit dem Namen ab, und beenden Sie Sie.
```powershell
PS C:\> Get-AzFunctionApp -Name MyAppName -ResourceGroupName MyResourceGroupName | Stop-AzFunctionApp -Force
```

Dieser Befehl ruft eine Funktions-APP mit dem Namen ab und beendet Sie.

### Beispiel 2: Beenden einer Funktions-APP mit dem Namen
```powershell
PS C:\> Stop-AzFunctionApp -Name MyAppName -ResourceGroupName MyResourceGroupName -Force
```

Mit diesem Befehl wird eine Funktions-APP nach Namen angehalten.
...
### -Force
Erzwingt das Cmdlet, die Funktions-APP zu beenden, ohne die Bestätigung zu bestätigen.
