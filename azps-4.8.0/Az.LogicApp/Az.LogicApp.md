---
Module Name: Az.LogicApp
Module Guid: 8a7d44bb-e747-4265-8f22-b3937961d709
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.logicapp
Help Version: 4.0.3.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/LogicApp/LogicApp/help/Az.LogicApp.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/LogicApp/LogicApp/help/Az.LogicApp.md
ms.openlocfilehash: ddad081b0c1609f046aa26b963a04cd50ace8448
ms.sourcegitcommit: 1de2b6c3c99197958fa2101bc37680e7507f91ac
ms.translationtype: MT
ms.contentlocale: de-DE
ms.lasthandoff: 10/13/2020
ms.locfileid: "94166550"
---
# AZ. LogicApp-Modul
## Beschreibung
In diesem Thema werden Hilfethemen zu den Logik-apps-Cmdlets angezeigt.

## AZ. LogicApp-Cmdlets
### [Get-AzIntegrationAccount](Get-AzIntegrationAccount.md)
Ruft Integrations Konten ab.

### [Get-AzIntegrationAccountAgreement](Get-AzIntegrationAccountAgreement.md)
Ruft eine Integrations Kontovereinbarung ab.

### [Get-AzIntegrationAccountAssembly](Get-AzIntegrationAccountAssembly.md)
Ruft eine Integration Account-Assembly ab.

### [Get-AzIntegrationAccountBatchConfiguration](Get-AzIntegrationAccountBatchConfiguration.md)
Ruft eine batchkonfiguration für das Integrations Konto ab.

### [Get-AzIntegrationAccountCallbackUrl](Get-AzIntegrationAccountCallbackUrl.md)
Ruft eine Integrations Konto-Rückruf-URL ab.

### [Get-AzIntegrationAccountCertificate](Get-AzIntegrationAccountCertificate.md)
Ruft Integrations Kontozertifikate aus einer Ressourcengruppe ab.

### [Get-AzIntegrationAccountGeneratedIcn](Get-AzIntegrationAccountGeneratedIcn.md)
Mit diesem Cmdlet wird der aktuelle Wert der generierten Interchange Control Number pro Vereinbarung abgerufen.

### [Get-AzIntegrationAccountMap](Get-AzIntegrationAccountMap.md)
Ruft eine Integration-Kontozuordnung ab.

### [Get-AzIntegrationAccountPartner](Get-AzIntegrationAccountPartner.md)
Ruft Integrations Kontopartner ab.

### [Get-AzIntegrationAccountReceivedIcn](Get-AzIntegrationAccountReceivedIcn.md)
Mit diesem Cmdlet wird eine bestimmte empfangene Austauschkontrollnummer pro Vereinbarung und Steuernummern Wert abgerufen.

### [Get-AzIntegrationAccountSchema](Get-AzIntegrationAccountSchema.md)
Ruft Schemas für das Integrations Konto ab.

### [Get-AzLogicApp](Get-AzLogicApp.md)
Ruft eine Logik-App aus einer Ressourcengruppe ab.

### [Get-AzLogicAppRunAction](Get-AzLogicAppRunAction.md)
Ruft eine Aktion aus einer Logik-App-Ausführung ab.

### [Get-AzLogicAppRunHistory](Get-AzLogicAppRunHistory.md)
Ruft den Ausführungsverlauf einer Logik-App ab.

### [Get-AzLogicAppTrigger](Get-AzLogicAppTrigger.md)
Ruft die Trigger einer Logik-App ab.

### [Get-AzLogicAppTriggerCallbackUrl](Get-AzLogicAppTriggerCallbackUrl.md)
Ruft eine Logik-App-Trigger-Rückruf-URL ab.

### [Get-AzLogicAppTriggerHistory](Get-AzLogicAppTriggerHistory.md)
Ruft den Verlauf von Triggern in einer Logik-App ab.

### [Get-AzLogicAppUpgradedDefinition](Get-AzLogicAppUpgradedDefinition.md)
Ruft die aktualisierte Definition für eine Logik-App ab.

### [Neu – AzIntegrationAccount](New-AzIntegrationAccount.md)
Erstellt ein Integrations Konto.

### [Neu – AzIntegrationAccountAgreement](New-AzIntegrationAccountAgreement.md)
Erstellt eine Integrations Kontovereinbarung.

### [Neu – AzIntegrationAccountAssembly](New-AzIntegrationAccountAssembly.md)
Erstellt eine Integration Account-Assembly.

### [Neu – AzIntegrationAccountBatchConfiguration](New-AzIntegrationAccountBatchConfiguration.md)
Erstellt eine batchkonfiguration für das Integrations Konto.

### [Neu – AzIntegrationAccountCertificate](New-AzIntegrationAccountCertificate.md)
Erstellt ein Integrations Kontozertifikat.

### [Neu – AzIntegrationAccountMap](New-AzIntegrationAccountMap.md)
Erstellt eine Integration-Kontozuordnung.

### [Neu – AzIntegrationAccountPartner](New-AzIntegrationAccountPartner.md)
Erstellt einen Integrations Kontopartner.

### [Neu – AzIntegrationAccountSchema](New-AzIntegrationAccountSchema.md)
Erstellt ein Schema für ein Integrations Konto.

### [Neu – AzLogicApp](New-AzLogicApp.md)
Erstellt eine Logik-app in einer Ressourcengruppe.

### [Remove-AzIntegrationAccount](Remove-AzIntegrationAccount.md)
Entfernt ein Integrations Konto.

### [Remove-AzIntegrationAccountAgreement](Remove-AzIntegrationAccountAgreement.md)
Entfernt eine Integration-Kontovereinbarung.

### [Remove-AzIntegrationAccountAssembly](Remove-AzIntegrationAccountAssembly.md)
Entfernt eine Integration-Konto-Assembly.

### [Remove-AzIntegrationAccountBatchConfiguration](Remove-AzIntegrationAccountBatchConfiguration.md)
Entfernt eine Integration Account-batchkonfiguration.

### [Remove-AzIntegrationAccountCertificate](Remove-AzIntegrationAccountCertificate.md)
Entfernt ein Integrations Kontozertifikat aus einer Ressourcengruppe.

### [Remove-AzIntegrationAccountMap](Remove-AzIntegrationAccountMap.md)
Entfernt eine Integration-Kontozuordnung.

### [Remove-AzIntegrationAccountPartner](Remove-AzIntegrationAccountPartner.md)
Entfernt einen Integrations Kontopartner.

### [Remove-AzIntegrationAccountReceivedIcn](Remove-AzIntegrationAccountReceivedIcn.md)
Dieses Cmdlet entfernt eine bestimmte empfangene Austauschkontrollnummer pro Vereinbarung und Steuernummern Wert.

### [Remove-AzIntegrationAccountSchema](Remove-AzIntegrationAccountSchema.md)
Entfernt ein Schema des Integrations Kontos.

### [Remove-AzLogicApp](Remove-AzLogicApp.md)
Entfernt eine Logik-App aus einer Ressourcengruppe.

### [Satz-AzIntegrationAccount](Set-AzIntegrationAccount.md)
Ändert ein Integrations Konto.

### [Satz-AzIntegrationAccountAgreement](Set-AzIntegrationAccountAgreement.md)
Ändert eine Integration-Kontovereinbarung.

### [Satz-AzIntegrationAccountAssembly](Set-AzIntegrationAccountAssembly.md)
Ändert eine Integration-Konto-Assembly.

### [Satz-AzIntegrationAccountBatchConfiguration](Set-AzIntegrationAccountBatchConfiguration.md)
Ändert die Konfiguration eines Integrations Konto-Batches.

### [Satz-AzIntegrationAccountCertificate](Set-AzIntegrationAccountCertificate.md)
Ändert ein Integrations Kontozertifikat.

### [Satz-AzIntegrationAccountGeneratedIcn](Set-AzIntegrationAccountGeneratedIcn.md)
Aktualisiert die in der Azure-Ressourcengruppe generierte ICN (Interchange Control Number) des Integrations Kontos.

### [Satz-AzIntegrationAccountMap](Set-AzIntegrationAccountMap.md)
Ändert eine Integration-Kontozuordnung.

### [Satz-AzIntegrationAccountPartner](Set-AzIntegrationAccountPartner.md)
Ändert einen Integrations Kontopartner.

### [Satz-AzIntegrationAccountReceivedIcn](Set-AzIntegrationAccountReceivedIcn.md)
Aktualisiert das Integrations Konto, das ICN (Interchange Control Number) in der Azure-Ressourcengruppe erhalten hat.

### [Satz-AzIntegrationAccountSchema](Set-AzIntegrationAccountSchema.md)
Ändert ein Schema des Integrations Kontos.

### [Satz-AzLogicApp](Set-AzLogicApp.md)
Ändert eine Logik-app in einer Ressourcengruppe.

### [Anfang-AzLogicApp](Start-AzLogicApp.md)
Führt eine Logik-app in einer Ressourcengruppe aus.

### [Stopp-AzLogicAppRun](Stop-AzLogicAppRun.md)
Bricht die Ausführung einer Logik-App ab.

### [Test-AzLogicApp](Test-AzLogicApp.md)
Validiert eine Logik-App-Definition.

