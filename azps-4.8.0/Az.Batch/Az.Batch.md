---
Module Name: Az.Batch
Module Guid: a8f00f40-1c1a-49b5-9db3-24076b75c3cf
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.batch
Help Version: 4.0.0.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Batch/Batch/help/Az.Batch.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Batch/Batch/help/Az.Batch.md
ms.openlocfilehash: f6bd55502c5c800bff245b905bc5334b1bf35bdb
ms.sourcegitcommit: 1de2b6c3c99197958fa2101bc37680e7507f91ac
ms.translationtype: MT
ms.contentlocale: de-DE
ms.lasthandoff: 10/13/2020
ms.locfileid: "94008115"
---
# Az.Batch-Modul
## Beschreibung
Mit den Azure Batch-Cmdlets im Azure-Modul können Sie Microsoft Azure-Batch Dienste in Azure PowerShell verwalten.

## Az.Batch-Cmdlets
### [Deaktivieren-AzBatchAutoScale](Disable-AzBatchAutoScale.md)
Deaktiviert die automatische Skalierung eines Pools.

### [Deaktivieren-AzBatchComputeNodeScheduling](Disable-AzBatchComputeNodeScheduling.md)
Deaktiviert die Vorgangsplanung auf dem angegebenen Compute-Knoten.

### [Deaktivieren-AzBatchJob](Disable-AzBatchJob.md)
Deaktiviert eine Stapelverarbeitung.

### [Deaktivieren-AzBatchJobSchedule](Disable-AzBatchJobSchedule.md)
Deaktiviert einen stapelverarbeitungsauftrags Zeitplan.

### [Enable-AzBatchAutoScale](Enable-AzBatchAutoScale.md)
Aktiviert die automatische Skalierung eines Pools.

### [Enable-AzBatchComputeNodeScheduling](Enable-AzBatchComputeNodeScheduling.md)
Aktiviert die Aufgabenplanung auf dem angegebenen Compute-Knoten.

### [Enable-AzBatchJob](Enable-AzBatchJob.md)
Aktiviert eine Stapelverarbeitung.

### [Enable-AzBatchJobSchedule](Enable-AzBatchJobSchedule.md)
Aktiviert einen Stapel Auftragszeitplan.

### [Enable-AzBatchTask](Enable-AzBatchTask.md)
Reaktiviert eine Aufgabe.

### [Get-AzBatchAccount](Get-AzBatchAccount.md)
Ruft ein Batch Konto im aktuellen Abonnement ab.

### [Get-AzBatchAccountKey](Get-AzBatchAccountKey.md)
Ruft die Schlüssel eines Batch Kontos ab.

### [Get-AzBatchApplication](Get-AzBatchApplication.md)
Ruft Informationen zur angegebenen Anwendung ab.

### [Get-AzBatchApplicationPackage](Get-AzBatchApplicationPackage.md)
Ruft Informationen zu einem Anwendungspaket in einem Batch Konto ab.

### [Get-AzBatchCertificate](Get-AzBatchCertificate.md)
Ruft die Zertifikate in einem Batch Konto ab.

### [Get-AzBatchComputeNode](Get-AzBatchComputeNode.md)
Ruft Batch-Compute-Knoten aus einem Pool ab.

### [Get-AzBatchJob](Get-AzBatchJob.md)
Ruft Stapelverarbeitungsaufträge für ein Stapelverarbeitungs Konto oder einen Auftragszeitplan ab.

### [Get-AzBatchJobPreparationAndReleaseTaskStatus](Get-AzBatchJobPreparationAndReleaseTaskStatus.md)
Ruft die Vorbereitung des stapelverarbeitungsauftrags ab und gibt den Aufgabenstatus frei.

### [Get-AzBatchJobSchedule](Get-AzBatchJobSchedule.md)
Ruft Stapel Auftragszeitpläne ab.

### [Get-AzBatchJobStatistic](Get-AzBatchJobStatistic.md)
Ruft Auftrags Zusammenfassungs Statistiken für ein Stapelverarbeitungs Konto ab.

### [Get-AzBatchLocationQuota](Get-AzBatchLocationQuota.md)
Ruft die Stapelverarbeitungs Dienst Kontingente für Ihr Abonnement an der angegebenen Position ab.

### [Get-AzBatchNodeFile](Get-AzBatchNodeFile.md)
Ruft die Eigenschaften von Batch-Knoten Dateien ab.

### [Get-AzBatchNodeFileContent](Get-AzBatchNodeFileContent.md)
Ruft eine Batch-Knoten Datei ab.

### [Get-AzBatchPool](Get-AzBatchPool.md)
Ruft Batch Pools unter dem angegebenen Batch Konto ab.

### [Get-AzBatchPoolNodeCount](Get-AzBatchPoolNodeCount.md)
Ruft Batch-Knoten Zähler pro Knotenzustand nach Pool-ID ab.

### [Get-AzBatchPoolStatistic](Get-AzBatchPoolStatistic.md)
Ruft Pool Zusammenfassungs Statistiken für ein Stapelverarbeitungs Konto ab.

### [Get-AzBatchPoolUsageMetric](Get-AzBatchPoolUsageMetric.md)
Ruft Metriken für die Poolnutzung für ein Stapelverarbeitungs Konto ab.

### [Get-AzBatchRemoteDesktopProtocolFile](Get-AzBatchRemoteDesktopProtocolFile.md)
Ruft eine RDP-Datei von einem Compute-Knoten ab.

### [Get-AzBatchRemoteLoginSetting](Get-AzBatchRemoteLoginSetting.md)
Ruft die Remote Anmeldeeinstellungen für einen Compute-Knoten ab.

### [Get-AzBatchSubtask](Get-AzBatchSubtask.md)
Ruft die Untervorgangs Informationen der angegebenen Aufgabe ab.

### [Get-AzBatchSupportedImage](Get-AzBatchSupportedImage.md)
Ruft Batch gestützte Bilder für ein Stapelverarbeitungs Konto ab.

### [Get-AzBatchTask](Get-AzBatchTask.md)
Ruft die Stapelverarbeitungsaufgaben für einen Auftrag ab.

### [Get-AzBatchTaskCount](Get-AzBatchTaskCount.md)
Ruft die Aufgabenanzahl für den angegebenen Auftrag ab.

### [Neu – AzBatchAccount](New-AzBatchAccount.md)
Erstellt ein Stapelverarbeitungs Konto.

### [Neu – AzBatchAccountKey](New-AzBatchAccountKey.md)
Generiert einen Schlüssel eines Batch Kontos erneut.

### [Neu – AzBatchApplication](New-AzBatchApplication.md)
Fügt dem angegebenen Batch Konto eine Anwendung hinzu.

### [Neu – AzBatchApplicationPackage](New-AzBatchApplicationPackage.md)
Erstellt ein Anwendungspaket in einem Stapelverarbeitungs Konto.

### [Neu – AzBatchCertificate](New-AzBatchCertificate.md)
Fügt dem angegebenen Batch Konto ein Zertifikat hinzu.

### [Neu – AzBatchComputeNodeUser](New-AzBatchComputeNodeUser.md)
Erstellt ein Benutzerkonto in einem Batch Compute-Knoten.

### [Neu – AzBatchJob](New-AzBatchJob.md)
Erstellt einen Auftrag im Stapelverarbeitungs Dienst.

### [Neu – AzBatchJobSchedule](New-AzBatchJobSchedule.md)
Erstellt einen Auftragszeitplan im Stapelverarbeitungs Dienst.

### [Neu – AzBatchPool](New-AzBatchPool.md)
Erstellt einen Pool im Stapelverarbeitungs Dienst.

### [Neu – AzBatchResourceFile](New-AzBatchResourceFile.md)
Erstellt eine Ressourcendatei zur Verwendung durch `New-AzBatchTask` .

### [Neu – AzBatchTask](New-AzBatchTask.md)
Erstellt eine Batch Aufgabe unter einem Auftrag.

### [Remove-AzBatchAccount](Remove-AzBatchAccount.md)
Entfernt ein Stapelverarbeitungs Konto.

### [Remove-AzBatchApplication](Remove-AzBatchApplication.md)
Löscht eine Anwendung aus einem Stapelverarbeitungs Konto.

### [Remove-AzBatchApplicationPackage](Remove-AzBatchApplicationPackage.md)
Löscht einen Anwendungspaket-Datensatz und die binäre Datei.

### [Remove-AzBatchCertificate](Remove-AzBatchCertificate.md)
Löscht ein Zertifikat von einem Konto.

### [Remove-AzBatchComputeNode](Remove-AzBatchComputeNode.md)
Entfernt Compute-Knoten aus einem Pool.

### [Remove-AzBatchComputeNodeUser](Remove-AzBatchComputeNodeUser.md)
Löscht ein Benutzerkonto aus einem Batch Compute-Knoten.

### [Remove-AzBatchJob](Remove-AzBatchJob.md)
Löscht eine Stapelverarbeitung.

### [Remove-AzBatchJobSchedule](Remove-AzBatchJobSchedule.md)
Entfernt einen stapelverarbeitungsauftrags Zeitplan.

### [Remove-AzBatchNodeFile](Remove-AzBatchNodeFile.md)
Löscht eine Knoten Datei für einen Vorgang oder einen Compute-Knoten.

### [Remove-AzBatchPool](Remove-AzBatchPool.md)
Löscht den angegebenen Batch Pool.

### [Remove-AzBatchTask](Remove-AzBatchTask.md)
Löscht eine Batch Aufgabe.

### [Reset-AzBatchComputeNode](Reset-AzBatchComputeNode.md)
Neuinstallation des Betriebssystems auf dem angegebenen Compute-Knoten.

### [Neustart-AzBatchComputeNode](Restart-AzBatchComputeNode.md)
Startet den angegebenen Compute-Knoten neu.

### [Satz-AzBatchAccount](Set-AzBatchAccount.md)
Aktualisiert ein Stapelverarbeitungs Konto.

### [Satz-AzBatchApplication](Set-AzBatchApplication.md)
Aktualisiert die Einstellungen für die angegebene Anwendung.

### [Satz-AzBatchComputeNodeUser](Set-AzBatchComputeNodeUser.md)
Ändert die Eigenschaften eines Kontos in einem Batch Compute-Knoten.

### [Satz-AzBatchJob](Set-AzBatchJob.md)
Aktualisiert eine Stapelverarbeitung.

### [Satz-AzBatchJobSchedule](Set-AzBatchJobSchedule.md)
Legt einen Auftragszeitplan fest.

### [Satz-AzBatchPool](Set-AzBatchPool.md)
Aktualisiert die Eigenschaften eines Pools.

### [Satz-AzBatchTask](Set-AzBatchTask.md)
Aktualisiert die Eigenschaften eines Vorgangs.

### [Anfang-AzBatchComputeNodeServiceLogUpload](Start-AzBatchComputeNodeServiceLogUpload.md)
Hochladen von Protokolldateien des Compute-Knoten Diensts in einen Azure-Speichercontainer

### [Anfang-AzBatchPoolResize](Start-AzBatchPoolResize.md)
Beginnt, die Größe eines Pools zu ändern.

### [Stopp-AzBatchCertificateDeletion](Stop-AzBatchCertificateDeletion.md)
Bricht einen Fehler beim Löschen eines Zertifikats ab.

### [Stopp-AzBatchJob](Stop-AzBatchJob.md)
Beendet eine Stapelverarbeitung.

### [Stopp-AzBatchJobSchedule](Stop-AzBatchJobSchedule.md)
Beendet einen stapelverarbeitungsauftrags Zeitplan.

### [Stopp-AzBatchPoolResize](Stop-AzBatchPoolResize.md)
Beendet die Größe eines Pool-Vorgangs.

### [Stopp-AzBatchTask](Stop-AzBatchTask.md)
Beendet eine Batch Aufgabe.

### [Test-AzBatchAutoScale](Test-AzBatchAutoScale.md)
Ruft das Ergebnis einer automatischen Skalierungs Formel in einem Pool ab.

