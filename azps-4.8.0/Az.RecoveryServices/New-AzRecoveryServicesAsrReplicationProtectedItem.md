...
Startet den Replikations geschützten Element Erstellungsvorgang für das angegebene ASR-Schutzelement, einschließlich selektiver Datenträger, und gibt den ASR-Auftrag zurück, der zum Nachvollziehen des Vorgangs (vmWare to Azure-Szenario) mit ausgewählten Datenträgern verwendet wird.
...
Startet den Replikations geschützten Element Erstellungsvorgang für den angegebenen VmId und gibt den ASR-Auftrag zurück, der zum Nachvollziehen des Vorgangs verwendet wird (Azure to Azure-Szenario). Für die Failover-VM-Details, die in Cmdlet für Verschlüsselung übergeben werden, werden verwendet.
...
Startet den Replikations geschützten Element Erstellungsvorgang für einen virtuellen Computer innerhalb der Näherungs Platzierungs Gruppe und gibt den ASR-Auftrag zurück, der zum Nachvollziehen des Vorgangs verwendet wird (Azure to Azure-Szenario).
...
### -AzureToAzure
Parameter "Switch" gibt an, dass das replizierte Element in Azure to Azure-Szenario erstellt wird.
...
Gibt die Datenträgerkonfiguration für die Verwendung von VM für Azure to Azure Disaster Recovery-Szenario an.
...
### -AzureVmId
Gibt die Azure VM-ID für den Disaster Recovery-Schutz im Wiederherstellungsbereich an.
...
### -Disktype
Gibt den Typ der verwalteten VM-Wiederherstellungsfestplatte an.
...
Gibt die Datenträgerkonfiguration für vMWare Disk ID an, die vor dem angegebenen, geschützten Element geschützt werden soll.
...
### -KeyEncryptionKeyUrl
Gibt die URL des Datenträger Verschlüsselungsschlüssels mit der Version (Azure Disk Encryption) an, die nach dem Failover als Wiederherstellungs-VM verwendet werden soll.
...
Gibt den Schlüssel für die Datenträgerverschlüsselung an – Vault-ID (Azure Disk Encryption), die nach dem Failover als Wiederherstellungs-VM verwendet werden soll.
...
Gibt die verfügbarkeitszone des Wiederherstellungs-VM nach einem Failover an.
...
Geben Sie die ID der Näherungs Platzierungs Gruppe an, die vom Failover-VM im Ziel Wiederherstellungsbereich verwendet wird.
