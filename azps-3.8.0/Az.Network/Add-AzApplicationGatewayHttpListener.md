...
## Beschreibung
Mit dem Cmdlet **Add-AzApplicationGatewayHttpListener** wird ein HTTP-Listener zu einem Anwendungsgateway hinzugefügt.
...
Der erste Befehl ruft das Anwendungsgateway ab und speichert es in der $AppGw-Variablen. Mit dem zweiten Befehl wird der HTTP-Listener dem Anwendungsgateway hinzugefügt.

### Beispiel 2: Hinzufügen eines HTTPS-Listener mit SSL
```
PS C:\>$AppGw = Get-AzApplicationGateway -Name "ApplicationGateway01" -ResourceGroupName "ResourceGroup01"
...
Mit dem zweiten Befehl wird der Listener, der das HTTPS-Protokoll verwendet, zum Anwendungsgateway hinzugefügt.

### Beispiel 3: Hinzufügen eines HTTPS-Listener mit SSL und Hostnamen
```
PS C:\>$AppGw = Get-AzApplicationGateway -Name "ApplicationGateway01" -ResourceGroupName "ResourceGroup01"
...
Der erste Befehl ruft das Anwendungsgateway ab und speichert es in der $AppGw-Variablen.
Mit dem zweiten Befehl wird der Listener, der das HTTPS-Protokoll verwendet, mit SSL-Zertifikaten und-Hostnamen zum Anwendungsgateway hinzugefügt.
...
Gibt das Front-End-IP-Ressourcenobjekt des Application Gateway an.
...
Gibt die Front-End-IP-ID des Application Gateway an.
...
### -FrontendPort
Gibt das Front-End-Port Objekt des Application Gateway an.
...
### -Protokoll
Gibt das Protokoll des HTTP-Listener an.
...
Muss angegeben werden, wenn HTTPS als Listener-Protokoll ausgewählt wird.
...
### -SslCertificateId
Gibt die SSL-Zertifikat-ID des HTTP-Listener an.
Muss angegeben werden, wenn HTTPS als Listener-Protokoll ausgewählt wird.
