---
Module Name: Az.Websites
Module Guid: cc69c625-e961-43f4-8b50-0061eba6e4b6
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.websites
Help Version: 1.0.0.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Websites/Websites/help/Az.Websites.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Websites/Websites/help/Az.Websites.md
ms.openlocfilehash: 174da8669c84442f24866b8ab8831ee70d2d239d
ms.sourcegitcommit: 6a91b4c545350d316d3cf8c62f384478e3f3ba24
ms.translationtype: MT
ms.contentlocale: de-DE
ms.lasthandoff: 04/21/2020
ms.locfileid: "93996380"
---
# AZ. Websites-Modul
## Beschreibung
Arm (Azure Resource Manager) Web App-und App-Service Plan-Befehle.

## AZ. Websites-Cmdlets
### [Bearbeiten-AzWebAppBackupConfiguration](Edit-AzWebAppBackupConfiguration.md)


### [Enter-AzWebAppContainerPSSession](Enter-AzWebAppContainerPSSession.md)
Öffnet eine PowerShell-Remotesitzung in dem Windows-Container, der in einer bestimmten Website oder in einem bestimmten Steckplatz und einer gegebenen Ressourcengruppe angegeben ist.

### [Get-AzAppServicePlan](Get-AzAppServicePlan.md)
Ruft einen Azure-App-Dienstplan in der angegebenen Ressourcengruppe ab.

### [Get-AzAppServicePlanMetric](Get-AzAppServicePlanMetric.md)


### [Get-AzDeletedWebApp](Get-AzDeletedWebApp.md)
Ruft gelöschte Web-Apps im Abonnement ab.

### [Get-AzWebApp](Get-AzWebApp.md)
Ruft Azure Web Apps in der angegebenen Ressourcengruppe ab.

### [Get-AzWebAppBackup](Get-AzWebAppBackup.md)


### [Get-AzWebAppBackupConfiguration](Get-AzWebAppBackupConfiguration.md)


### [Get-AzWebAppBackupList](Get-AzWebAppBackupList.md)


### [Get-AzWebAppCertificate](Get-AzWebAppCertificate.md)
Ruft ein Azure Web App-Zertifikat ab.

### [Get-AzWebAppContainerContinuousDeploymentUrl](Get-AzWebAppContainerContinuousDeploymentUrl.md)
Get-AzWebAppContainerContinuousDeploymentUrl gibt die URL der fortlaufenden Container Bereitstellung zurück

### [Get-AzWebAppMetric](Get-AzWebAppMetric.md)
Ruft Azure Web App-Metriken ab.

### [Get-AzWebAppPublishingProfile](Get-AzWebAppPublishingProfile.md)
Ruft ein Azure Web App-Veröffentlichungsprofil ab.

### [Get-AzWebAppSlot](Get-AzWebAppSlot.md)
Ruft einen Azure Web App-Steckplatz ab.

### [Get-AzWebAppSlotConfigName](Get-AzWebAppSlotConfigName.md)
Abrufen der Liste der config-Namen für Web App-Steckplätze

### [Get-AzWebAppSlotMetric](Get-AzWebAppSlotMetric.md)
Ruft Metriken für einen Azure Web App-Slot ab.

### [Get-AzWebAppSlotPublishingProfile](Get-AzWebAppSlotPublishingProfile.md)
Ruft ein Azure Web App-Slot-Veröffentlichungsprofil ab.

### [Get-AzWebAppSnapshot](Get-AzWebAppSnapshot.md)
Ruft die für eine Web-App verfügbaren Snapshots ab.

### [Get-AzWebAppSSLBinding](Get-AzWebAppSSLBinding.md)
Ruft eine Azure Web App-Zertifikat SSL-Bindung ab.

### [Neu – AzAppServicePlan](New-AzAppServicePlan.md)
Erstellt einen Azure App-Service Plan an einem bestimmten Geo-Standort.

### [Neu – AzWebApp](New-AzWebApp.md)
Erstellt eine Azure Web App.

### [Neu – AzWebAppAzureStoragePath](New-AzWebAppAzureStoragePath.md)
Erstellt ein Objekt, das einen Azure-Speicherpfad darstellt, der in einer Web-App bereitgestellt werden soll. Sie ist als Parameter (-AzureStoragePath) zum Set-AzWebApp und Set-AzWebAppSlot

### [Neu – AzWebAppBackup](New-AzWebAppBackup.md)


### [Neu – AzWebAppContainerPSSession](New-AzWebAppContainerPSSession.md)
New-AzWebAppContainerPSSession erstellt eine neue Remote-PowerShell-Sitzung in dem Windows-Container, der in einer bestimmten Website oder in einem bestimmten Steckplatz und einer gegebenen Ressourcengruppe angegeben ist.

### [Neu – AzWebAppDatabaseBackupSetting](New-AzWebAppDatabaseBackupSetting.md)


### [Neu – AzWebAppSlot](New-AzWebAppSlot.md)
Erstellt einen Azure Web App-Steckplatz.

### [Neu – AzWebAppSSLBinding](New-AzWebAppSSLBinding.md)
Erstellt eine SSL-Zertifikat Bindung für eine Azure Web App.

### [Publish-AzWebApp](Publish-AzWebApp.md)
Stellt eine Azure Web-App aus einer ZIP-, JAR-oder war-Datei mithilfe von zipdeploy bereit.

### [Remove-AzAppServicePlan](Remove-AzAppServicePlan.md)
Entfernt einen Azure App-Service Plan.

### [Remove-AzWebApp](Remove-AzWebApp.md)
Entfernt eine Azure Web-App.

### [Remove-AzWebAppBackup](Remove-AzWebAppBackup.md)


### [Remove-AzWebAppSlot](Remove-AzWebAppSlot.md)


### [Remove-AzWebAppSSLBinding](Remove-AzWebAppSSLBinding.md)
Entfernt eine SSL-Bindung aus einem hochgeladenen Zertifikat.

### [Reset-AzWebAppPublishingProfile](Reset-AzWebAppPublishingProfile.md)


### [Reset-AzWebAppSlotPublishingProfile](Reset-AzWebAppSlotPublishingProfile.md)


### [Neustart-AzWebApp](Restart-AzWebApp.md)
Startet eine Azure Web App neu.

### [Neustart-AzWebAppSlot](Restart-AzWebAppSlot.md)


### [Wiederherstellen – AzDeletedWebApp](Restore-AzDeletedWebApp.md)
Stellt eine gelöschte Web-App in einer neuen oder vorhandenen Web-App wieder her.

### [Wiederherstellen – AzWebAppBackup](Restore-AzWebAppBackup.md)


### [Wiederherstellen – AzWebAppSnapshot](Restore-AzWebAppSnapshot.md)
Stellt einen Web-App-Schnappschuss wieder her.

### [Satz-AzAppServicePlan](Set-AzAppServicePlan.md)
Legt einen Azure-App-Service Plan fest.

### [Satz-AzWebApp](Set-AzWebApp.md)
Ändert eine Azure Web App.

### [Satz-AzWebAppSlot](Set-AzWebAppSlot.md)
Ändert einen Azure Web App-Steckplatz.

### [Satz-AzWebAppSlotConfigName](Set-AzWebAppSlotConfigName.md)
Einrichten von config-Namen für Web App-Steckplätze

### [Anfang-AzWebApp](Start-AzWebApp.md)
Startet eine Azure Web App.

### [Anfang-AzWebAppSlot](Start-AzWebAppSlot.md)
Startet einen Azure Web App-Steckplatz.

### [Stopp-AzWebApp](Stop-AzWebApp.md)
Beendet eine Azure Web App.

### [Stopp-AzWebAppSlot](Stop-AzWebAppSlot.md)
Beendet einen Azure Web App-Steckplatz.

### [Switch-AzWebAppSlot](Switch-AzWebAppSlot.md)
Austauschen von zwei Slots mit einer Web-App

### [Get-AzWebAppAccessRestrictionConfig](Get-AzWebAppAccessRestrictionConfig.md)
Ruft die Zugriffs Einschränkungseinstellungen einer Azure Web App ab.

### [Update-AzWebAppAccessRestrictionConfig](Update-AzWebAppAccessRestrictionConfig.md)
Aktualisieren Sie die Konfiguration der SCM-Website Vererbung für eine Azure Web App.

### [Add-AzWebAppAccessRestrictionRule](Add-AzWebAppAccessRestrictionRule.md)
Fügt eine Zugriffseinschränkung zu einer Azure Web App hinzu.

### [Remove-AzWebAppAccessRestrictionRule](Remove-AzWebAppAccessRestrictionRule.md)
Entfernt eine Zugriffsbeschränkung auf eine Azure Web App