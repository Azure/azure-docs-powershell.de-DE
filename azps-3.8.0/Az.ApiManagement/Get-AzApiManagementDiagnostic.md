...
## Beschreibung
Der **Get-AzApiManagementDiagnostic** ruft Details der Diagnose ab, die im API-Verwaltungsdienst in einem bestimmten Bereich konfiguriert wurde.

## Beispiele

### Beispiel 1: Abrufen aller für den MANDANTENBEREICH konfigurierten Diagnosen
```powershell
...
Dieser Befehl ruft alle Diagnosen ab, die im API-Verwaltungsdienst konfiguriert sind.

### Beispiel 2: Abrufen aller für den API-Bereich konfigurierten Diagnosen
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Dieser Befehl ruft alle Diagnosen ab, die im `echo-api` API-Bereich konfiguriert sind.

### Beispiel 3: Abrufen der durch eine ID angegebenen API-Scope-Diagnose
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Mit diesem Befehl wird die `applicationinsights` in API konfigurierte Diagnose abgerufen `echo-api` .

## Parameter

### -ApiId
Bezeichner der vorhandenen API.
Wenn angegeben, wird die API-Scope-Diagnose zurückgegeben.
...
### -Diagnose-Nr
Bezeichner der vorhandenen Diagnose.
Wenn angegeben, wird die Richtlinie für den Produktbereich zurückgegeben.
...
### -Resourcen-Nr
Arm-Ressourcenbezeichner einer Diagnose-oder API-Diagnose. Wenn angegeben, wird versucht, die Diagnose nach dem Bezeichner zu finden. Dieser Parameter ist erforderlich.
