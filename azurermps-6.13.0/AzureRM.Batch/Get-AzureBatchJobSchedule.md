...
## Beschreibung
Das Cmdlet " **Get-AzureBatchJobSchedule** " ruft Azure-batchauftrags Zeitpläne für das vom *batchcontext* -Parameter angegebene Batch Konto ab.
Geben Sie eine ID an, um einen einzelnen Auftragszeitplan abzurufen.
Geben Sie den *Filter* -Parameter an, um die Auftragszeitpläne abzurufen, die einem Open Data Protocol (OData)-Filter entsprechen.

## Beispiele

### Beispiel 1: Abrufen eines Auftragszeitplans durch Angabe einer ID
```
...
Dieser Befehl ruft den Auftragszeitplan ab, der die ID JobSchedule23.
Verwenden Sie das Get-AzureRmBatchAccountKeys-Cmdlet, um der $Context Variablen einen Kontext zuzuweisen.

### Beispiel 2: Abrufen von Auftrags Terminplänen mithilfe eines Filters
```
...
Dieser Befehl ruft alle Auftragszeitpläne ab, die IDs aufweisen, die mit Job beginnen, indem Sie den Parameter *Filter* angeben.
...
Dieses Cmdlet gibt Auftragszeitpläne zurück, die dem Filter entsprechen, den dieser Parameter angibt.
Wenn Sie keinen Filter angeben, gibt dieses Cmdlet alle Auftragszeitpläne für den Batch Kontext zurück.
...
### -ID
Gibt die ID des Auftragszeitplans an, den dieses Cmdlet erhält.
...
### -MaxCount
Gibt die maximale Anzahl von Auftrags Terminen an, die zurückgegeben werden sollen.
