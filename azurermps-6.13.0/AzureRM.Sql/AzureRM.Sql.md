---
Module Name: AzureRM.Sql
Module Guid: 150d9544-6348-4373-806f-10cd0b4de4cb
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/azurerm.sql
Help Version: 4.4.0.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/preview/src/ResourceManager/Sql/Commands.Sql/help/AzureRM.Sql.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/preview/src/ResourceManager/Sql/Commands.Sql/help/AzureRM.Sql.md
ms.openlocfilehash: 272d0d6605b9c9eb13ccfd31c97c7890d123cc28
ms.sourcegitcommit: 0b644bfecf4224b2ea83520d1a6a956734d9fba4
ms.translationtype: MT
ms.contentlocale: de-DE
ms.lasthandoff: 07/12/2019
ms.locfileid: "93474545"
---
# AzureRM. SQL-Modul
## Beschreibung
In diesem Thema werden Hilfethemen zu den Azure SQL-Datenbank-Cmdlets angezeigt.

## AzureRM. SQL-Cmdlets
### [Add-AzureRmSqlDatabaseToFailoverGroup](Add-AzureRmSqlDatabaseToFailoverGroup.md)
Fügt eine oder mehrere Datenbanken zu einer Azure SQL-Daten Bank Failover-Gruppe hinzu.

### [Add-AzureRmSqlManagedInstanceTransparentDataEncryptionCertificate](Add-AzureRmSqlManagedInstanceTransparentDataEncryptionCertificate.md)
Hinzufügen eines transparenten Daten Verschlüsselungszertifikats für die angegebene verwaltete Instanz

### [Add-AzureRmSqlServerKeyVaultKey](Add-AzureRmSqlServerKeyVaultKey.md)
Fügt einen schlüsseltresor-Schlüssel zu einem SQL Server hinzu.

### [Add-AzureRmSqlServerTransparentDataEncryptionCertificate](Add-AzureRmSqlServerTransparentDataEncryptionCertificate.md)
Hinzufügen eines transparenten Daten Verschlüsselungszertifikats für die angegebene SQL Server-Instanz

### [Löschen-AzureRmSqlDatabaseVulnerabilityAssessmentRuleBaseline](Clear-AzureRmSqlDatabaseVulnerabilityAssessmentRuleBaseline.md)
Löscht die Richtlinie für die Anfälligkeitsbewertung.

### [Löschen-AzureRmSqlDatabaseVulnerabilityAssessmentSettings](Clear-AzureRmSqlDatabaseVulnerabilityAssessmentSettings.md)
Löscht die Einstellungen für die Sicherheitsrisikobewertung einer Datenbank.

### [Konvertieren-AzureRmSqlDatabaseVulnerabilityAssessmentScan](Convert-AzureRmSqlDatabaseVulnerabilityAssessmentScan.md)
Wandelt die Ergebnisse einer Schwachstellenbewertung in das Excel-Format um.

### [Deaktivieren-AzureRmSqlServerAdvancedThreatProtection](Disable-AzureRmSqlServerAdvancedThreatProtection.md)
Deaktiviert den erweiterten Bedrohungsschutz auf einem Server.

### [Enable-AzureRmSqlServerAdvancedThreatProtection](Enable-AzureRmSqlServerAdvancedThreatProtection.md)
Ermöglicht erweiterten Bedrohungsschutz auf einem Server.

### [Get-AzureRmSqlCapability](Get-AzureRmSqlCapability.md)
Ruft SQL-Datenbankfunktionen für das aktuelle Abonnement ab.

### [Get-AzureRmSqlDatabase](Get-AzureRmSqlDatabase.md)
Ruft mindestens eine Datenbank ab.

### [Get-AzureRmSqlDatabaseActivity](Get-AzureRmSqlDatabaseActivity.md)
Ruft den Status von Datenbankvorgängen ab.

### [Get-AzureRmSqlDatabaseAdvisor](Get-AzureRmSqlDatabaseAdvisor.md)
Ruft mindestens einen Ratgeber für eine Azure SQL-Datenbank ab.

### [Get-AzureRmSqlDatabaseAuditing](Get-AzureRmSqlDatabaseAuditing.md)
Ruft die Überwachungseinstellungen einer Azure SQL-Datenbank ab.

### [Get-AzureRmSqlDatabaseAuditingPolicy](Get-AzureRmSqlDatabaseAuditingPolicy.md)
Ruft die Überwachungsrichtlinie einer Datenbank ab.

### [Get-AzureRmSqlDatabaseBackupLongTermRetentionPolicy](Get-AzureRmSqlDatabaseBackupLongTermRetentionPolicy.md)
Ruft eine Datenbank-langfristige Aufbewahrungsrichtlinie ab.

### [Get-AzureRmSqlDatabaseDataMaskingPolicy](Get-AzureRmSqlDatabaseDataMaskingPolicy.md)
Ruft die Daten Maskierungs Richtlinie für eine Datenbank ab.

### [Get-AzureRmSqlDatabaseDataMaskingRule](Get-AzureRmSqlDatabaseDataMaskingRule.md)
Ruft die Regeln für die Daten Maskierung aus einer Datenbank ab.

### [Get-AzureRmSqlDatabaseExpanded](Get-AzureRmSqlDatabaseExpanded.md)
Ruft eine Datenbank und deren erweiterte Eigenschaftswerte ab.

### [Get-AzureRmSqlDatabaseFailoverGroup](Get-AzureRmSqlDatabaseFailoverGroup.md)
Ruft Azure SQL-Datenbank-failovergruppen ab oder listet Sie auf.

### [Get-AzureRmSqlDatabaseGeoBackup](Get-AzureRmSqlDatabaseGeoBackup.md)
Ruft eine Geo-redundante Sicherung einer Datenbank ab.

### [Get-AzureRmSqlDatabaseGeoBackupPolicy](Get-AzureRmSqlDatabaseGeoBackupPolicy.md)
Ruft eine Daten Bank Geo-Sicherungsrichtlinie ab.

### [Get-AzureRmSqlDatabaseImportExportStatus](Get-AzureRmSqlDatabaseImportExportStatus.md)
Ruft die Details eines Imports oder Exports einer Azure SQL-Datenbank ab.

### [Get-AzureRmSqlDatabaseIndexRecommendations](Get-AzureRmSqlDatabaseIndexRecommendations.md)
Ruft die empfohlenen Indexvorgänge für einen Server oder eine Datenbank ab.

### [Get-AzureRmSqlDatabaseLongTermRetentionBackup](Get-AzureRmSqlDatabaseLongTermRetentionBackup.md)
Ruft eine oder mehrere langfristige Aufbewahrungs Sicherungen ab.

### [Get-AzureRmSqlDatabaseRecommendedAction](Get-AzureRmSqlDatabaseRecommendedAction.md)
Ruft eine oder mehrere Empfohlene Aktionen für einen Azure SQL-Daten Bank Ratgeber ab.

### [Get-AzureRmSqlDatabaseReplicationLink](Get-AzureRmSqlDatabaseReplicationLink.md)
Ruft die Geo-Replikationsverknüpfungen zwischen einer Azure SQL-Datenbank und einer Ressourcengruppe oder SQL Server ab.

### [Get-AzureRmSqlDatabaseRestorePoints](Get-AzureRmSqlDatabaseRestorePoints.md)
Ruft die eindeutigen Wiederherstellungspunkte ab, aus denen ein SQL Data Warehouse wiederhergestellt werden kann.

### [Get-AzureRmSqlDatabaseSecureConnectionPolicy](Get-AzureRmSqlDatabaseSecureConnectionPolicy.md)
Ruft die sichere Verbindungsrichtlinie für eine Datenbank ab.

### [Get-AzureRmSqlDatabaseThreatDetectionPolicy](Get-AzureRmSqlDatabaseThreatDetectionPolicy.md)
Ruft die Richtlinie für die Bedrohungserkennung für eine Datenbank ab.

### [Get-AzureRmSqlDatabaseTransparentDataEncryption](Get-AzureRmSqlDatabaseTransparentDataEncryption.md)
Ruft den Status "DSA" für eine Datenbank ab.

### [Get-AzureRmSqlDatabaseTransparentDataEncryptionActivity](Get-AzureRmSqlDatabaseTransparentDataEncryptionActivity.md)
Ruft den Status eines DSA-Scans einer Datenbank ab.

### [Get-AzureRmSqlDatabaseUpgradeHint](Get-AzureRmSqlDatabaseUpgradeHint.md)
Ruft Preisstufen Hinweise für eine Datenbank ab.

### [Get-AzureRmSqlDatabaseVulnerabilityAssessmentRuleBaseline](Get-AzureRmSqlDatabaseVulnerabilityAssessmentRuleBaseline.md)
Ruft den Basisplan für die Anfälligkeitsbewertung ab.

### [Get-AzureRmSqlDatabaseVulnerabilityAssessmentScanRecord](Get-AzureRmSqlDatabaseVulnerabilityAssessmentScanRecord.md)
Ruft alle mit einer bestimmten Datenbank verknüpften Datensätze für die Vulnerability Assessment-Überprüfung ab.

### [Get-AzureRmSqlDatabaseVulnerabilityAssessmentSettings](Get-AzureRmSqlDatabaseVulnerabilityAssessmentSettings.md)
Ruft die Einstellungen für die Sicherheitsrisikobewertung einer Datenbank ab.

### [Get-AzureRmSqlDeletedDatabaseBackup](Get-AzureRmSqlDeletedDatabaseBackup.md)
Ruft eine gelöschte Datenbank ab, die Sie wiederherstellen können.

### [Get-AzureRmSqlElasticPool](Get-AzureRmSqlElasticPool.md)
Ruft elastische Pools und ihre Eigenschaftswerte in einer Azure SQL-Datenbank ab.

### [Get-AzureRmSqlElasticPoolActivity](Get-AzureRmSqlElasticPoolActivity.md)
Ruft den Status von Vorgängen in einem elastischen Pool ab.

### [Get-AzureRmSqlElasticPoolAdvisor](Get-AzureRmSqlElasticPoolAdvisor.md)
Ruft einen oder mehrere Ratgeber für einen Azure SQL-elastischen Pool ab.

### [Get-AzureRmSqlElasticPoolDatabase](Get-AzureRmSqlElasticPoolDatabase.md)
Ruft elastische Datenbanken in einem elastischen Pool und deren Eigenschaftenwerten ab.

### [Get-AzureRmSqlElasticPoolRecommendation](Get-AzureRmSqlElasticPoolRecommendation.md)
Erhält Empfehlungen für elastische Pools.

### [Get-AzureRmSqlElasticPoolRecommendedAction](Get-AzureRmSqlElasticPoolRecommendedAction.md)
Ruft eine oder mehrere Empfohlene Aktionen für einen Azure SQL Elastic Pool Advisor ab.

### [Get-AzureRmSqlInstance](Get-AzureRmSqlInstance.md)
Gibt Informationen zu Azure SQL-Datenbankinstanz zurück.

### [Get-AzureRmSqlInstanceDatabase](Get-AzureRmSqlInstanceDatabase.md)
Gibt Informationen zur Datenbank der Azure SQL-Instanz zurück.

### [Get-AzureRmSqlServer](Get-AzureRmSqlServer.md)
Gibt Informationen zu SQL-Datenbankservern zurück.

### [Get-AzureRmSqlServerActiveDirectoryAdministrator](Get-AzureRmSqlServerActiveDirectoryAdministrator.md)
Ruft Informationen zu einem Azure AD-Administrator für SQL Server ab.

### [Get-AzureRmSqlServerAdvancedThreatProtectionPolicy](Get-AzureRmSqlServerAdvancedThreatProtectionPolicy.md)
Ruft erweiterte Bedrohungsschutz Richtlinien eines Servers ab.

### [Get-AzureRmSqlServerAdvisor](Get-AzureRmSqlServerAdvisor.md)
Ruft einen oder mehrere Ratgeber für einen Azure SQL Server ab.

### [Get-AzureRmSqlServerAuditing](Get-AzureRmSqlServerAuditing.md)
Ruft die Überwachungseinstellungen eines Azure SQL Server ab.

### [Get-AzureRmSqlServerAuditingPolicy](Get-AzureRmSqlServerAuditingPolicy.md)
Ruft die Überwachungsrichtlinie eines SQL Server ab.

### [Get-AzureRmSqlServerBackupLongTermRetentionVault](Get-AzureRmSqlServerBackupLongTermRetentionVault.md)
Ruft einen langfristigen Aufbewahrungs Tresor für Server ab.

### [Get-AzureRmSqlServerCommunicationLink](Get-AzureRmSqlServerCommunicationLink.md)
Ruft Kommunikationslinks für elastische Datenbanktransaktionen zwischen Datenbankservern ab.

### [Get-AzureRmSqlServerDisasterRecoveryConfiguration](Get-AzureRmSqlServerDisasterRecoveryConfiguration.md)
Ruft eine Datenbankserver-System Wiederherstellungskonfiguration ab.

### [Get-AzureRmSqlServerDisasterRecoveryConfigurationActivity](Get-AzureRmSqlServerDisasterRecoveryConfigurationActivity.md)
Ruft die Aktivität für eine Datenbankserver-System Wiederherstellungskonfiguration ab.

### [Get-AzureRmSqlServerDnsAlias](Get-AzureRmSqlServerDnsAlias.md)
Ruft den Azure SQL Server-DNS-Alias ab oder listet ihn auf.

### [Get-AzureRmSqlServerFirewallRule](Get-AzureRmSqlServerFirewallRule.md)
Ruft Firewallregeln für einen SQL-Datenbankserver ab.

### [Get-AzureRmSqlServerKeyVaultKey](Get-AzureRmSqlServerKeyVaultKey.md)
Ruft die schlüsseltresor Schlüssel von SQL Server ab.

### [Get-AzureRmSqlServerRecommendedAction](Get-AzureRmSqlServerRecommendedAction.md)
Ruft eine oder mehrere Empfohlene Aktionen für einen Azure SQL Server-Ratgeber ab.

### [Get-AzureRmSqlServerServiceObjective](Get-AzureRmSqlServerServiceObjective.md)
Ruft Dienst Ziele für einen Azure SQL-Datenbankserver ab.

### [Get-AzureRmSqlServerThreatDetectionPolicy](Get-AzureRmSqlServerThreatDetectionPolicy.md)
Ruft die Richtlinie für die Bedrohungserkennung für einen Server ab.

### [Get-AzureRmSqlServerTransparentDataEncryptionProtector](Get-AzureRmSqlServerTransparentDataEncryptionProtector.md)
Ruft die transparente Datenverschlüsselung (DSA)-Beschützer ab

### [Get-AzureRmSqlServerUpgrade](Get-AzureRmSqlServerUpgrade.md)
Ruft den Status eines Azure SQL-Datenbankserver-Upgrades ab.

### [Get-AzureRmSqlServerUpgradeHint](Get-AzureRmSqlServerUpgradeHint.md)
Ruft Informationen zu Preisstufen für das Upgrade eines Azure SQL-Datenbankservers ab.

### [Get-AzureRmSqlServerVirtualNetworkRule](Get-AzureRmSqlServerVirtualNetworkRule.md)
Ruft die virtuelle Azure SQL Server-Netzwerkregel ab oder listet Sie auf.

### [Get-AzureRmSqlSyncAgent](Get-AzureRmSqlSyncAgent.md)
Gibt Informationen zu Azure SQL-Synchronisierungs-Agents zurück.

### [Get-AzureRmSqlSyncAgentLinkedDatabase](Get-AzureRmSqlSyncAgentLinkedDatabase.md)
Gibt Informationen zu SQL Server-Datenbanken zurück, die von einem Synchronisierungs-Agent verknüpft sind.

### [Get-AzureRmSqlSyncGroup](Get-AzureRmSqlSyncGroup.md)
Gibt Informationen zu Azure SQL-Daten Bank Synchronisierungsgruppen zurück.

### [Get-AzureRmSqlSyncGroupLog](Get-AzureRmSqlSyncGroupLog.md)
Gibt die Protokolle einer Azure SQL-Daten Bank Synchronisierungsgruppe zurück.

### [Get-AzureRmSqlSyncMember](Get-AzureRmSqlSyncMember.md)
Gibt Informationen zu Azure SQL-Daten Bank Synchronisierungs Mitgliedern zurück.

### [Get-AzureRmSqlSyncSchema](Get-AzureRmSqlSyncSchema.md)
Gibt Informationen zum Synchronisierungs Schema einer Mitgliedsdatenbank oder einer Hub-Datenbank zurück.

### [Neu – AzureRmSqlDatabase](New-AzureRmSqlDatabase.md)
Erstellt eine Datenbank oder eine elastische Datenbank.

### [Neu – AzureRmSqlDatabaseCopy](New-AzureRmSqlDatabaseCopy.md)
Erstellt eine Kopie einer SQL-Datenbank, die den Snapshot zum aktuellen Zeitpunkt verwendet.

### [Neu – AzureRmSqlDatabaseDataMaskingRule](New-AzureRmSqlDatabaseDataMaskingRule.md)
Erstellt eine Daten Maskierungs Regel für eine Datenbank.

### [Neu – AzureRmSqlDatabaseExport](New-AzureRmSqlDatabaseExport.md)
Exportiert eine Azure SQL-Datenbank als BacPac-Datei in ein Speicherkonto.

### [Neu – AzureRmSqlDatabaseFailoverGroup](New-AzureRmSqlDatabaseFailoverGroup.md)
Mit diesem Befehl wird eine neue Azure SQL-Daten Bank Failover-Gruppe erstellt.

### [Neu – AzureRmSqlDatabaseImport](New-AzureRmSqlDatabaseImport.md)
Importiert eine BacPac-Datei und erstellt eine neue Datenbank auf dem Server.

### [Neu – AzureRmSqlDatabaseRestorePoint](New-AzureRmSqlDatabaseRestorePoint.md)
Erstellt einen neuen Wiederherstellungspunkt, von dem eine SQL-Datenbank wiederhergestellt werden kann.

### [Neu – AzureRmSqlDatabaseSecondary](New-AzureRmSqlDatabaseSecondary.md)
Erstellt eine sekundäre Datenbank für eine vorhandene Datenbank und startet die Datenreplikation.

### [Neu – AzureRmSqlElasticPool](New-AzureRmSqlElasticPool.md)
Erstellt einen elastischen datenbankpool für eine SQL-Datenbank.

### [Neu – AzureRmSqlInstance](New-AzureRmSqlInstance.md)
Erstellt eine Azure SQL-Daten Bank verwaltete Instanz.

### [Neu – AzureRmSqlInstanceDatabase](New-AzureRmSqlInstanceDatabase.md)
Erstellt eine Azure SQL-Datenbank mit verwalteten Instanzen.

### [Neu – AzureRmSqlServer](New-AzureRmSqlServer.md)
Erstellt einen SQL-Datenbankserver.

### [Neu – AzureRmSqlServerCommunicationLink](New-AzureRmSqlServerCommunicationLink.md)
Erstellt eine Kommunikationsverknüpfung für elastische Datenbanktransaktionen zwischen zwei SQL-Datenbankservern.

### [Neu – AzureRmSqlServerDisasterRecoveryConfiguration](New-AzureRmSqlServerDisasterRecoveryConfiguration.md)
Erstellt eine Datenbankserver-System Wiederherstellungskonfiguration.

### [Neu – AzureRmSqlServerDnsAlias](New-AzureRmSqlServerDnsAlias.md)
Dieser Befehl erstellt einen neuen Azure SQL Server-DNS-Alias.

### [Neu – AzureRmSqlServerFirewallRule](New-AzureRmSqlServerFirewallRule.md)
Erstellt eine Firewall-Regel für einen SQL-Datenbankserver.

### [Neu – AzureRmSqlServerVirtualNetworkRule](New-AzureRmSqlServerVirtualNetworkRule.md)
Erstellt eine Azure SQL Server-Regel für virtuelles Netzwerk. 

### [Neu – AzureRmSqlSyncAgent](New-AzureRmSqlSyncAgent.md)
Erstellt einen Azure SQL-Synchronisierungs-Agent.

### [Neu – AzureRmSqlSyncAgentKey](New-AzureRmSqlSyncAgentKey.md)
Erstellt einen Azure SQL-Synchronisierungs-Agent-Schlüssel.

### [Neu – AzureRmSqlSyncGroup](New-AzureRmSqlSyncGroup.md)
Erstellt eine Azure SQL-Daten Bank Synchronisierungsgruppe.

### [Neu – AzureRmSqlSyncMember](New-AzureRmSqlSyncMember.md)
Erstellt ein Azure SQL-Daten Bank Synchronisierungs Mitglied.

### [Remove-AzureRmSqlDatabase](Remove-AzureRmSqlDatabase.md)
Entfernt eine Azure SQL-Datenbank.

### [Remove-AzureRmSqlDatabaseAuditing](Remove-AzureRmSqlDatabaseAuditing.md)
Entfernt die Überwachung einer Datenbank.

### [Remove-AzureRmSqlDatabaseDataMaskingRule](Remove-AzureRmSqlDatabaseDataMaskingRule.md)
Entfernt eine Daten Maskierungs Regel aus einer Datenbank.

### [Remove-AzureRmSqlDatabaseFailoverGroup](Remove-AzureRmSqlDatabaseFailoverGroup.md)
Entfernt eine Azure SQL-Daten Bank Failover-Gruppe.

### [Remove-AzureRmSqlDatabaseFromFailoverGroup](Remove-AzureRmSqlDatabaseFromFailoverGroup.md)
Entfernt mindestens eine Datenbank aus einer Azure SQL-Daten Bank Failover-Gruppe.

### [Remove-AzureRmSqlDatabaseLongTermRetentionBackup](Remove-AzureRmSqlDatabaseLongTermRetentionBackup.md)
Löscht eine langfristige Aufbewahrungs Sicherung.

### [Remove-AzureRmSqlDatabaseRestorePoint](Remove-AzureRmSqlDatabaseRestorePoint.md)
Entfernt den angegebenen Wiederherstellungspunkt aus einer SQL-Datenbank.

### [Remove-AzureRmSqlDatabaseSecondary](Remove-AzureRmSqlDatabaseSecondary.md)
Beendet die Datenreplikation zwischen einer SQL-Datenbank und der angegebenen sekundären Datenbank.

### [Remove-AzureRmSqlDatabaseThreatDetectionPolicy](Remove-AzureRmSqlDatabaseThreatDetectionPolicy.md)
Entfernt die Bedrohungs Erkennungsrichtlinie aus einer Datenbank.

### [Remove-AzureRmSqlElasticPool](Remove-AzureRmSqlElasticPool.md)
Löscht einen elastischen datenbankpool.

### [Remove-AzureRmSqlInstance](Remove-AzureRmSqlInstance.md)
Entfernt eine Azure SQL-Datenbankinstanz.

### [Remove-AzureRmSqlInstanceDatabase](Remove-AzureRmSqlInstanceDatabase.md)
Entfernt eine Azure SQL-Datenbank mit verwalteten Instanzen.

### [Remove-AzureRmSqlServer](Remove-AzureRmSqlServer.md)
Entfernt einen Azure SQL-Datenbankserver.

### [Remove-AzureRmSqlServerActiveDirectoryAdministrator](Remove-AzureRmSqlServerActiveDirectoryAdministrator.md)
Entfernt einen Azure AD-Administrator für SQL Server.

### [Remove-AzureRmSqlServerAuditing](Remove-AzureRmSqlServerAuditing.md)
Entfernt die Überwachung eines SQL-Servers.

### [Remove-AzureRmSqlServerCommunicationLink](Remove-AzureRmSqlServerCommunicationLink.md)
Löscht eine Kommunikationsverknüpfung für elastische Datenbanktransaktionen zwischen zwei Servern.

### [Remove-AzureRmSqlServerDisasterRecoveryConfiguration](Remove-AzureRmSqlServerDisasterRecoveryConfiguration.md)
Entfernt eine SQL Database Server-System Wiederherstellungskonfiguration.

### [Remove-AzureRmSqlServerDnsAlias](Remove-AzureRmSqlServerDnsAlias.md)
Entfernt den Azure SQL Server-DNS-Alias.

### [Remove-AzureRmSqlServerFirewallRule](Remove-AzureRmSqlServerFirewallRule.md)
Löscht eine Firewall-Regel von einem SQL-Datenbankserver.

### [Remove-AzureRmSqlServerKeyVaultKey](Remove-AzureRmSqlServerKeyVaultKey.md)
Entfernt einen Key Vault-Schlüssel aus einem SQL Server.

### [Remove-AzureRmSqlServerThreatDetectionPolicy](Remove-AzureRmSqlServerThreatDetectionPolicy.md)
Entfernt die Bedrohungs Erkennungsrichtlinie von einem Server.

### [Remove-AzureRmSqlServerVirtualNetworkRule](Remove-AzureRmSqlServerVirtualNetworkRule.md)
Löscht eine Azure SQL Server-Regel für virtuelles Netzwerk.

### [Remove-AzureRmSqlSyncAgent](Remove-AzureRmSqlSyncAgent.md)
Entfernt einen Azure SQL-Synchronisierungs-Agent.

### [Remove-AzureRmSqlSyncGroup](Remove-AzureRmSqlSyncGroup.md)
Entfernt eine Azure SQL-Daten Bank Synchronisierungsgruppe.

### [Remove-AzureRmSqlSyncMember](Remove-AzureRmSqlSyncMember.md)
Entfernt ein Azure SQL-Daten Bank Synchronisierungs Mitglied.

### [Wiederherstellen – AzureRmSqlDatabase](Restore-AzureRmSqlDatabase.md)
Stellt eine SQL-Datenbank wieder her.

### [Wiederherstellen – AzureRmSqlInstanceDatabase](Restore-AzureRmSqlInstanceDatabase.md)
Stellt eine Azure SQL-Datenbank für verwaltete Instanzen wieder her.

### [Lebenslauf-AzureRmSqlDatabase](Resume-AzureRmSqlDatabase.md)
Setzt eine SQL Data Warehouse-Datenbank fort.

### [Satz-AzureRmSqlDatabase](Set-AzureRmSqlDatabase.md)
Legt Eigenschaften für eine Datenbank fest oder verschiebt eine vorhandene Datenbank in einen elastischen Pool.

### [Satz-AzureRmSqlDatabaseAdvisorAutoExecuteStatus](Set-AzureRmSqlDatabaseAdvisorAutoExecuteStatus.md)
Ändert den automatischen Ausführungsstatus eines Azure SQL-Daten Bank Beraters.

### [Satz-AzureRmSqlDatabaseAuditing](Set-AzureRmSqlDatabaseAuditing.md)
Ändert die Überwachungseinstellungen für eine Azure SQL-Datenbank.

### [Satz-AzureRmSqlDatabaseAuditingPolicy](Set-AzureRmSqlDatabaseAuditingPolicy.md)
Legt die Überwachungsrichtlinie für eine Datenbank fest.

### [Satz-AzureRmSqlDatabaseBackupLongTermRetentionPolicy](Set-AzureRmSqlDatabaseBackupLongTermRetentionPolicy.md)
Legt eine Server-langfristige Aufbewahrungsrichtlinie fest.

### [Satz-AzureRmSqlDatabaseDataMaskingPolicy](Set-AzureRmSqlDatabaseDataMaskingPolicy.md)
Legt die Daten Maskierung für eine Datenbank fest.

### [Satz-AzureRmSqlDatabaseDataMaskingRule](Set-AzureRmSqlDatabaseDataMaskingRule.md)
Legt die Eigenschaften einer Daten Maskierungs Regel für eine Datenbank fest.

### [Satz-AzureRmSqlDatabaseFailoverGroup](Set-AzureRmSqlDatabaseFailoverGroup.md)
Ändert die Konfiguration einer Azure SQL-Daten Bank Failover-Gruppe.

### [Satz-AzureRmSqlDatabaseGeoBackupPolicy](Set-AzureRmSqlDatabaseGeoBackupPolicy.md)
Legt eine Daten Bank Geo-Sicherungsrichtlinie fest.

### [Satz-AzureRmSqlDatabaseRecommendedActionState](Set-AzureRmSqlDatabaseRecommendedActionState.md)
Aktualisiert den Zustand einer Azure SQL-Datenbank Empfohlene Aktion.

### [Satz-AzureRmSqlDatabaseSecondary](Set-AzureRmSqlDatabaseSecondary.md)
Wechselt eine sekundäre Datenbank als primäre Datenbank, um ein Failover zu initiieren.

### [Satz-AzureRmSqlDatabaseThreatDetectionPolicy](Set-AzureRmSqlDatabaseThreatDetectionPolicy.md)
Legt eine Bedrohungs Erkennungsrichtlinie für eine Datenbank fest.

### [Satz-AzureRmSqlDatabaseTransparentDataEncryption](Set-AzureRmSqlDatabaseTransparentDataEncryption.md)
Ändert die DSA-Eigenschaft für eine Datenbank.

### [Satz-AzureRmSqlDatabaseVulnerabilityAssessmentRuleBaseline](Set-AzureRmSqlDatabaseVulnerabilityAssessmentRuleBaseline.md)
Legt die Richtlinie für die Anfälligkeitsbewertung fest.

### [Satz-AzureRmSqlElasticPool](Set-AzureRmSqlElasticPool.md)
Ändert die Eigenschaften eines elastischen Daten Bank Pools in Azure SQL-Datenbank.

### [Satz-AzureRmSqlElasticPoolAdvisorAutoExecuteStatus](Set-AzureRmSqlElasticPoolAdvisorAutoExecuteStatus.md)
Aktualisiert den automatischen Ausführungsstatus eines Azure SQL Elastic Pool Advisor.

### [Satz-AzureRmSqlElasticPoolRecommendedActionState](Set-AzureRmSqlElasticPoolRecommendedActionState.md)
Aktualisiert den Zustand eines Azure SQL-elastischen Pools Empfohlene Aktion.

### [Satz-AzureRmSqlInstance](Set-AzureRmSqlInstance.md)
Legt Eigenschaften für eine Azure SQL-Daten Bank verwaltete Instanz fest.

### [Satz-AzureRmSqlServer](Set-AzureRmSqlServer.md)
Ändert die Eigenschaften eines SQL-Datenbankservers.

### [Satz-AzureRmSqlServerActiveDirectoryAdministrator](Set-AzureRmSqlServerActiveDirectoryAdministrator.md)
Stellt einen Azure AD-Administrator für SQL Server vor.

### [Satz-AzureRmSqlServerAdvisorAutoExecuteStatus](Set-AzureRmSqlServerAdvisorAutoExecuteStatus.md)
Aktualisiert den automatischen Ausführungsstatus eines Azure SQL Server-Ratgebers.

### [Satz-AzureRmSqlServerAuditing](Set-AzureRmSqlServerAuditing.md)
Ändert die Überwachungseinstellungen eines Azure SQL Server.

### [Satz-AzureRmSqlServerAuditingPolicy](Set-AzureRmSqlServerAuditingPolicy.md)
Ändert die Überwachungsrichtlinie eines SQL-Datenbankservers.

### [Satz-AzureRmSqlServerBackupLongTermRetentionVault](Set-AzureRmSqlServerBackupLongTermRetentionVault.md)
Legt einen langfristigen Aufbewahrungs Tresor für Server fest.

### [Satz-AzureRmSqlServerDisasterRecoveryConfiguration](Set-AzureRmSqlServerDisasterRecoveryConfiguration.md)
Ändert eine Datenbankserver-Wiederherstellungskonfiguration.

### [Satz-AzureRmSqlServerDnsAlias](Set-AzureRmSqlServerDnsAlias.md)
Ändert den Server, auf den der Azure SQL Server-DNS-Alias zeigt

### [Satz-AzureRmSqlServerFirewallRule](Set-AzureRmSqlServerFirewallRule.md)
Ändert eine Firewallregel in Azure SQL-Datenbankserver.

### [Satz-AzureRmSqlServerRecommendedActionState](Set-AzureRmSqlServerRecommendedActionState.md)
Aktualisiert den Zustand der empfohlenen Azure SQL Server-Aktion.

### [Satz-AzureRmSqlServerThreatDetectionPolicy](Set-AzureRmSqlServerThreatDetectionPolicy.md)
Legt eine Bedrohungs Erkennungsrichtlinie auf einem Server fest.

### [Satz-AzureRmSqlServerTransparentDataEncryptionProtector](Set-AzureRmSqlServerTransparentDataEncryptionProtector.md)
Legt die transparente Daten Verschlüsselungs-Beschützer für einen SQL Server fest.

### [Satz-AzureRmSqlServerVirtualNetworkRule](Set-AzureRmSqlServerVirtualNetworkRule.md)
Ändert die Konfiguration einer virtuellen Azure SQL Server-Netzwerkregel.

### [Anfang-AzureRmSqlDatabaseExecuteIndexRecommendation](Start-AzureRmSqlDatabaseExecuteIndexRecommendation.md)
Startet den Workflow, der einen empfohlenen Indexvorgang ausführt.

### [Anfang-AzureRmSqlDatabaseVulnerabilityAssessmentScan](Start-AzureRmSqlDatabaseVulnerabilityAssessmentScan.md)
Startet eine Prüfung zur Gefährdungsbeurteilung.

### [Anfang-AzureRmSqlServerUpgrade](Start-AzureRmSqlServerUpgrade.md)
Startet das Upgrade eines SQL-Datenbankservers.

### [Anfang-AzureRmSqlSyncGroupSync](Start-AzureRmSqlSyncGroupSync.md)
Startet eine Synchronisierungsgruppen Synchronisierung.

### [Stopp-AzureRmSqlDatabaseActivity](Stop-AzureRmSqlDatabaseActivity.md)
Bricht den asynchronen Aktualisierungsvorgang für die Datenbank ab.

### [Stopp-AzureRmSqlDatabaseExecuteIndexRecommendation](Stop-AzureRmSqlDatabaseExecuteIndexRecommendation.md)
Beendet den Workflow, der einen empfohlenen Indexvorgang ausführt.

### [Stopp-AzureRmSqlElasticPoolActivity](Stop-AzureRmSqlElasticPoolActivity.md)
Bricht den asynchronen Aktualisierungsvorgang für einen elastischen Pool ab.

### [Stopp-AzureRmSqlServerUpgrade](Stop-AzureRmSqlServerUpgrade.md)
Beendet das Upgrade eines SQL-Datenbankservers.

### [Stopp-AzureRmSqlSyncGroupSync](Stop-AzureRmSqlSyncGroupSync.md)
Beendet die Synchronisierung einer Synchronisierungsgruppe.

### [Suspend-AzureRmSqlDatabase](Suspend-AzureRmSqlDatabase.md)
Unterbricht eine SQL Data Warehouse-Datenbank.

### [Switch-AzureRmSqlDatabaseFailoverGroup](Switch-AzureRmSqlDatabaseFailoverGroup.md)
Führt ein Failover einer Azure SQL-Datenbankfailover-Gruppe aus.

### [Update-AzureRmSqlDatabaseVulnerabilityAssessmentSettings](Update-AzureRmSqlDatabaseVulnerabilityAssessmentSettings.md)
Aktualisiert die Einstellungen für die Sicherheitsrisikobewertung einer Datenbank.

### [Update-AzureRmSqlSyncGroup](Update-AzureRmSqlSyncGroup.md)
Aktualisiert eine Azure SQL-Daten Bank Synchronisierungsgruppe.

### [Update-AzureRmSqlSyncMember](Update-AzureRmSqlSyncMember.md)
Aktualisiert ein Azure SQL-Daten Bank Synchronisierungs Mitglied.

### [Update-AzureRmSqlSyncSchema](Update-AzureRmSqlSyncSchema.md)
Aktualisieren Sie das Synchronisierungs Schema für eine Synchronisierungs Mitglieder-Datenbank oder eine Synchronisierungs-Hub-Datenbank.
Sie erhält das aktuelle Datenbankschema aus der realen Datenbank und verwendet Sie dann, um das von der Synchronisierungs-Metadaten-Datenbank zwischengespeicherte Schema zu aktualisieren.
Wenn "SyncMemberName" angegeben ist, wird das Mitgliedsdaten Bank Schema aktualisiert; Wenn dies nicht der Fall ist, wird das Hub-Datenbankschema aktualisiert.

### [Verwenden von-AzureRmSqlServerAuditingPolicy](Use-AzureRmSqlServerAuditingPolicy.md)
Gibt an, dass eine Datenbank die Überwachungsrichtlinie Ihres Hostservers verwendet.

