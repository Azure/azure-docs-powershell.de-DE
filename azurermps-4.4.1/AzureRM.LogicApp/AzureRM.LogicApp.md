---
Module Name: AzureRM.LogicApp
Module Guid: XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX
Download Help Link:
  object Object: 
Help Version:
  object Object: 
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/preview/src/ResourceManager/LogicApp/Commands.LogicApp/help/AzureRM.LogicApp.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/preview/src/ResourceManager/LogicApp/Commands.LogicApp/help/AzureRM.LogicApp.md
ms.openlocfilehash: b54e29093a8ec6476d7979f10909c3d20d165346
ms.sourcegitcommit: 43f4bdf2a59dd82fd881512aa9761bf72eb5703c
ms.translationtype: MT
ms.contentlocale: de-DE
ms.lasthandoff: 04/23/2019
ms.locfileid: "93474205"
---
# AzureRM. LogicApp-Modul
## Beschreibung
In diesem Thema werden Hilfethemen zu den Logik-apps-Cmdlets angezeigt.

## AzureRM. LogicApp-Cmdlets
### [Get-AzureRmIntegrationAccount](Get-AzureRmIntegrationAccount.md)
Ruft Integrations Konten ab.

### [Get-AzureRmIntegrationAccountAgreement](Get-AzureRmIntegrationAccountAgreement.md)
Ruft eine Integrations Kontovereinbarung ab.

### [Get-AzureRmIntegrationAccountCallbackUrl](Get-AzureRmIntegrationAccountCallbackUrl.md)
Ruft eine Integrations Konto-Rückruf-URL ab.

### [Get-AzureRmIntegrationAccountCertificate](Get-AzureRmIntegrationAccountCertificate.md)
Ruft Integrations Kontozertifikate aus einer Ressourcengruppe ab.

### [Get-AzureRmIntegrationAccountGeneratedIcn](Get-AzureRmIntegrationAccountGeneratedIcn.md)
Ruft die für das Integrations Konto generierte X12 Interchange Control Number pro Vereinbarung ab.

### [Get-AzureRmIntegrationAccountMap](Get-AzureRmIntegrationAccountMap.md)
Ruft eine Integration-Kontozuordnung ab.

### [Get-AzureRmIntegrationAccountPartner](Get-AzureRmIntegrationAccountPartner.md)
Ruft Integrations Kontopartner ab.

### [Get-AzureRmIntegrationAccountReceivedIcn](Get-AzureRmIntegrationAccountReceivedIcn.md)
Ruft das für das Integration-Konto erhaltene X12 Interchange Control Number pro Vertrag ab.

### [Get-AzureRmIntegrationAccountSchema](Get-AzureRmIntegrationAccountSchema.md)
Ruft Schemas für das Integrations Konto ab.

### [Get-AzureRmLogicApp](Get-AzureRmLogicApp.md)
Ruft eine Logik-App aus einer Ressourcengruppe ab.

### [Get-AzureRmLogicAppRunAction](Get-AzureRmLogicAppRunAction.md)
Ruft eine Aktion aus einer Logik-App-Ausführung ab.

### [Get-AzureRmLogicAppRunHistory](Get-AzureRmLogicAppRunHistory.md)
Ruft den Ausführungsverlauf einer Logik-App ab.

### [Get-AzureRmLogicAppTrigger](Get-AzureRmLogicAppTrigger.md)
Ruft die Trigger einer Logik-App ab.

### [Get-AzureRmLogicAppTriggerCallbackUrl](Get-AzureRmLogicAppTriggerCallbackUrl.md)
Ruft eine Logik-App-Trigger-Rückruf-URL ab.

### [Get-AzureRmLogicAppTriggerHistory](Get-AzureRmLogicAppTriggerHistory.md)
Ruft den Verlauf von Triggern in einer Logik-App ab.

### [Get-AzureRmLogicAppUpgradedDefinition](Get-AzureRmLogicAppUpgradedDefinition.md)
Ruft die aktualisierte Definition für eine Logik-App ab.

### [Neu – AzureRmIntegrationAccount](New-AzureRmIntegrationAccount.md)
Erstellt ein Integrations Konto.

### [Neu – AzureRmIntegrationAccountAgreement](New-AzureRmIntegrationAccountAgreement.md)
Erstellt eine Integrations Kontovereinbarung.

### [Neu – AzureRmIntegrationAccountCertificate](New-AzureRmIntegrationAccountCertificate.md)
Erstellt ein Integrations Kontozertifikat.

### [Neu – AzureRmIntegrationAccountMap](New-AzureRmIntegrationAccountMap.md)
Erstellt eine Integration-Kontozuordnung.

### [Neu – AzureRmIntegrationAccountPartner](New-AzureRmIntegrationAccountPartner.md)
Erstellt einen Integrations Kontopartner.

### [Neu – AzureRmIntegrationAccountSchema](New-AzureRmIntegrationAccountSchema.md)
Erstellt ein Schema für ein Integrations Konto.

### [Neu – AzureRmLogicApp](New-AzureRmLogicApp.md)
Erstellt eine Logik-app in einer Ressourcengruppe.

### [Remove-AzureRmIntegrationAccount](Remove-AzureRmIntegrationAccount.md)
Entfernt ein Integrations Konto.

### [Remove-AzureRmIntegrationAccountAgreement](Remove-AzureRmIntegrationAccountAgreement.md)
Entfernt eine Integration-Kontovereinbarung.

### [Remove-AzureRmIntegrationAccountCertificate](Remove-AzureRmIntegrationAccountCertificate.md)
Entfernt ein Integrations Kontozertifikat aus einer Ressourcengruppe.

### [Remove-AzureRmIntegrationAccountMap](Remove-AzureRmIntegrationAccountMap.md)
Entfernt eine Integration-Kontozuordnung.

### [Remove-AzureRmIntegrationAccountPartner](Remove-AzureRmIntegrationAccountPartner.md)
Entfernt einen Integrations Kontopartner.

### [Remove-AzureRmIntegrationAccountReceivedIcn](Remove-AzureRmIntegrationAccountReceivedIcn.md)
Entfernt ein Integration-Konto, das die X12 Interchange-Kontrollnummer erhalten hat.

### [Remove-AzureRmIntegrationAccountSchema](Remove-AzureRmIntegrationAccountSchema.md)
Entfernt ein Schema des Integrations Kontos.

### [Remove-AzureRmLogicApp](Remove-AzureRmLogicApp.md)
Entfernt eine Logik-App aus einer Ressourcengruppe.

### [Satz-AzureRmIntegrationAccount](Set-AzureRmIntegrationAccount.md)
Ändert ein Integrations Konto.

### [Satz-AzureRmIntegrationAccountAgreement](Set-AzureRmIntegrationAccountAgreement.md)
Ändert eine Integration-Kontovereinbarung.

### [Satz-AzureRmIntegrationAccountCertificate](Set-AzureRmIntegrationAccountCertificate.md)
Ändert ein Integrations Kontozertifikat.

### [Satz-AzureRmIntegrationAccountGeneratedIcn](Set-AzureRmIntegrationAccountGeneratedIcn.md)
Ändert eine vom Integration Account generierte X12 Interchange-Steuerelementnummer.

### [Satz-AzureRmIntegrationAccountMap](Set-AzureRmIntegrationAccountMap.md)
Ändert eine Integration-Kontozuordnung.

### [Satz-AzureRmIntegrationAccountPartner](Set-AzureRmIntegrationAccountPartner.md)
Ändert einen Integrations Kontopartner.

### [Satz-AzureRmIntegrationAccountReceivedIcn](Set-AzureRmIntegrationAccountReceivedIcn.md)
Ändert ein Integration-Konto, das die X12 Interchange-Kontrollnummer erhalten hat.

### [Satz-AzureRmIntegrationAccountSchema](Set-AzureRmIntegrationAccountSchema.md)
Ändert ein Schema des Integrations Kontos.

### [Satz-AzureRmLogicApp](Set-AzureRmLogicApp.md)
Ändert eine Logik-app in einer Ressourcengruppe.

### [Anfang-AzureRmLogicApp](Start-AzureRmLogicApp.md)
Führt eine Logik-app in einer Ressourcengruppe aus.

### [Stopp-AzureRmLogicAppRun](Stop-AzureRmLogicAppRun.md)
Bricht die Ausführung einer Logik-App ab.

### [Test-AzureRmLogicApp](Test-AzureRmLogicApp.md)
Validiert eine Logik-App-Definition.

