...
## Beschreibung
Mit dem Cmdlet " **Export-AzureRmRedisCache** " werden Daten aus Azure-a/a-Cache in einen Container exportiert.
...
Dieser Befehl exportiert Daten aus einer Azure-Member-Cache-Instanz in den Container, der von der SAS-URL angegeben wird.

## Parameter

### -Container
Gibt die Dienst-SAS-URL des Containers an, in dem dieses Cmdlet Daten exportiert. Mit den folgenden PowerShell-Befehlen können Sie eine Dienst-SAS-URL generieren:

$storageAccountContext = New-AzureStorageContext-StorageAccountName "Speichername"-StorageAccountKey "Schlüssel"

$sasKeyForContainer = New-AzureStorageContainerSASToken-Name "Containername"-Berechtigung "rwdl"-Startzeit ([System. DateTime]:: Now). AddMinutes (-15)-ExpiryTime ([System. DateTime]:: Now). AddHours (5)-Kontext $storageAccountContext-FullUri 

Export-AzureRmRedisCache-ResourceGroupName "ResourceGroupName"-Name "cachename"-Präfix "blobprefix"-Container ($sasKeyForContainer)
...
### -Format
Gibt ein Format für das BLOB an.
...
### -PassThru
Gibt an, dass dieses Cmdlet einen booleschen Wert zurückgibt, der angibt, ob der Vorgang erfolgreich ausgeführt wurde.
...
### -Prefix
Gibt ein Präfix an, das für BLOB-Namen verwendet werden soll.
