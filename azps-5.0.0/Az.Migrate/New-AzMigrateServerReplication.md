...
Dies gilt für das Szenario, wenn nur ein einziger Datenträger vorhanden ist, der geschützt werden muss.
...
Dies gilt für das Szenario, wenn mehrere Datenträger vorhanden sind, die geschützt werden müssen.
...
### -DiskEncryptionSetID
Gibt den festgelegten Festplatten-Verschlüsselungs an.
...
Informationen zum Erstellen finden Sie unter Abschnitt "Notizen" für DISKTOINCLUDE-Eigenschaften und Erstellen einer Hashtabelle.
...
### -Disktype
Gibt den Typ der für den Azure VM zu verwendenden Datenträger an.
...
### -Inputobject
Gibt den zu migrierenden gefundenen Server an.
Das Serverobjekt kann mit dem Get-AzMigrateServer-Cmdlet abgerufen werden.
...
### -LicenseType
Gibt an, ob Azure Hybrid Benefit für den zu migrierenden Quellserver gilt.
...
### -Maschinen-Nr
Gibt die Computer-ID des zu migrierenden gefundenen Servers an.
...
### -OSDiskID
Gibt den Betriebs System Datenträger für den zu migrierenden Quellserver an.
...
### -PerformAutoResync
Gibt an, ob die Replikation automatisch repariert werden soll, falls die Änderungsnachverfolgung für den Quellserver unter Replikation verloren geht.
...
Gibt den verfügbaren Verfügbarkeits Satz an, der für die VM-creationSpecifies des für die VM-Erstellung zu verwendenden Verfügbarkeits Satzes verwendet wird.
...
Gibt die Verfügbarkeits Zone an, die für die VM-Erstellung verwendet werden soll.
...
Gibt das für die Start Diagnose zu verwendende Speicherkonto an.
...
### -TargetNetworkId
Gibt die virtuelle Netzwerk-ID innerhalb des Ziels Azure-Abonnements an, für das der Server migriert werden muss.
...
Gibt die ID der Ressourcengruppe innerhalb des Ziels Azure-Abonnements an, für das der Server migriert werden muss.
...
### -TargetSubnetName
Gibt den Namen des Subnetzes innerhalb des virtuellen Ziel Netowk an, zu dem der Server migriert werden muss.
...
### -TargetVMSize
Gibt die SKU der Azure-VM an, die erstellt werden soll.
...
DISKTOINCLUDE <IVMwareCbtDiskInput [] >: gibt die Datenträger auf dem Quellserver an, die für die Replikation einbezogen werden sollen.
  ...
  - `LogStorageAccountId <String>`: Die ID des Protokollspeicher Kontos.
  - `LogStorageAccountSasSecretName <String>`: Der schlüsseltresor-geheim Name des Protokollspeicher Kontos.
  - `[DiskEncryptionSetId <String>]`: Die DiskEncryptionSet-Arm-ID.
  - `[DiskType <DiskAccountType?>]`: Der Datenträgertyp.

Inputobject <IVMwareMachine> : gibt den zu migrierenden gefundenen Server an. Das Serverobjekt kann mit dem Get-AzMigrateServer-Cmdlet abgerufen werden.
