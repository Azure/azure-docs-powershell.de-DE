...
### Neues a AKS mit Standard params.

```powershell
PS C:\> New-AzAks -ResourceGroupName myResourceGroup -Name myCluster
```

### Erstellen eines Windows Server-Containers auf einem AKS
Zum Erstellen eines Windows Server-Containers für eine AKS müssen Sie beim Erstellen der AKS mindestens vier folgende Parameter angeben und den Wert für `NetworkPlugin` und `NodeVmSetType` muss und müssen sein `azure` `VirtualMachineScaleSets` .
...
### -AcrNameToAttach
Erteilen der Rolle "acrpull" des angegebenen ACR an AKS-Dienstprinzipal, beispielsweise myacr
...
Add-on-Namen müssen aktiviert werden, wenn Cluster erstellt wird.
...
### -EnableRbac
Ob Kubernetes Role-Based Access aktiviert werden soll
...
### -GenerateSshKey
Generieren Sie eine SSH-Schlüsseldatei in {Home}/.ssh/id_rsa.
...
### -LoadBalancerSku
Die SKU des Lastenausgleichs für den verwalteten Cluster.
...
### -NetworkPlugin
Netzwerk-Plugin zum Erstellen von Kubernetes-Netzwerken.
...
### -NodeMaxPodCount
Die maximale Anzahl von Hülsen, die auf dem Knoten ausgeführt werden können.
...
### -Nodename
Eindeutiger Name des Agenten Pool Profils im Kontext der Abonnement-und Ressourcengruppe.
...
### -NodePoolMode
NodePoolMode stellt den Modus eines Knoten Pools dar.
...
ScaleSetEvictionPolicy, das zum Angeben von Räumungs Richtlinien für den Skalierungs Satz der virtuellen Maschine mit niedriger Priorität verwendet werden soll. Standard zum Löschen.
...
### -NodeSetPriority
ScaleSetPriority, das zum Angeben der Skalierungs Priorität für den virtuellen Computer verwendet werden soll. Standardmäßig auf normal.
...
### -NodeVmSetType
AgentPoolType stellt Typen eines Agenten Pools dar. Mögliche Werte sind: ' VirtualMachineScaleSets ', ' availabilityset '
...
### -NodeVnetSubnetID
VNet Subnet-ID gibt den Subnetzmasken des VNet an.
...
### -SubnetName
Subnet-Name des VirtualNode-addons.
...
Der für Windows-VMS zu verwendende Administrator-Benutzername.
...
Das für Windows-VMS zu verwendende Administratorkennwort, dessen Länge mindestens 12 sein muss, mit mindestens einem Kleinbuchstaben, also `[a-z]` einem `[A-Z]` und einem Sonderzeichen `[!@#$%^&_()]` .
...
### -WorkspaceResourceId
Ressourcen-ID des Arbeitsbereichs des Überwachungs-addons.
