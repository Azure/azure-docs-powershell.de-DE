...
## Beschreibung
Das Cmdlet " **Satz-AzVirtualNetworkGateway** " aktualisiert ein virtuelles Netzwerkgateway.
...
Der erste Befehl ruft ein virtuelles Netzwerkgateway mit dem Namen Gateway01 ab, das zu Ressourcengruppen-ResourceGroup001 gehört, und speichert es in der Variablen mit dem Namen $Gateway der zweite Befehl das virtuelle Netzwerkgateway aktualisiert, das in Variablen $Gateway gespeichert ist.
...
Der dritte Befehl aktualisiert das virtuelle Netzwerkgateway, das in Variablen $Gateway gespeichert ist.
...
Der erste Befehl ruft ein virtuelles Netzwerkgateway mit dem Namen Gateway01 ab, das zu Ressourcengruppen-ResourceGroup001 gehört, und speichert es in der Variablen mit dem Namen $Gateway der zweite Befehl den virtuellen Netzwerkgateway-Gateway01 mit den Tags @ {testtagKey = "SomeTagKey"; testtagValue = "SomeKeyValue"} aktualisiert.
...
Der erste Befehl ruft ein virtuelles Netzwerkgateway mit dem Namen Gateway01 ab, das zu Ressourcengruppen-ResourceGroup001 gehört, und speichert es in der Variablen mit dem Namen $Gateway der zweite Befehl den virtuellen Netzwerkgateway-Gateway01 mit den Aad-Authentifizierungs Konfigurationen params: aadTenantUri, aadAudienceId, aadIssuerUri für VpnClient.
Der dritte Befehl entfernt die Aad-Authentifizierungskonfiguration aus VpnClient des virtuellen Netzwerkgateways.
...
Der erste Befehl ruft ein virtuelles Netzwerkgateway mit dem Namen Gateway01 ab, das zu Ressourcengruppen-ResourceGroup001 gehört, und speichert es in der Variablen mit dem Namen $Gateway der zweite Befehl den Wert des virtuellen Netzwerkgateways Gateway01 IPKonfigurationsdateiAddress-ID in Variable ipconfigurationId1 zuweist.
Der dritte Befehl weist die Adressliste zu addresslist1.
Der vierte Befehl hat ein PSIpConfigurationBgpPeeringAddress-Objekt erstellt.
Der fünfte Befehl hat dieses neu erstellte PSIpConfigurationBgpPeeringAddress auf IpConfigurationBgpPeeringAddresses und das Gateway aktualisieren gesetzt.
...
### -ASN
ASN des virtuellen Netzwerkgateways, das zum Einrichten von BGP-Sitzungen in IPSec-Tunnels verwendet wird
...
Flag zum Deaktivieren des aktiven aktiven Features auf dem virtuellen Netzwerkgateway
...
### -GatewayDefaultSite
Die Standardwebsite, die für das Erzwingen des Tunnelns verwendet werden soll.
Wenn eine Standardwebsite angegeben wird, wird der gesamte Internetdatenverkehr vom vnet des Gateways an diese Website weitergeleitet.
...
### -GatewaySku
Die SKU des virtuellen Netzwerkgateways
...
### -RadiusServerList
P2S mehrerer externer RADIUS-Server.
...
Flag, um die Aad-Authentifizierung für den P2S-Client vom virtuellen Netzwerkgateway zu entfernen.
...
Das virtuelle Netzwerk-Gatewayobjekt, auf dem Änderungen basieren sollen.
Dies kann über Get-AzVirtualNetworkGateway abgerufen werden.
...
### -VpnClientAddressPool
Der Adressraum, von dem VPN-Client-IP-Adressen zugewiesen werden sollen.
...
### -VpnClientRevokedCertificates
Eine Liste der gesperrten VPN-Clientzertifikate.
Ein VPN-Client mit einem Zertifikat, das einem dieser Zertifikate entspricht, wird dazu aufgefordert, zu verschwinden.
...
### -VpnClientRootCertificates
Eine Liste der VPN-Client Stammzertifikate, die für die VPN-Clientauthentifizierung verwendet werden sollen.
