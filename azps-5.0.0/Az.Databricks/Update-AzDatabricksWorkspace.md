...
### Beispiel 1: aktualisiert die Tags eines databricks-Arbeitsbereichs
```powershell
...
Mit diesem Befehl werden die Tags eines databricks-Arbeitsbereichs aktualisiert.

### Beispiel 2: Aktivieren der Verschlüsselung für einen databricks-Arbeitsbereich
```powershell
...
Die Aktivierung der Verschlüsselung für einen databricks-Arbeitsbereich erfolgt in drei Schritten:
1.
Aktualisieren Sie den Arbeitsbereich mit `-PrepareEncryption` (sofern er nicht erstellt wurde).
1.
Suchen Sie `StorageAccountIdentityPrincipalId` in der Ausgabe des letzten Schritts.
Erteilen Sie dem Prinzipal Schlüssel Berechtigungen.
1.
Aktualisieren Sie den Arbeitsbereich erneut, um Informationen zum Verschlüsselungsschlüssel einzugeben:
    - `-EncryptionKeySource`
    - `-EncryptionKeyVaultUri`
    - `-EncryptionKeyName`
    - `-EncryptionKeyVersion`

### Beispiel 3: Deaktivieren der Verschlüsselung für einen databricks-Arbeitsbereich
```powershell
...
Um die Verschlüsselung zu deaktivieren, setzen `-EncryptionKeySource` Sie einfach auf `'Default'` .
...
### -EncryptionKeyName
Der Name des Key Vault-Schlüssels.
...
### -EncryptionKeySource
Die Verschlüsselungs-keySource (Anbieter).
Mögliche Werte (Groß-/Kleinschreibung wird nicht berücksichtigt): Standard, Microsoft. keyvault
...
### -EncryptionKeyVaultUri
Der URI (DNS-Name) des Schlüsselspeichers.
...
Aktiviert die verwaltete Identität für das verwaltete Speicherkonto.
