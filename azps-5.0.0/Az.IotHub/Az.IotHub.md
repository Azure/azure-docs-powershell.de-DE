---
Module Name: Az.IotHub
Module Guid: 51a3a993-fe39-4f8f-a437-2cb751584af5
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.iothub
Help Version: 3.1.2.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/IotHub/IotHub/help/Az.IotHub.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/IotHub/IotHub/help/Az.IotHub.md
ms.openlocfilehash: ba7e171dfa925460d9b8aa7eadcd018cc388c219
ms.sourcegitcommit: b4a38bcb0501a9016a4998efd377aa75d3ef9ce8
ms.translationtype: MT
ms.contentlocale: de-DE
ms.lasthandoff: 10/27/2020
ms.locfileid: "94177978"
---
# AZ. IotHub-Modul
## Beschreibung
Befehle zum Verwalten von Azure IotHubs

## AZ. IotHub-Cmdlets
### [Add-AzIotHubCertificate](Add-AzIotHubCertificate.md)
Erstellen/Aktualisieren eines Azure-viel-Hub-Zertifikats

### [Add-AzIotHubConfiguration](Add-AzIotHubConfiguration.md)
Fügen Sie eine sehr viele automatische Geräte Verwaltungskonfiguration in einem Ziel-viel-Hub hinzu.

### [Add-AzIotHubDeployment](Add-AzIotHubDeployment.md)
Fügen Sie eine viel Edge-Bereitstellung in einem Ziel-viel-Hub hinzu.

### [Add-AzIotHubDevice](Add-AzIotHubDevice.md)
Erstellen Sie ein Gerät in einem viel Hub.

### [Add-AzIotHubDeviceChildren](Add-AzIotHubDeviceChildren.md)
Hinzufügen von nicht-Edge-Geräten als untergeordnete Geräte zum Edge-Gerät.

### [Add-AzIotHubEventHubConsumerGroup](Add-AzIotHubEventHubConsumerGroup.md)
Erstellt eine eventhub-Verbrauchergruppe.

### [Add-AzIotHubKey](Add-AzIotHubKey.md)
Erstellt einen IotHub-Schlüssel.

### [Add-AzIotHubMessageEnrichment](Add-AzIotHubMessageEnrichment.md)
Erstellt eine Nachrichten Anreicherung für ausgewählte Endpunkte in Ihrem viel-Hub.

### [Add-AzIotHubModule](Add-AzIotHubModule.md)
Erstellen Sie ein Modul auf einem Ziel-viel-Gerät in einem viel Hub.

### [Add-AzIotHubRoute](Add-AzIotHubRoute.md)
Erstellen einer Route im viel Hub

### [Add-AzIotHubRoutingEndpoint](Add-AzIotHubRoutingEndpoint.md)
Hinzufügen eines Endpunkts zu Ihrem viel-Hub

### [Get-AzIotHub](Get-AzIotHub.md)
Ruft Informationen zu den IotHubs in einem Abonnement ab.

### [Get-AzIotHubCertificate](Get-AzIotHubCertificate.md)
Listet alle Zertifikate oder ein bestimmtes Zertifikat auf, das in einem Azure-viel-Hub enthalten ist. 

### [Get-AzIotHubCertificateVerificationCode](Get-AzIotHubCertificateVerificationCode.md)
Generiert einen Bestätigungscode für ein Azure-viel-Hub-Zertifikat. 

### [Get-AzIotHubConfiguration](Get-AzIotHubConfiguration.md)
Listet alle oder eine bestimmte viel automatische Geräte Verwaltungskonfiguration auf.

### [Get-AzIotHubConnectionString](Get-AzIotHubConnectionString.md)
Ruft die IotHub-connectionStrings ab.

### [Get-AzIotHubDeployment](Get-AzIotHubDeployment.md)
Listet alle oder eine bestimmte viel Edge-Bereitstellung auf.

### [Get-AzIotHubDevice](Get-AzIotHubDevice.md)
Listet alle Geräte oder ein bestimmtes Gerät auf, die in einem Azure-viel-Hub enthalten sind. 

### [Get-AzIotHubDeviceChildren](Get-AzIotHubDeviceChildren.md)
Drucken Sie eine durch trennzeichengetrennte Liste der zugewiesenen untergeordneten Geräte.

### [Get-AzIotHubDeviceConnectionString](Get-AzIotHubDeviceConnectionString.md)
Rufen Sie die Verbindungszeichenfolge eines Ziel-viel-Geräts in einem viel Hub ab.

### [Get-AzIotHubDeviceParent](Get-AzIotHubDeviceParent.md)
Rufen Sie das übergeordnete Gerät des angegebenen Geräts ab.

### [Get-AzIotHubDeviceTwin](Get-AzIotHubDeviceTwin.md)
Ruft ein Gerät Twin ab.

### [Get-AzIotHubDistributedTracing](Get-AzIotHubDistributedTracing.md)
Abrufen der Einstellungen für die verteilte Ablaufverfolgung für ein Gerät.

### [Get-AzIotHubEventHubConsumerGroup](Get-AzIotHubEventHubConsumerGroup.md)
Ruft alle eventhub-consumergroups ab.

### [Get-AzIotHubJob](Get-AzIotHubJob.md)
Ruft die Informationen zu einem IotHub-Auftrag ab.

### [Get-AzIotHubKey](Get-AzIotHubKey.md)
Ruft einen IotHub-Schlüssel ab.

### [Get-AzIotHubMessageEnrichment](Get-AzIotHubMessageEnrichment.md)
Listet alle Nachrichten-Anreicherungen oder eine bestimmte Nachrichten Anreicherung für Ihre viel-Nabe auf.

### [Get-AzIotHubModule](Get-AzIotHubModule.md)
Rufen Sie die Details eines viel Geräte Moduls oder Listen Moduls ab, das sich auf einem viel Gerät in einem viel Hub befindet.

### [Get-AzIotHubModuleConnectionString](Get-AzIotHubModuleConnectionString.md)
Besorgen Sie sich die Verbindungszeichenfolge eines Device-Moduls für Zielgeräte in einem viel Hub.

### [Get-AzIotHubModuleTwin](Get-AzIotHubModuleTwin.md)
Ruft ein viel Gerätemodul Twin ab.

### [Get-AzIotHubQuotaMetric](Get-AzIotHubQuotaMetric.md)
Ruft die Kontingent Metrik für eine IotHub ab.

### [Get-AzIotHubRegistryStatistic](Get-AzIotHubRegistryStatistic.md)
Ruft die RegistryStatistics für eine IotHub.

### [Get-AzIotHubRoute](Get-AzIotHubRoute.md)
Abrufen von Informationen über die Route in "viel Hub"

### [Get-AzIotHubRoutingEndpoint](Get-AzIotHubRoutingEndpoint.md)
Abrufen von Informationen zu allen Endpunkten für Ihren viel Hub

### [Get-AzIotHubValidSku](Get-AzIotHubValidSku.md)
Ruft alle gültigen SKUs ab, zu denen dieser IotHub wechseln kann.

### [Invoke-AzIotHubConfigurationMetricsQuery](Invoke-AzIotHubConfigurationMetricsQuery.md)
Rufen Sie eine Abfrage mit einer sehr viel Device Configuration-Metrik auf.

### [Invoke-AzIotHubDeploymentMetricsQuery](Invoke-AzIotHubDeploymentMetricsQuery.md)
Rufen Sie eine sehr viel Edge Deployment Metric-Abfrage auf.

### [Invoke-AzIotHubDeviceMethod](Invoke-AzIotHubDeviceMethod.md)
Rufen Sie eine direkte Methode auf einem Gerät auf.

### [Invoke-AzIotHubManualFailover](Invoke-AzIotHubManualFailover.md)
Rufen Sie den Failoverprozess für den viel-Hub für den Geo-gekoppelten Disaster Recovery-Bereich auf.

### [Invoke-AzIotHubModuleMethod](Invoke-AzIotHubModuleMethod.md)
Rufen Sie eine Edge-Modul-Methode auf.

### [Invoke-AzIotHubQuery](Invoke-AzIotHubQuery.md)
Abfragen eines vielseitigen Hubs mithilfe einer leistungsfähigen SQL-ähnlichen Sprache.

### [Neu – AzIotHub](New-AzIotHub.md)
Erstellt eine neue IotHub.

### [Neu – AzIotHubExportDevice](New-AzIotHubExportDevice.md)
Erstellt einen neuen Auftrag für Export Geräte.

### [Neu – AzIotHubImportDevice](New-AzIotHubImportDevice.md)
Erstellt einen neuen Auftrag für Import Geräte.

### [Neu – AzIotHubKey](New-AzIotHubKey.md)
Generieren Sie einen Azure-viel-Hub-Schlüssel.

### [Neu – AzIotHubSasToken](New-AzIotHubSasToken.md)
Generieren eines SAS-Tokens für einen Ziel-viel-Hub,-Gerät oder-Modul

### [Remove-AzIotHub](Remove-AzIotHub.md)
Löscht eine IotHub.

### [Remove-AzIotHubCertificate](Remove-AzIotHubCertificate.md)
Löscht ein Azure-viel-Hub-Zertifikat.

### [Remove-AzIotHubConfiguration](Remove-AzIotHubConfiguration.md)
Löschen Sie eine viel Gerätekonfiguration.

### [Remove-AzIotHubDeployment](Remove-AzIotHubDeployment.md)
Löschen Sie eine viel Edge-Bereitstellung.

### [Remove-AzIotHubDevice](Remove-AzIotHubDevice.md)
Löschen Sie ein viel Hub-Gerät.

### [Remove-AzIotHubDeviceChildren](Remove-AzIotHubDeviceChildren.md)
Entfernen Sie nicht-Edge-Geräte als untergeordnete Elemente von einem bestimmten Edge-Gerät.

### [Remove-AzIotHubEventHubConsumerGroup](Remove-AzIotHubEventHubConsumerGroup.md)
Löscht eine eventhub-verbrauchergroup.

### [Remove-AzIotHubKey](Remove-AzIotHubKey.md)
Entfernt einen IotHub-Schlüssel.

### [Remove-AzIotHubMessageEnrichment](Remove-AzIotHubMessageEnrichment.md)
Löschen Sie eine Nachrichten-Bereicherung in Ihrem viel-Hub.

### [Remove-AzIotHubModule](Remove-AzIotHubModule.md)
Löschen von Modulen auf einem Ziel-viel-Gerät in einem viel Hub

### [Remove-AzIotHubRoute](Remove-AzIotHubRoute.md)
Löschen einer Route in "viel Hub"

### [Remove-AzIotHubRoutingEndpoint](Remove-AzIotHubRoutingEndpoint.md)
Löschen eines Endpunkts für Ihren viel Hub

### [Senden-AzIotHubDevice2CloudMessage](Send-AzIotHubDevice2CloudMessage.md)
Nachricht "Gerät zu Wolke senden".

### [Satz-AzIotHub](Set-AzIotHub.md)
Aktualisiert die Eigenschaften eines IotHub.

### [Satz-AzIotHubConfiguration](Set-AzIotHubConfiguration.md)
Aktualisieren Sie die änderbaren Felder der Konfigurationsregistrierung.

### [Satz-AzIotHubDeployment](Set-AzIotHubDeployment.md)
Aktualisieren Sie die änderbaren Felder der Bereitstellung viel Edge.

### [Satz-AzIotHubDevice](Set-AzIotHubDevice.md)
Aktualisieren Sie ein viel Hub-Gerät.

### [Satz-AzIotHubDeviceParent](Set-AzIotHubDeviceParent.md)
Das übergeordnete Gerät des angegebenen Geräts festzulegen.

### [Satz-AzIotHubDistributedTracing](Set-AzIotHubDistributedTracing.md)
Aktualisieren Sie die Optionen für die verteilte Ablaufverfolgung für ein Gerät.

### [Satz-AzIotHubEdgeModule](Set-AzIotHubEdgeModule.md)
Setzen Sie Edge-Module auf einem einzelnen Edge-Gerät.

### [Satz-AzIotHubMessageEnrichment](Set-AzIotHubMessageEnrichment.md)
Aktualisieren Sie eine Nachrichten Anreicherung in Ihrem viel Hub.

### [Satz-AzIotHubModule](Set-AzIotHubModule.md)
Aktualisieren Sie ein Modul auf einem Ziel-viel-Gerät in einem viel Hub.

### [Satz-AzIotHubRoute](Set-AzIotHubRoute.md)
Aktualisieren einer Route im viel Hub

### [Satz-AzIotHubVerifiedCertificate](Set-AzIotHubVerifiedCertificate.md)
Überprüft ein Azure-viel-Hub-Zertifikat. 

### [Test-AzIotHubRoute](Test-AzIotHubRoute.md)
Test Routen im viel Hub

### [Update-AzIotHub](Update-AzIotHub.md)
Aktualisieren eines Azure viel-Hubs

### [Update-AzIotHubDeviceTwin](Update-AzIotHubDeviceTwin.md)
Aktualisiert Tags und die gewünschten Eigenschaften eines Geräte Zwillings.

### [Update-AzIotHubModuleTwin](Update-AzIotHubModuleTwin.md)
Aktualisiert Tags und gewünschte Eigenschaften eines viel Geräte Moduls Twin.

