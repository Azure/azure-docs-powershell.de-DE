...
### Beispiel 2: Erstellen einer API durch Kopieren aller Vorgänge, Tags, Produkte und Richtlinien aus der Echo-API und in eine ApiVersionSet
```powershell
...
Dieser Befehl erstellt eine API `echoapiv3` in ApiVersionSet `xmsVersionSet` und kopiert alle Vorgänge, Tags und Richtlinien aus der Quell-API `echo-api` . Sie überschreibt die SubscriptionRequired, serviceUrl, Path, Protocols

### Beispiel 3

Erstellt eine API. automatisch
...
Beschreibung der API-Version. Dieser Parameter ist optional.
...
### -BearerTokenSendingMethod
OpenID Authorization Server-Mechanismus, mit dem das Zugriffstoken an die API übergeben wird. Weitere Informationen finden Sie unter http://tools.ietf.org/html/rfc6749#section-4 . Dieser Parameter ist optional. Der Standardwert ist $NULL.
...
### -SourceApiId
API-Bezeichner der Quell-API. Dieser Parameter ist optional.
