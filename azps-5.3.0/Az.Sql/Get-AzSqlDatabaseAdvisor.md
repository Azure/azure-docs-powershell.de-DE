...
## BESCHREIBUNG
Das **Cmdlet "Get-AzSqlDatabaseAdvisor"** ruft mindestens einen Azure SQL Datenbankratgeber für eine Azure SQL ab.

## BEISPIELE

### Beispiel 1: Auflisten aller Berater für die angegebene Datenbank
```
...
Mit diesem Befehl werden alle Berater für die Datenbank mit dem Namen "WI Runner" aufgeführt, die zu dem Server mit dem Namen "wi-runner-australia-east" gehört.

### Beispiel 2: Einen einzigen Berater für die angegebene Datenbank erhalten
```
...
Dieser Befehl ruft den Berater mit dem Namen "CreateIndex" für die Datenbank mit dem Namen WI Eigenes ab.

### Beispiel 3: Auflisten aller Berater mit den in der Antwort enthaltenen empfohlenen Aktionen
```
...
Mit diesem Befehl werden alle Berater für die Datenbank mit dem Namen "WI Eigenes" mit den empfohlenen Aktionen in die Antwort einbezogen.
Da der Befehl den *Parameter "ExpandRecommendedActions"* verwendet, ruft das Cmdlet die empfohlenen Aktionen mit der Antwort ab.

### Beispiel 4: Einen einzigen Berater mit den empfohlenen Aktionen in der Antwort erhalten
```
...
Dieser Befehl ruft den Ratgeber mit dem Namen CreateIndex aus der Datenbank mit dem Namen WI Eine Antwort enthält die empfohlenen Aktionen.
Da der Befehl den *Parameter "ExpandRecommendedActions"* verwendet, ruft das Cmdlet die empfohlenen Aktionen mit der Antwort ab.

### Beispiel 5: Auflisten aller Berater für die angegebene Datenbank mithilfe von Filtern
```
...
Dieser Befehl ruft alle Berater für die Datenbank mit dem Namen WI Aus dem Server mit dem Namen "wi-runner-australia-east" auf und beginnt mit dem Buchstaben "d".
...
### -DatabaseName
Gibt den Namen der Datenbank an, für die dieses Cmdlet den Berater anfordert.
...
Gibt an, dass dieses Cmdlet die empfohlenen Aktionen mit der Antwort erhält.
...
## HINWEISE
* Schlüsselwörter: azure, azurerm, arm, resource, management, manager, sql, database, mssql, advisor
