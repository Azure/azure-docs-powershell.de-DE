...
## BESCHREIBUNG
Das **Cmdlet "Restore-AzSqlInstanceDatabase"** stellt eine Instanzdatenbank aus einer georedundierten Sicherung, einem Zeitpunkt in einer Livedatenbank oder einer langfristigen Aufbewahrungssicherung wieder her.
Die wiederhergestellte Datenbank wird als neue Instanzdatenbank erstellt.

## BEISPIELE

### Beispiel 1: Wiederherstellen einer Instanzdatenbank ab einem bestimmten Zeitpunkt
```
...
Der Befehl stellt die Instanzdatenbank Database01 aus der angegebenen Point-in-Time-Sicherung in der Instanzdatenbank namens "Database01_restored.

### Beispiel 2: Wiederherstellen einer Instanzdatenbank von einem Bestimmten Zeitpunkt in eine andere Instanz in einer anderen Ressourcengruppe
```
...
Der Befehl stellt die Instanzdatenbank "Database01" für die Instanz "managedInstance1" der Ressourcengruppe "ResourceGroup01" aus der angegebenen Point-in-Time-Sicherung in der Instanzdatenbank namens Database01_restored für die Instanz "managedInstance2" der Ressourcengruppe "ResourceGroup02" wieder her.

### Beispiel 3: Geo-Restore einer Instanzdatenbank
```
...
Der erste Befehl ruft die georedundierte Sicherung für die Datenbank namens "Database01" ab und speichert sie dann in $GeoBackup Variable.
Mit dem zweiten Befehl wird die Sicherung in $GeoBackup Instanzdatenbank mit dem Namen Database01_restored.

### Beispiel 4: Wiederherstellen einer gelöschten Instanzdatenbank ab einem bestimmten Zeitpunkt
```
...
Der erste Befehl ruft die gelöschte Instanzdatenbank mit dem Namen "DB1" in instanz "managedInstance1" ab.
Mit dem zweiten Befehl wird die abgerufene Datenbank aus der angegebenen Point-in-Time-Sicherung in die Instanzdatenbank mit dem Namen Database01_restored.

### Beispiel 5: Wiederherstellen einer gelöschten Instanzdatenbank ab einem bestimmten Zeitpunkt
```
PS C:\> $deletedDatabase = Get-AzSqlDeletedInstanceDatabaseBackup -ResourceGroupName "ResourceGroup01" -InstanceName "managedInstance1" -DatabaseName "DB1"
...
Der erste Befehl ruft die gelöschte Instanzdatenbank mit dem Namen "DB1" in instanz "managedInstance1" ab.
Der zweite Befehl stellt die abgerufene Datenbank mithilfe des Eingabeobjekts aus der angegebenen Point-in-Time-Sicherung in die Instanzdatenbank namens Database01_restored Datenbank wieder her.

### Beispiel 6: Wiederherstellen einer Datenbank aus der LTR-Sicherung.
```
...
Stellt die LTR-Sicherung mit der angegebenen Ressourcen-ID wieder her (die sie finden, indem Sie "Get-AzSqlInstanceDatabaseLongTermRetentionBackup" ausführen).
...
### -DeletionDate
Das Löschdatum der gelöschten Datenbank.
...
### -FromGeoBackup
Wiederherstellen aus einer Geosicherung.
...
Wiederherstellen aus einer langfristigen Aufbewahrungssicherung
...
### -GeoBackupObject
Das wiederherstellbare Instanzdatenbankobjekt, das wiederhergestellt werden soll
...
### -InputObject
Das wiederherzustellende Instanzdatenbankobjekt
...
### -Name
Der Wiederherstellungsname der Instanzdatenbank.
...
### -ResourceId
Die Ressourcen-ID des wiederherzustellende Instanzdatenbankobjekts
...
Der Name der Zielinstanzdatenbank, in der wiederhergestellt werden soll.
...
### -TargetInstanceName
Der Name der Zielinstanz, auf die wiederhergestellt werden soll.
Wenn keine Angabe ausgeführt wird, ist die Zielinstanz dieselbe wie die Quellinstanz.
...
Der Name der Zielressourcengruppe, auf die wiederhergestellt werden soll.
Wenn keine Angabe ausgeführt wird, ist die Zielressourcengruppe mit der Quellressourcengruppe identisch.
