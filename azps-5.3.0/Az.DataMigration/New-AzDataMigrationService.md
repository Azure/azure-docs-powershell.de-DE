...
## BESCHREIBUNG
Das New-AzDataMigrationService erstellt eine neue Instanz des Azure-Datenbankmigrationsdiensts. Dieses Cmdlet trägt den Namen der vorhandenen Azure-Ressourcengruppe, den eindeutigen Namen für die neue Instanz des zu erstellenden Azure-Datenbankmigrationsdiensts, die Region, in der die Instanz bereitgestellt wird, den Namen der DMS-Worker-SKU und den Namen des virtuellen Azure-Subnetzes, in dem sich der Dienst befinden soll. Es gibt keinen Parameter für den Abonnementnamen, da erwartet wird, dass der Benutzer das Standardabonnement der Azure-Anmeldesitzung anknrichte oder Get-AzSubscription -SubscriptionName "MySubscription" | Select-AzSubscription, um ein anderes Abonnement auszuwählen.
...
Das beispiel oben zeigt, wie Sie eine neue Instanz des Azure Database Migration Service namens TestService in der Region "USA, Mitte" erstellen.
...
### -Location
Der Speicherort der zu erstellende Azure-Datenbankmigrationsdienstinstanz, der einer Azure-Region entspricht.
...
### -SKU
Die SKU für die Instanz des Azure-Datenbankmigrationsdiensts. Mögliche Werte sind derzeit Basic_1vCore.Basic_2vCores.GeneralPurpose_4vCores
...
### -VirtualSubnetId
Der Name des Subnetzes unter dem angegebenen virtuellen Netzwerk, das für die Instanz des Azure Database Migration Service verwendet werden soll.
