...
### Beispiel 2: Hinzufügen einer API zu einer vorhandenen ApiVersionSet
```powershell
PS C:\>$ApiMgmtContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
In diesem Beispiel wird einer vorhandenen API-Versionssatz eine API hinzufügt.
...
In diesem Beispiel wird die ServiceUrl `echo-api` aktualisiert, auf die der Dienst zeigt.
...
Gibt die Id des OAuth-Autorisierungsservers an.
Der Standardwert ist $Null.
...
### -Path
Gibt den Web-API-Pfad an, der den letzten Teil der öffentlichen URL der API angibt.
...
Gibt den Namen der Kopfzeile des Abonnementschlüssels an.
Der Standardwert ist $Null.
...
Gibt den Namen des Abfragezeichenfolgeparameters für den Abonnementschlüssel an.
Der Standardwert ist $Null.
