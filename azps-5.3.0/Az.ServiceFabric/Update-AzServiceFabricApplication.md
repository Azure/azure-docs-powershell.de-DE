...
## SYNOPSIS
Aktualisieren Sie eine Service Fabric-Anwendung. Dadurch können die Anwendungsparameter aktualisiert und/oder die Anwendungstypversion aktualisiert werden, wodurch ein Anwendungsupgrade ausgelöst wird. Unterstützt nur ARM bereitgestellten Anwendungen.
...
In diesem Beispiel wird die minimale und maximale Anzahl der Knotenbeschränkung für die Anwendung aktualisiert.
...
In diesem Beispiel wird ein Anwendungsupgrade gestartet, um die Typversion auf "v2" zu aktualisieren, und außerdem einige Upgraderichtlinienparameter festgelegt, die ab dem aktuellen Upgrade wirksam werden.
...
In diesem Beispiel werden die Anwendungsparameter aktualisiert, doch werden diese Änderungen nur bis zum nächsten Upgrade der Version auf die Anwendung wirksam.
...
Gibt an, ob ein Warnungsinte health-Ereignis während der Integritätsauswertung als Fehlerereignis behandelt werden soll.
...
Gibt den in der Integritätsrichtlinie für den standardmäßigen Diensttyp zulässigen maximalen Prozentwert von nicht standardmäßigen Partitionen pro Dienst an, der für das überwachte Upgrade verwendet werden soll.
...
Gibt den maximalen Prozentwert der nicht standardmäßigen Replikate pro Dienst an, der von der Integritätsrichtlinie für den standardmäßigen Diensttyp für das überwachte Upgrade verwendet werden kann.
...
Gibt den maximalen Prozentwert der nicht standardmäßigen Dienste an, der in der Integritätsrichtlinie für den Standarddiensttyp zulässig ist, der für das überwachte Upgrade verwendet werden soll.
...
### -FailureAction
Gibt die Aktion an, die bei einem Fehler beim überwachten Upgrade zu ergreifen ist.
...
### -ForceRestart
Gibt an, dass der Diensthost neu gestartet wird, auch wenn es sich bei dem Upgrade nur um eine Konfigurationsänderung handelt.
...
Specifies the duration, in seconds, that Service Fabric waits to verify that the application is stable before moving to the next upgrade domain or completing the upgrade.
Durch diese Wartezeit werden nicht erkannte Integritätsänderungen direkt nach der Integritätsprüfung verhindert.
...
Specifies the duration, in seconds, that Service Fabric waits before it perform the initial health check after it finishes the upgrade on the upgrade domain.
...
Gibt die Zuordnung der Integritätsrichtlinie an, die für verschiedene Diensttypen als Hashtabelle im folgenden Format verwendet werden soll: @ {"ServiceTypeName": "MaxPercentUnhealthyPartitionsPerService,MaxPercentUnhealthyReplicasPerPartition,MaxPercentUnhealthyServices"}.
...
Gibt den maximalen Prozentsatz der Anwendungsinstanzen an, die auf den Knoten im Cluster bereitgestellt werden, für die ein Fehlerstatus vor dem Fehler des Anwendungsstatus für den Cluster auftritt.
