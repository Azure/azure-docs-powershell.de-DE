...
## BESCHREIBUNG
Mit Update-AzMlWebService cmdlet können Sie die nicht statischen Eigenschaften eines Webdiensts aktualisieren.
...
Hier ändern wir die Beschreibung, den primären Zugriffsschlüssel und aktivieren die Diagnosesammlung für alle Ablaufverfolgungen zur Laufzeit für den Webdienst.

### Beispiel 2: Aktualisieren auf Der Grundlage einer Webdienstinstanz
```
...
Im Beispiel wird zuerst eine Webdienstdefinition erstellt, die nur die zu aktualisierenden Felder enthält. Anschließend wird die Update-AzMlWebService aufgerufen, um sie mit dem Parameter "ServiceUpdates" anzuwenden.

## PARAMETERS

### -Assets
Die Gruppe von Ressourcen (z. B. Module, Datasets), aus der der Webdienst wird.
...
### -Input
Die Definition für die Eingaben des Webdiensts als Schemakonstrukt "Swagger".
...
Nach dem Festlegen kann der Webdienst länger aktualisiert werden, einschließlich der Änderung des Werts dieser Eigenschaft, und er kann nur gelöscht werden.
...
### -Keys
Aktualisiert einen oder beide der Zugriffstasten, die zum Authentifizieren von Aufrufen der Laufzeit-APIs des Diensts verwendet werden.
...
### -Output
Die Definition für die Ausgabe(n) des Webdiensts, die als Schemakonstrukt "Swagger" bereitgestellt wird.
...
### -Parameters
Die Gruppe der für den Webdienst definierten globalen Parameterwerte, die als globaler Parametername angegeben werden – \> Sammlung mit Standardwerten.
...
### -ServiceUpdates
Eine Reihe von Updates, die auf den Webdienst angewendet werden sollen, der als Webdienstdefinitionsinstanz bereitgestellt wird.
Es werden nur nicht statische Felder geändert.
