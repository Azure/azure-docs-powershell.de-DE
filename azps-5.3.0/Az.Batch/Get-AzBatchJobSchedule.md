...
## BESCHREIBUNG
Das **Cmdlet "Get-AzBatchJobSchedule"** ruft Projektzeitpläne für den Azure Batch ab, die vom Parameter *"BatchContext" angegeben* werden.
Geben Sie eine ID an, um einen einzelnen Auftragszeitplan zu erhalten.
Geben Sie den *Parameter "Filter"* an, um die Auftragszeitpläne zu erhalten, die einem Open Data Protocol (OData)-Filter entsprechen.

## BEISPIELE

### Beispiel 1: Erhalten eines Auftragsplans durch Angeben einer ID
```
...
Dieser Befehl ruft den Auftragszeitplan mit der ID "JobSchedule23" ab.
Verwenden Sie das Get-AzBatchAccountKey-Cmdlet, um der Variablen einen Kontext $Context zuzuordnen.

### Beispiel 2: Erhalten von Stellenplänen mithilfe eines Filters
```
...
Dieser Befehl ruft alle Auftragsterminpläne ab, die IDs haben, die mit "Auftrag" beginnen, indem der Parameter *"Filter" angegeben* wird.
...
Dieses Cmdlet gibt Auftragsterminpläne zurück, die dem von diesem Parameter angegebenen Filter entsprechen.
Wenn Sie keinen Filter angeben, gibt dieses Cmdlet alle Auftragsterminpläne für den Kontext "Batch" zurück.
...
### -ID
Gibt die ID des Auftragsplans an, den dieses Cmdlet erhält.
...
### -MaxCount
Gibt die maximale Anzahl der zurückzukehrenden Stellenpläne an.
