---
Module Name: Az.Batch
Module Guid: a8f00f40-1c1a-49b5-9db3-24076b75c3cf
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.batch
Help Version: 4.0.0.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Batch/Batch/help/Az.Batch.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Batch/Batch/help/Az.Batch.md
ms.openlocfilehash: f6bd55502c5c800bff245b905bc5334b1bf35bdb
ms.sourcegitcommit: 68451baa389791703e666d95469602c5652609ee
ms.translationtype: MT
ms.contentlocale: de-DE
ms.lasthandoff: 01/05/2021
ms.locfileid: "98471218"
---
# Az.Batch Module
## Beschreibung
Mit den Azure -Batch-Cmdlets im Azure-Modul können Sie Microsoft Azure Batch services in Azure PowerShell verwalten.

## Az.Batch-Cmdlets
### [Disable-AzBatchAutoScale](Disable-AzBatchAutoScale.md)
Deaktiviert die automatische Skalierung eines Pools.

### [Disable-AzBatchComputeNodeScheduling](Disable-AzBatchComputeNodeScheduling.md)
Deaktiviert die Vorgangsplanung für den angegebenen Rechenknoten.

### [Disable-AzBatchJob](Disable-AzBatchJob.md)
Deaktiviert einen Stapelauftrag.

### [Disable-AzBatchJobSchedule](Disable-AzBatchJobSchedule.md)
Deaktiviert einen Stapelauftragszeitplan.

### [Enable-AzBatchAutoScale](Enable-AzBatchAutoScale.md)
Ermöglicht die automatische Skalierung eines Pools.

### [Enable-AzBatchComputeNodeScheduling](Enable-AzBatchComputeNodeScheduling.md)
Ermöglicht die Aufgabenplanung für den angegebenen Rechenknoten.

### [Enable-AzBatchJob](Enable-AzBatchJob.md)
Aktiviert einen Stapelauftrag.

### [Enable-AzBatchJobSchedule](Enable-AzBatchJobSchedule.md)
Aktiviert einen Stapelauftragszeitplan.

### [Enable-AzBatchTask](Enable-AzBatchTask.md)
Reaktiviert einen Vorgang.

### [Get-AzBatchAccount](Get-AzBatchAccount.md)
Ruft ein Batchkonto im aktuellen Abonnement ab.

### [Get-AzBatchAccountKey](Get-AzBatchAccountKey.md)
Ruft die Schlüssel eines Batchkontos ab.

### [Get-AzBatchApplication](Get-AzBatchApplication.md)
Ruft Informationen zur angegebenen Anwendung ab.

### [Get-AzBatchApplicationPackage](Get-AzBatchApplicationPackage.md)
Ruft Informationen zu einem Anwendungspaket in einem Batchkonto ab.

### [Get-AzBatchCertificate](Get-AzBatchCertificate.md)
Ruft die Zertifikate in einem Batchkonto ab.

### [Get-AzBatchComputeNode](Get-AzBatchComputeNode.md)
Ruft Stapelberechnungsknoten aus einem Pool ab.

### [Get-AzBatchJob](Get-AzBatchJob.md)
Ruft Stapelaufträge für ein Batchkonto oder einen Auftragszeitplan ab.

### [Get-AzBatchJobPreparationAndReleaseTaskStatus](Get-AzBatchJobPreparationAndReleaseTaskStatus.md)
Ruft die Batchauftragsvorbereitung und den Status der Veröffentlichung ab.

### [Get-AzBatchJobSchedule](Get-AzBatchJobSchedule.md)
Ruft Stapelauftragszeitpläne ab.

### [Get-AzBatchJobStatistic](Get-AzBatchJobStatistic.md)
Ruft Auftragszusammenfassungsstatistiken für ein Batchkonto ab.

### [Get-AzBatchLocationQuota](Get-AzBatchLocationQuota.md)
Ruft die Batchdienstkontingente für Ihr Abonnement am angegebenen Speicherort ab.

### [Get-AzBatchNodeFile](Get-AzBatchNodeFile.md)
Ruft die Eigenschaften von Batchknotendateien ab.

### [Get-AzBatchNodeFileContent](Get-AzBatchNodeFileContent.md)
Ruft eine Batchknotendatei ab.

### [Get-AzBatchPool](Get-AzBatchPool.md)
Ruft Batchpools unter dem angegebenen Batchkonto ab.

### [Get-AzBatchPoolNodeCount](Get-AzBatchPoolNodeCount.md)
Ruft die Batchknotenanzahl pro Knotenstatus nach Pool-ID gruppieren ab.

### [Get-AzBatchPoolStatistic](Get-AzBatchPoolStatistic.md)
Ruft Zusammenfassungsstatistiken für einen Pool für ein Batchkonto ab.

### [Get-AzBatchPoolUsageMetric](Get-AzBatchPoolUsageMetric.md)
Ruft Poolnutzungsmetriken für ein Batchkonto ab.

### [Get-AzBatchRemoteDesktopProtocolFile](Get-AzBatchRemoteDesktopProtocolFile.md)
Ruft eine RDP-Datei aus einem Rechenknoten ab.

### [Get-AzBatchRemoteLoginSetting](Get-AzBatchRemoteLoginSetting.md)
Ruft Remoteanmeldungseinstellungen für einen Rechenknoten ab.

### [Get-AzBatchSubtask](Get-AzBatchSubtask.md)
Ruft die Teilvorganginformationen des angegebenen Vorgangs ab.

### [Get-AzBatchSupportedImage](Get-AzBatchSupportedImage.md)
Ruft unterstützte Stapelbilder für ein Batchkonto ab.

### [Get-AzBatchTask](Get-AzBatchTask.md)
Ruft die Stapelaufgaben für einen Auftrag ab.

### [Get-AzBatchTaskCount](Get-AzBatchTaskCount.md)
Ruft die Anzahl der Aufgaben für den angegebenen Auftrag ab.

### [New-AzBatchAccount](New-AzBatchAccount.md)
Erstellt ein Batchkonto.

### [New-AzBatchAccountKey](New-AzBatchAccountKey.md)
Generiert einen Schlüssel eines Batchkontos erneut.

### [New-AzBatchApplication](New-AzBatchApplication.md)
Fügt dem angegebenen Batchkonto eine Anwendung hinzu.

### [New-AzBatchApplicationPackage](New-AzBatchApplicationPackage.md)
Erstellt ein Anwendungspaket in einem Batchkonto.

### [New-AzBatchCertificate](New-AzBatchCertificate.md)
Fügt dem angegebenen Batchkonto ein Zertifikat hinzu.

### [New-AzBatchComputeNodeUser](New-AzBatchComputeNodeUser.md)
Erstellt ein Benutzerkonto auf einem Batchberechnungsknoten.

### [New-AzBatchJob](New-AzBatchJob.md)
Erstellt einen Auftrag im Batchdienst.

### [New-AzBatchJobSchedule](New-AzBatchJobSchedule.md)
Erstellt einen Auftragszeitplan im Batchdienst.

### [New-AzBatchPool](New-AzBatchPool.md)
Erstellt einen Pool im Batchdienst.

### [New-AzBatchResourceFile](New-AzBatchResourceFile.md)
Erstellt eine Ressourcendatei zur Verwendung durch `New-AzBatchTask` .

### [New-AzBatchTask](New-AzBatchTask.md)
Erstellt eine Batchaufgabe unter einem Auftrag.

### [Remove-AzBatchAccount](Remove-AzBatchAccount.md)
Entfernt ein Batchkonto.

### [Remove-AzBatchApplication](Remove-AzBatchApplication.md)
Löscht eine Anwendung aus einem Batchkonto.

### [Remove-AzBatchApplicationPackage](Remove-AzBatchApplicationPackage.md)
Löscht einen Anwendungspaketdatensatz und die Binärdatei.

### [Remove-AzBatchCertificate](Remove-AzBatchCertificate.md)
Löscht ein Zertifikat aus einem Konto.

### [Remove-AzBatchComputeNode](Remove-AzBatchComputeNode.md)
Entfernt Berechnungsknoten aus einem Pool.

### [Remove-AzBatchComputeNodeUser](Remove-AzBatchComputeNodeUser.md)
Löscht ein Benutzerkonto aus einem Batchverarbeitungsknoten.

### [Remove-AzBatchJob](Remove-AzBatchJob.md)
Löscht einen Stapelauftrag.

### [Remove-AzBatchJobSchedule](Remove-AzBatchJobSchedule.md)
Entfernt einen Stapelauftragszeitplan.

### [Remove-AzBatchNodeFile](Remove-AzBatchNodeFile.md)
Löscht eine Knotendatei für eine Aufgabe oder einen Rechenknoten.

### [Remove-AzBatchPool](Remove-AzBatchPool.md)
Löscht den angegebenen Batchpool.

### [Remove-AzBatchTask](Remove-AzBatchTask.md)
Löscht eine Batchaufgabe.

### [Reset-AzBatchComputeNode](Reset-AzBatchComputeNode.md)
Installiert das Betriebssystem erneut auf dem angegebenen Rechenknoten.

### [Restart-AzBatchComputeNode](Restart-AzBatchComputeNode.md)
Der angegebene Rechenknoten wird neu gestartet.

### [Set-AzBatchAccount](Set-AzBatchAccount.md)
Aktualisiert ein Batchkonto.

### [Set-AzBatchApplication](Set-AzBatchApplication.md)
Aktualisiert die Einstellungen für die angegebene Anwendung.

### [Set-AzBatchComputeNodeUser](Set-AzBatchComputeNodeUser.md)
Ändert die Eigenschaften eines Kontos auf einem Batchverarbeitungsknoten.

### [Set-AzBatchJob](Set-AzBatchJob.md)
Aktualisiert einen Batchauftrag.

### [Set-AzBatchJobSchedule](Set-AzBatchJobSchedule.md)
Legt einen Auftragszeitplan fest.

### [Set-AzBatchPool](Set-AzBatchPool.md)
Aktualisiert die Eigenschaften eines Pools.

### [Set-AzBatchTask](Set-AzBatchTask.md)
Aktualisiert die Eigenschaften einer Aufgabe.

### [Start-AzBatchComputeNodeServiceLogUpload](Start-AzBatchComputeNodeServiceLogUpload.md)
Laden Sie Die Protokolldateien des Compute Node Service in einen Azure Storage Container hoch.

### [Start-AzBatchPoolResize](Start-AzBatchPoolResize.md)
Beginnt mit dem Ändern der Größe eines Pools.

### [Stop-AzBatchCertificateDeletion](Stop-AzBatchCertificateDeletion.md)
Ein fehlgeschlagenes Löschen eines Zertifikats wird abgebrochen.

### [Stop-AzBatchJob](Stop-AzBatchJob.md)
Beendet einen Stapelauftrag.

### [Stop-AzBatchJobSchedule](Stop-AzBatchJobSchedule.md)
Beendet einen Stapelauftragszeitplan.

### [Stop-AzBatchPoolResize](Stop-AzBatchPoolResize.md)
Beendet den Größenänderungsvorgang eines Pools.

### [Stop-AzBatchTask](Stop-AzBatchTask.md)
Beendet eine Batchaufgabe.

### [Test-AzBatchAutoScale](Test-AzBatchAutoScale.md)
Ruft das Ergebnis einer automatischen Skalierungsformel für einen Pool ab.

