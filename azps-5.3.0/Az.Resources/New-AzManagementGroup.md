...
Erstellen einer neuen Gruppe mit `DisplayName` und `ParentId` festlegen auf `null` Dies ist identisch mit dem übergeordneten Element der Gruppe und das übergeordnete Element `DisplayName` der Gruppe ist der `GroupName` Mandant.  
