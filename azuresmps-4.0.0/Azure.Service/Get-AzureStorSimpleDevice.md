...
## Synopsis
Ruft Geräte an, die an die Ressource angefügt sind.
...
## Beschreibung
Das Cmdlet " **Get-AzureStorSimpleDevice** " Ruft eine Liste der StorSimple-Geräte ab, die der Ressource zugeordnet sind.
Sie können die Geräte-ID, den Namen, die Modell-ID und den Typ angeben.
Verwenden Sie die mit diesem Cmdlet abgerufene **Device** -Eigenschaft, um Geräte für andere StorSimple-Cmdlets anzugeben.

## Beispiele

### Beispiel 1: Abrufen verfügbarer Geräte für eine Ressource
```
...
Dieser Befehl ruft alle verfügbaren Geräte für eine Ressource ab.
In diesem Beispiel steht nur ein Gerät zur Verfügung.

### Beispiel 2: Abrufen bestimmter verfügbarer Geräte für eine Ressource
```
...
Dieser Befehl ruft alle verfügbaren Geräte für eine Ressource ab, die den angegebenen Namen, Typ und die Modell-ID aufweisen.

### Beispiel 3: Abrufen von Details für ein Gerät
```
...
Dieser Befehl ruft alle verfügbaren Geräte für eine Ressource ab, die den angegebenen Namen und Typ aufweisen.
Dieser Befehl gibt den *detaillierten* Parameter an.
Der Befehl bietet zusätzliche Details zu den von ihm zurückgegebenen Geräten.

## Parameter

### -Detailliert
Gibt an, dass dieses Cmdlet Gerätedetails für die Geräte zurückgibt, die es erhält.
...
### -Geräte-Nr
Gibt die Instanz-ID des abzurufenden Geräts an.
...
### -DeviceName
Gibt den Namen des abzurufenden StorSimple-Geräts an.
...
### -Modell-Nr.
Gibt die ID des Modells der StorSimple-Geräte an, die abgerufen werden sollen.
...
### -Typ
Gibt den Typ eines StorSimple-Geräts an.
Gültige Werte sind: Appliance und VirtualAppliance.
...
Wenn Sie den *detaillierten* Parameter angeben, gibt dieses Cmdlet ein **List \<DeviceDetails\>** -Objekt zurück.
Wenn Sie diesen Parameter nicht angeben, wird ein **IEnumerable \<DeviceInfo\>** -Objekt zurückgegeben.
