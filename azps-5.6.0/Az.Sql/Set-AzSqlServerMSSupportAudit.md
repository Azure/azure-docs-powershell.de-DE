...
## SYNOPSIS
Ändert die Überwachungseinstellungen für Microsoft-Supportvorgänge eines Azure SQL Server.
...
## BESCHREIBUNG
Das **Cmdlet Set-AzSqlServerMSSupportAudit** ändert die Einstellungen für die Überwachung von Microsoft-Supportvorgängen eines Azure SQL Server.
...
### Beispiel 1: Aktivieren der Microsoft-Supportüberwachungsrichtlinie für Den Blobspeicher eines Azure SQL Server
```powershell
...
### Beispiel 2: Deaktivieren der Überwachungsrichtlinie für Den Blobspeicher microsoft support operations eines Azure SQL Server
```powershell
PS C:\>Set-AzSqlServerMSSupportAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -BlobStorageTargetState Disabled
```

### Beispiel 3: Aktivieren der Microsoft-Supportüberwachungsrichtlinie für Vorgänge eines Azure SQL Server
```powershell
...
### Beispiel 4: Deaktivieren der Microsoft-Supportüberwachungsrichtlinie für Vorgänge eines Azure SQL Server
```powershell
PS C:\>Set-AzSqlServerMSSupportAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -EventHubTargetState Disabled
```

### Beispiel 5: Aktivieren der Überwachungsrichtlinie für Die Überwachung von Vorgängen durch Microsoft für die Protokollanalyse eines Azure SQL Server
```powershell
...
### Beispiel 6: Deaktivieren der Überwachungsrichtlinie für Die Überwachung von Vorgängen durch Microsoft für die Protokollanalyse eines Azure SQL Server
```powershell
...
### Beispiel 7: Deaktivieren der Überwachungsrichtlinie für Die Überwachung von Vorgängen durch Microsoft durch die Pipeline eines Azure SQL Server
```powershell
...
### Beispiel 8: Deaktivieren Sie das Senden von Überwachungsprotokollen für Microsoft-Supportvorgänge eines Azure SQL-Servers an den Blobspeicher, und aktivieren Sie das Senden zum Protokollieren von Analysen.
```powershell
...
### Beispiel 9: Aktivieren des Sendens von Überwachungsprotokollen eines Azure SQL-Servers an blob storage, event hub und log analytics.
```powershell
...
Gibt an, ob blob storage ein Ziel für Überwachungsdatensätze für Microsoft-Supportvorgänge ist.
...
### -EventHubName
Der Name des Ereignishubs. Wenn beim Bereitstellen von EventHubAuthorizationRuleResourceId keine angegeben wird, wird der Standardereignishub ausgewählt.
...
### -EventHubTargetState
Gibt an, ob der Ereignishub ein Ziel für Überwachungsprotokolle für Microsoft-Supportvorgänge ist.
...
Gibt an, ob die Protokollanalyse ein Ziel für Überwachungsprotokolle für Microsoft-Supportvorgänge ist.
...
### -PassThru
Gibt an, ob die Überwachungsrichtlinie für Microsoft-Supportvorgänge am Ende der Cmdletausführung ausgegeben werden soll.
...
### -ServerObject
Das Serverobjekt zum Verwalten seiner Überwachungsrichtlinie für Microsoft-Supportvorgänge.
...
### -WorkspaceResourceId
Die Arbeitsbereichs-ID (Ressourcen-ID eines Log Analytics-Arbeitsbereichs) für einen Log Analytics-Arbeitsbereich, an den Sie Überwachungsprotokolle für Microsoft-Supportvorgänge senden möchten. Beispiel: /subscriptions/4b9e8510-67ab-4e9a-95a9-e2f1e570ea9c/resourceGroups/insights-integration/providers/Microsoft.OperationalInsights/workspaces/viruela2
