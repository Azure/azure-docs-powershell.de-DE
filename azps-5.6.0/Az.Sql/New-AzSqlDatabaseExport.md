...
## BESCHREIBUNG
Das **New-AzSqlDatabaseExport-Cmdlet** exportiert eine Azure SQL-Datenbank als BACPAC-Datei in ein Speicherkonto.
Die Anforderung zum Abrufen des Exportdatenbankstatus wird möglicherweise gesendet, um Statusinformationen für diese Anforderung abzurufen.
Dieses Cmdlet wird auch vom SQL Server Stretch Database Service in Azure unterstützt.

> [!IMPORTANT]
> Um dieses Cmdlet nutzen zu können, muss die Firewall im Azure SQL Server so konfiguriert sein, dass "Azure-Dienste und -Ressourcen den Zugriff auf diesen Server zulassen" ausgeführt wird. Wenn dies nicht konfiguriert ist, werden GatewayTimeout-Fehler auftreten.
...
Legen Sie *AdministratorLogin* und *AdministratorLoginPassword* auf den Benutzernamen und das Kennwort SQL Administrator ein. 
- ADPassword.
Azure Active Directory-Authentifizierung.
Legen *Sie AdministratorLogin* und *AdministratorLoginPassword* auf den Benutzernamen und das Kennwort des Azure AD-Administrators fest.
...
- SharedAccessKey.
Für diesen Wert wird ein SAS-Schlüssel (Shared Access Signature) verwendet.
...
### -StorageUri
Gibt den Bloblink als URL für die BACPAC-Datei an.
