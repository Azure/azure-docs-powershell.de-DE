...
## SYNOPSIS
Stellt eine Azure SQL Verwaltete Instanz-Datenbank wieder bereit.
...
### Beispiel 1: Wiederherstellen einer Instanzdatenbank von einem Zeitpunkt aus
```
...
Der Befehl stellt die Instanzdatenbank Datenbank01 aus der angegebenen Punkt-in-Zeit-Sicherung in der Instanzdatenbank mit dem Namen Database01_restored.

### Beispiel 2: Wiederherstellen einer Instanzdatenbank von einem Zeitpunkt auf eine andere Instanz in einer anderen Ressourcengruppe
```
...
### Beispiel 4: Wiederherstellen einer gelöschten Instanzdatenbank von einem Zeitpunkt aus
```
...
Der erste Befehl ruft die gelöschten Instanzdatenbanken mit dem Namen "DB1" in der Instanz "managedInstance1" ab.
Mit dem zweiten Befehl wird die abgerufene Datenbank von der angegebenen Punkt-in-Zeit-Sicherung in die Instanzdatenbank mit dem Namen Database01_restored.

### Beispiel 5: Wiederherstellen einer gelöschten Instanzdatenbank von einem Zeitpunkt aus
```
PS C:\> $deletedDatabase = Get-AzSqlDeletedInstanceDatabaseBackup -ResourceGroupName "ResourceGroup01" -InstanceName "managedInstance1" -DatabaseName "DB1"
...
Der erste Befehl ruft die gelöschten Instanzdatenbanken mit dem Namen "DB1" in der Instanz "managedInstance1" ab.
Mit dem zweiten Befehl wird die abgerufene Datenbank von der angegebenen Punkt-in-Zeit-Sicherung in die Instanzdatenbank mit dem Namen Database01_restored Eingabeobjekts wiederhergestellt.
...
Stellt die LTR-Sicherung mit der angegebenen Ressourcen-ID wieder her (die Sie finden können, indem Sie Get-AzSqlInstanceDatabaseLongTermRetentionBackup ausführen).
...
Wiederherstellen aus einer Punkt-in-Zeit-Sicherung.
...
### -InputObject
Das wiederherstellende Instanzdatenbankobjekt
...
### -Name
Der Name der Instanzdatenbank, der wiederhergestellt werden soll.
...
### -PointInTime
Der Zeitpunkt zum Wiederherstellen der Datenbank.
...
### -ResourceId
Die Ressourcen-ID des zu wiederherstellende Instanzdatenbankobjekts
...
### -TargetInstanceName
Der Name der Zielinstanz, auf der wiederhergestellt werden soll.
Wenn nicht angegeben, ist die Zielinstanz mit der Quellinstanz identisch.
...
Der Name der Zielressourcengruppe, die wiederhergestellt werden soll.
Wenn nicht angegeben, ist die Zielressourcengruppe mit der Quellressourcengruppe identisch.
