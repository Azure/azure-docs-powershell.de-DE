...
### Beispiel 1: Aktivieren der Blobspeicherüberwachungsrichtlinie einer Azure SQL Datenbank
```powershell
...
### Beispiel 2: Deaktivieren der Blobspeicherüberwachungsrichtlinie einer Azure SQL Datenbank
```powershell
...
### Beispiel 3: Aktivieren der Blobspeicherüberwachungsrichtlinie einer Azure SQL-Datenbank mit Filtern mithilfe eines T-SQL Prädikats
```powershell
...
### Beispiel 10: Deaktivieren sie das Senden von Überwachungsdatensätzen einer Azure SQL-Datenbank an den Blobspeicher, und aktivieren Sie das Senden zum Protokollieren von Analysen.
```powershell
...
### Beispiel 11: Aktivieren des Sendens von Überwachungsdatensätzen einer Azure SQL-Datenbank an blob storage, event hub und log analytics.
```powershell
...
### -AuditAction
Die Gruppe der Überwachungsaktionen.  
Die unterstützten Aktionen zur Überwachung sind:  
SELECT  
UPDATE  
EINFÜGEN  
LÖSCHEN  
...
RECEIVE  
VERWEISE  
Das allgemeine Formular zum Definieren einer zu überwachenden Aktion ist: [Aktion] EIN [Objekt] BY [Prinzipal] Beachten Sie, dass [Objekt] im oben angegebenen Format auf ein Objekt wie eine Tabelle, Ansicht oder gespeicherte Prozedur oder eine gesamte Datenbank oder ein gesamtes Schema verweisen kann. In letzteren Fällen werden die Formulare DATABASE::[dbname] und SCHEMA::[Schemaname] verwendet.
Zum Beispiel:  
SELECT on dbo.myTable by public  
SELECT on DATABASE::myDatabase by public  
SELECT on SCHEMA::mySchema by public  
Weitere Informationen finden Sie unter https://docs.microsoft.com/sql/relational-databases/security/auditing/sql-server-audit-action-groups-and-actions#database-level-audit-actions .
