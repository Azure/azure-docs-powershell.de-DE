...
## SYNOPSIS
Get-AzPolicyAlias Azure-Anbieterressourcentypen abruft und aus, deren Aliase definiert sind und mit den angegebenen Parameterwerten übereinstimmen. Wenn keine Parameter bereitgestellt werden, werden alle Anbieterressourcentypen ausgegeben, die einen Alias enthalten.
Der Schalter -ListAvailable ändert dieses Verhalten, indem alle übereinstimmenden Ressourcentypen einschließlich derjenigen ohne Aliase aufgeführt werden.
...
## BESCHREIBUNG
Das **Get-AzPolicyAlias-Cmdlet** ruft eine Auflistung der Richtlinienaliase ab.
Richtlinienaliase werden von Azure Policy verwendet, um auf Ressourcentypeigenschaften zu verweisen.
Es werden Parameter bereitgestellt, die Elemente in der Auflistung einschränken, indem verschiedene Eigenschaften des Ressourcentyps oder der zugehörigen Aliase übereinstimmen.
Ein gegebener Übereinstimmungswert entspricht, wenn die Zielzeichenfolge ihn enthält, und verwendet einen Vergleich ohne Groß-/Kleinschreibung.
...
Listet alle Anbieterressourcentypen auf, auch solche ohne Aliase.
...
Listet alle Anbieterressourcentypen auf, deren Namespace "compute" entspricht und einen Alias enthält.
...
Listet alle Anbieterressourcentypen auf, deren Ressourcentyp "virtuell" entspricht, einschließlich derjenigen ohne Aliase.
...
Listet alle Anbieterressourcentypen auf, deren Namespace "Compute" und Ressourcentyp "virtuell" entspricht und einen Alias enthält.
Hinweis: -NamespaceMatch und -ResourceTypeMatch bieten exklusive Übereinstimmungen, während die anderen übereinstimmen.
...
Listet alle Anbieterressourcentypen auf, die einen Alias enthalten, der "virtuell" enthält.
...
Listet alle Anbieterressourcentypen auf, die einen Alias enthalten, der "virtuell" oder einen Alias mit einem Pfad enthält, der mit "Netzwerk" übereinstimmen.
...
Listet alle Anbieterressourcentypen mit Alpha-API-Version oder einem Alias mit einer Alpha-API-Version auf.

## PARAMETER

### -AliasMatch
Enthält in die Ausgabeelemente mit Aliasen, deren Name diesem Wert entspricht.
...
### -ApiVersion
Wenn festgelegt, gibt die Version der zu verwendenden Ressourcenanbieter-API an. Wenn nicht angegeben, wird die API-Version automatisch als die neueste verfügbare Version bestimmt.
...
### -ApiVersionMatch
Enthält in die Ausgabeelemente, deren Ressourcentypen oder Aliase über eine übereinstimmende Apiversion verfügen.
...
### -LocationMatch
Enthält in die Ausgabeelemente, deren Ressourcentypen über einen übereinstimmenden Speicherort verfügen.
...
* Zusätzliche Eigenschaften sind in der Ausgabe verfügbar und können angezeigt werden, indem sie die Ausgabe an `Format-List` piping. Zum Beispiel: `Get-AzPolicyAlias -NamespaceMatch 'Web' -ResourceTypeMatch site -PathMatch cert | Format-List`
