...
Erstellen einer neuen Gruppe mit `DisplayName` und `ParentId` auf `null` . Der `DisplayName` ist identisch mit dem, und das übergeordnete Element der Gruppe ist der `GroupName` Mandant.  
...
In diesem Fall ist das übergeordnete Element der Gruppe der Mandant, und das -Element wird `DisplayName` auf den angegebenen Wert festgelegt.
...
### Beispiel 4: Erstellen einer Verwaltungsgruppe mit einem übergeordneten Objekt (mit einem übergeordneten Objekt)
```
