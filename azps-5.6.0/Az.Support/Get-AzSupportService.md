...
## BESCHREIBUNG
Ruft die aktuelle Liste der Azure-Dienste ab, für die Support verfügbar ist. Jeder Dienst kann mindestens einen Azure Resource Manager (ARM) informationen zum Ressourcentyp enthalten. Ressourcentypen (Beispiel: "microsoft.compute/virtualmachines")) können nützlich sein, um das richtige Produkt und die richtige ARM beim Erstellen eines Technischen Supporttickets zu zuordnungen. Jeder Azure-Dienst verfügt über einen eigenen Satz von Kategorien, die als Problemklassifizierung bezeichnet werden. Hier erhalten Sie die aktuelle Liste der Problemklassifizierung für einen Dienst mit Get-AzSupportProblemClassification. Sie können die GUID für Dienst- und Problemklassifizierung verwenden, um ein neues Supportticket mit New-AzSupportTicket zu erstellen.

Verwenden Sie immer die dienst- und problemklassifizierungs-GUIDs, die programmgesteuert ermittelt wurden. Diese Vorgehensweise stellt sicher, dass Sie über die neuesten GUIDs für Dienst- und Problemklassifizierungen für die Erstellung von Supporttickets verfügen.

## BEISPIELE

### Beispiel 1: Erhalten aller dienste, die für den Support verfügbar sind
```powershell
...
### Beispiel 2: Erhalten von Details zu einem einzelnen Dienst nach ID, der für den Support verfügbar ist
```powershell
