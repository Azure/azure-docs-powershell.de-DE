...
## BESCHREIBUNG
Listen `Reservation` s innerhalb eines einzelnen `ReservationOrder` .
...
Liste `Reservation` s innerhalb des angegebenen `ReservationOrder` .
...
Hier erhalten Sie spezifische `Reservation` Details.
...
### -ReservationId
ID des `Reservation` zu betrachtende
...
### -ReservationOrder
Parameter des Pipeobjekts für `ReservationOrder`
...
### -ReservationOrderId
ID des `ReservationOrder` - `Reservation` Erforderlich.
