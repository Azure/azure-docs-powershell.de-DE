...
Importieren sie "busybox" aus "Source ACR" in "Target ACR". Hinweis: 
...
### -Modus
Beim Erzwingen werden alle vorhandenen Zieltags überschrieben.
Wenn NoForce, alle vorhandenen Zieltags den Vorgang fehlschlagen, bevor das Kopieren beginnt.
...
### -Password
Das Kennwort, das zum Authentifizieren bei der Quellregistrierung verwendet wird.
...
Dabei wird das "neueste" -Tag verwendet.

Geben Sie ein Bild nach Tag an ('hello-world:latest').

Geben Sie ein Bild anhand des sha256-basierten Manifestdaps (' hello-world@sha256:abc123 ') an.
...
Der Ressourcenbezeichner der Azure Container Registry-Quelle.
...
### -TargetTag
Liste der Zeichenfolgen des Formulars repo \[ :tag \] .
Wenn das Tag nicht angegeben wird, wird die Quelle verwendet (oder "neueste", wenn das Quelltag ebenfalls nicht angegeben ist).
...
Liste der Zeichenfolgen mit Repositorynamen, die nur eine Manifestkopie erstellen.
