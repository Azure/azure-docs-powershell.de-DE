...
Mit diesem Befehl wird der Computeknoten entfernt, der die angegebene ID aus dem Pool mit dem ID-Pool07 hat.
...
Dieser Befehl ruft den Computeknoten ab, der über die angegebene ID aus dem Pool verfügt, der über den ID-Pool07 verfügt, indem er das cmdlet Get-AzBatchComputeNode verwendet.
Der Befehl übergibt diesen Knoten mithilfe der Pipeline an das aktuelle Cmdlet.
Das aktuelle Cmdlet entfernt den Computeknoten.
Der Befehl gibt den Parameter *"Erzwingen"* an.
Daher werden Sie mit dem Befehl nicht zur Bestätigung aufgefordert.
...
Mit diesem Befehl werden zwei Rechenknoten aus dem Pool entfernt, der über den ID-Pool07 verfügt.
...
### -ComputeNode
Gibt das **PSComputeNode-Objekt** an, das den von diesem Cmdlet entfernten Computeknoten darstellt.
...
### -DeallocationOption
Gibt eine Deallocation-Option für den Entfernungsvorgang an, den dieses Cmdlet startet.
...
### -Ids
Gibt ein Array von IDs von Computeknoten an, das von diesem Cmdlet aus dem Pool entfernt wird.
...
### -PoolId
Gibt die ID des Pools an, der die von diesem Cmdlet entfernten Computeknoten enthält.
...
### -ResizeTimeout
Gibt das Zeitintervall für das Entfernen der Computeknoten aus dem Pool an.
