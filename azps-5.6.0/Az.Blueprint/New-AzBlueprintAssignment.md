...
Erstellen Sie eine neue Blaupausenzuordnung der Blaupausendefinition innerhalb des angegebenen Abonnements mithilfe des definierten Parameters und des Ressourcengruppenwörterbuchs mit der angegebenen `$blueprintObject` vom Benutzer zugewiesenen Identitäts-ID.
...
Erstellen Sie mithilfe des definierten Parameters eine neue Blaupausenzuordnung der Blaupausendefinition für das angegebene Abonnement innerhalb der angegebenen `$blueprintObject` Verwaltungsgruppe.

## PARAMETER

### -AssignmentFile
Speicherort der Aufgabendatei im JSON-Format auf dem Datenträger.
...
### -ManagementGroupId
Die ID der Verwaltungsgruppe, in der die Blaupausenzuordnungen gespeichert werden.
...
Hashtable mit Parametern, die an das Ressourcengruppenartefakt übergeben werden.
...
### -SubscriptionId
Abonnement-ID, um die Blaupausendefinition zuzuordnen.
...
System assigned identity(MSI) zum Bereitstellen der Artefakte.
...
Benutzer assigned identity(MSI) to deploy the artifacts.
