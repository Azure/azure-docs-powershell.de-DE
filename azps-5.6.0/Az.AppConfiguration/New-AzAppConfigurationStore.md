...
### Beispiel 2: Erstellen einer App-Konfiguration mit dem Auf "UserAssigned" festgelegten IdentityType
```powershell
...
Mit diesem Befehl wird eine App-Konfiguration erstellt und ihr eine vom Benutzer zugewiesene verwaltete Identität zugewiesen.
Im Beispiel für finden Sie die folgenden Schritte zum Aktivieren `Update-AzAppConfigurationStore` von CMK (cusomer managed key).

### Beispiel 3: Erstellen einer App-Konfiguration mit dem Auf "SystemAssigned" festgelegten IdentityType 
```powershell
...
### Beispiel 4: Erstellen einer App-Konfiguration mit dem Auf "SystemAssigned, UserAssigned" festgelegten IdentityType
```powershell
PS C:\> $assignedIdentity = New-AzUserAssignedIdentity -ResourceGroupName azpwsh-manual-test -Name assignedIdentity
...
Sie können die vom System zugewiesene verwaltete Identität aktivieren und benutzer zugewiesene Identitäten gleichzeitig vergeben.
Im Beispiel für finden Sie die folgenden Schritte zum Aktivieren `Update-AzAppConfigurationStore` von CMK (cusomer managed key).
...
Dies kann nach dem Erstellen der Ressource nicht geändert werden.
