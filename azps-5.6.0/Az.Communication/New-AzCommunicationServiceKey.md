...
### Beispiel 1: Regenerates the Primary key using a IRegenerateKeyParameters hashtable
```powershell
...
Ungültiger vorheriger Primärschlüssel, neuer Schlüssel und Rückgabe.

### Beispiel 2: Regenerates the Secondary key using a KeyType
```powershell
...
Ungültig wird der vorherige sekundäre Schlüssel, neuer Schlüssel wird neu generiert und zurück.
...
Muss entweder "primär" oder "sekundär" (groß-/kleinschreibungsunempfindlich) sein.
...
### -Parameter
Parameter beschreiben die Anforderung zum Neu generieren von Zugriffstasten Zum Erstellen finden Sie im Abschnitt NOTIZEN die Parametereigenschaften und erstellen eine Hashtabelle.
...
PARAMETER <IRegenerateKeyParameters> : Parameter beschreiben die Anforderung zum Erneuten generieren von Zugriffstasten
  - `[KeyType <KeyType?>]`: Der keyType, der neu generiert werden soll. Muss entweder "primär" oder "sekundär" (groß-/kleinschreibungsunempfindlich) sein.
