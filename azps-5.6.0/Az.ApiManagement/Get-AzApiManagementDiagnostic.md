...
## SYNOPSIS
Hier erhalten Sie Details zur Diagnose, die auf Dienstebene oder Apiebene konfiguriert ist. Diagnosen werden zum Protokollieren von Anforderungen/Antworten vom Api-Verwaltungsgateway verwendet.
...
## BESCHREIBUNG
Die **Get-AzApiManagementDiagnostic** ruft Details der Diagnose ab, die im Api-Verwaltungsdienst in einem bestimmten Bereich konfiguriert wurde.

## BEISPIELE

### Beispiel 1: Alle Diagnosedaten erhalten, die im Mandantenbereich konfiguriert sind.
```powershell
...
Mit diesem Befehl werden alle Diagnosefunktionen im Api-Verwaltungsdienst konfiguriert.

### Beispiel 2: Abrufen aller diagnosen, die im Api-Bereich konfiguriert sind
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Dieser Befehl ruft alle Diagnosefunktionen ab, die im `echo-api` Api-Bereich konfiguriert sind.

### Beispiel 3: Abrufen der Api-Scope-Diagnose, die durch eine ID angegeben wird
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Wenn angegeben wird, wird die API-Bereichsdiagnose zurückgeben.
...
Wenn angegeben wird, gibt die Richtlinie für den Produktbereich zurück.
...
### -ResourceId
Arm Resource Identifier einer Diagnose- oder Apidiagnose. Wenn angegeben, wird versucht, die Diagnose durch den Bezeichner zu finden. Dieser Parameter ist erforderlich.
