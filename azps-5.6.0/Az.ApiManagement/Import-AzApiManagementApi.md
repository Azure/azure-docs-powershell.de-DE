...
## BESCHREIBUNG
Das **Cmdlet Import-AzApiManagementApi** importiert eine Azure API Management API aus einer Datei oder einer URL im Web Application Description Language (WADL), web Services Description Language (WSDL) oder im Swagger-Format.
...
Mit diesem Befehl wird eine API aus der angegebenen Swaggerdatei importiert.
...
Mit diesem Befehl wird eine API aus dem angegebenen Open 3.0-Spezifikationslink importiert.
...
### -ApiRevision
Bezeichner der API-Überarbeitung. Dieser Parameter ist optional. Wenn nicht angegeben, erfolgt der Import in die aktuell aktive Überarbeitung oder eine neue Api.
...
### -ApiType
Dieser Parameter ist optional mit dem Standardwert Http. Die Option Soap ist nur beim Importieren von WSDL anwendbar und erstellt eine SOAP-Passthrough-API.
...
Diese URL wird von API-Konsumenten zum Senden von Anforderungen an den Webdienst verwendet.
...
### -Protocol
Web-API-Protokolle (http, https). Protokolle, über die API verfügbar gemacht wird. Dieser Parameter ist optional. Sofern angegeben, werden die im Spezifikationsdokument angegebenen Protokolle außer Kraft setzen.
...
### -ServiceUrl
Eine URL des Webdiensts, der die API verfügbar machen soll. Diese URL wird nur von Azure API Management verwendet und nicht öffentlich gemacht. Dieser Parameter ist optional. Sofern angegeben, überschreibt es das im Dokument Spezifikationen angegebene ServiceUrl.
...
### -SpecificationPath
Gibt den Spezifikationsdateipfad an.
