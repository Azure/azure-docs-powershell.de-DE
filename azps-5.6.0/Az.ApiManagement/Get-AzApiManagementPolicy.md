...
## BESCHREIBUNG
Das **Get-AzApiManagementPolicy-Cmdlet** ruft die angegebene Bereichsrichtlinie ab.
...
Dieser Befehl ruft die Richtlinie auf Mandantenebene ab und speichert sie in einer Datei namens tenantpolicy.xml.

### Beispiel 2: Erhalten der Richtlinie für den Produktbereich
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
PS C:\>Get-AzApiManagementPolicy -Context $apimContext -ProductId "0123456789"
```

Dieser Befehl ruft eine Richtlinie für den Produktbereich ab.

### Beispiel 3: Abrufen der API-Scope-Richtlinie
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
PS C:\>Get-AzApiManagementPolicy -Context $apimContext -ApiId "9876543210"
```

Dieser Befehl ruft die API-Scope-Richtlinie ab.

### Beispiel 4: Erhalten der Richtlinie für den Vorgangsbereich
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
PS C:\>Get-AzApiManagementPolicy -Context $apimContext -ApiId "9876543210" -OperationId "777"
```

Dieser Befehl ruft die Richtlinie für den Vorgangsbereich ab.

### Beispiel 5: Herunterladen der Richtlinie für den Mandantenbereich im RawXml-Format
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Dieser Befehl ruft die Richtlinie für den Mandantenbereich im Nicht-Xml-Escapeformat ab.

## PARAMETER

### -ApiId
Gibt den Bezeichner der vorhandenen API an.
Wenn Sie diesen Parameter angeben, gibt das Cmdlet die API-Scope-Richtlinie zurück.
...
### -ApiRevision
Bezeichner der API-Überarbeitung. Dieser Parameter ist optional. Wenn nicht angegeben, wird die Richtlinie aus der aktuell aktiven Api-Überarbeitung abgerufen.
...
Wenn Sie diesen Parameter nicht angeben, wird das Ergebnis als Zierde pipelineiert.
