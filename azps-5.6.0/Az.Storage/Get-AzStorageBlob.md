...
## BESCHREIBUNG
Das **Get-AzStorageBlob-Cmdlet** listet Blobs im angegebenen Container in einem Azure-Speicherkonto auf.

## BEISPIELE

### Beispiel 1: Erhalten eines Blobs nach Blobname
```
PS C:\>Get-AzStorageBlob -Container "ContainerName" -Blob blob*
```

Dieser Befehl verwendet einen Blobnamen und ein Platzhalterzeichen, um einen Blob zu erhalten.
...
Dieser Befehl verwendet die Pipeline, um alle Blobs (blobs im Status Gelöscht) in einem Container zu erhalten.

### Beispiel 3: Erhalten von Blobs nach Namenspräfix
```
...
Der fünfte Befehl gibt eine **Do-While-Anweisung** an, die das **Get-AzStorageBlob-Cmdlet** verwendet, um Blobs zu erhalten.
...
Weitere Informationen finden Sie unter `Get-Help About_Do` .
Der letzte Befehl verwendet den **Befehl Echo,** um die Summe anzeigen zu können.

### Beispiel 5: Alle Blobs in einem Container erhalten enthalten blob-Version
```
...
Dieser Befehl ruft alle Blobs in einem Container mit Blobversion ab.

### Beispiel 6: Erhalten einer einzelnen Blobversion
```
...
### Beispiel 7: Erstellen einer einzelnen Blobmomentaufnahme
```
...
Wenn kein Blobname angegeben ist, werden im Cmdlet alle Blobs im angegebenen Container aufgeführt.
Wenn für diesen Parameter ein Wert angegeben ist, werden im Cmdlet alle Blobs mit Namen aufgeführt, die diesem Parameter entsprechen. Dieser Parameter unterstützt Platzhalter an einer beliebigen Stelle in der Zeichenfolge.
...
### -IncludeDeleted
"Gelöschten Blob enthalten", enthält "Get Blob" standardmäßig kein gelöschtes Blob.
...
### -IncludeVersion
Blobversionen werden nur aufgeführt, wenn dieser Parameter vorhanden ist, standardmäßig enthält get blob keine Blobversionen.
...
### -Prefix
Gibt ein Präfix für die Blobnamen an, die Sie erhalten möchten.
Dieser Parameter unterstützt keine Verwendung regulärer Ausdrücke oder Platzhalterzeichen für die Suche.
Dies bedeutet, dass das Cmdlet keine Blobs zurückgibt, wenn der Container nur Blobs mit den Namen "My", "MyBlob1" und "MyBlob2" enthält und Sie "-Prefix My*" angeben.
