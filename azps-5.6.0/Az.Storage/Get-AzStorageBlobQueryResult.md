...
## SYNOPSIS
Wendet eine einfache strukturierte Abfragesprache (SQL) auf den Inhalt eines Blobs an, und speichern Sie nur die abgefragte Teilmenge der Daten in einer lokalen Datei.
...
## BESCHREIBUNG
Das **Get-AzStorageBlobQueryResult-Cmdlet** wendet eine einfache strukturierte Abfragesprache (SQL)-Anweisung auf den Inhalt eines Blobs an, und speichern Sie die abgefragte Teilmenge der Daten in einer lokalen Datei.
...
Mit diesem Befehl wird ein Blob erfolgreich mit der Eingabekonfiguration als CSV und der Ausgabekonfiguration als json abgefragt, und die Ausgabe wird in der lokalen Datei "c:\resultfile.json" gespeichert.

### Beispiel 2: Abfragen einer Blobmomentaufnahme
```powershell
...
Dieser Befehl ruft zuerst ein Blobobjekt für die Blobmomentaufnahme ab, fragt dann die Blobmomentaufnahme ab und zeigt den Abfragefehler des Ergebnisses an.
...
### -Erzwingen
Erzwingen Sie, dass die vorhandene Datei überschrieben wird.
...
### -QueryString
Abfragezeichenfolge, weitere Details finden Sie unter: https://docs.microsoft.com/azure/storage/blobs/query-acceleration-sql-reference
