...
## BESCHREIBUNG
Das **Update-AzDataLakeGen2Item-Cmdlet** aktualisiert eine Datei oder ein Verzeichnis für Eigenschaften, Metadaten, Berechtigungen, ACL und Besitzer.
...
### Beispiel 1: Erstellen eines ACL-Objekts mit 3 ACL-Eintrag, und Aktualisieren der ACL auf alle Elemente in einem Dateisystem rekursiv
```
...
Mit diesem Befehl wird zuerst ein ACL-Objekt mit 3 acl-Eintrag erstellt (verwenden Sie den Parameter -InputObject, um dem vorhandenen acl-Objekt einen acl-Eintrag hinzuzufügen), dann alle Elemente in einem Dateisystem abzurufen und acl für die Elemente zu aktualisieren.
...
Mit diesem Befehl werden alle Eigenschaften einer Datei aktualisiert (ACL, Berechtigung, Besitzer, Gruppe, Metadaten, Eigenschaft kann mit beliebiger Konbination aktualisiert werden) und in der Powershell-Konsole angezeigt.
...
Dieser Befehl ruft ACL aus einem Verzeichnis ab, aktualisiert/fügt einen ACL-Eintrag hinzu und legt auf das Verzeichnis zurück.
Wenn der ACL-Eintrag mit demselben AccessControlType/EntityId/DefaultScope nicht vorhanden ist, wird ein neuer ACL-Eintrag hinzugefügt, sonst wird die Berechtigung für den vorhandenen ACL-Eintrag aktualisiert.
...
Kann eine Datei oder ein Verzeichnis im Format "verzeichnis/file.txt" oder "directory1/directory2/" sein.
Wenn Sie diesen Parameter nicht angeben, wird das Stammverzeichnis des Dateisystems aktualisiert.
...
### -Permission
Legt POSIX-Zugriffsberechtigungen für den Dateibesitzer, die Gruppe zum Besitzen von Dateien und andere Fest.
Jeder Klasse können Lese-, Schreib- oder Ausführungsberechtigungen erteilt werden.
Symbolische (rwxrw-rw-) wird unterstützt.
