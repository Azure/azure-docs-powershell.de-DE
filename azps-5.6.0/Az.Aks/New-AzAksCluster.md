...
### Neu ein AKS mit Standard-Params.

```powershell
PS C:\> New-AzAksCluster -ResourceGroupName myResourceGroup -Name myCluster
```

### Erstellen Sie den Windows Server-Container in einem AKS.
Wenn Sie einen Windows Server-Container in einem AKS erstellen möchten, müssen Sie beim Erstellen des AKS mindestens vier folgende Parameter angeben, und der Wert für und muss sein `NetworkPlugin` `NodeVmSetType` `azure` bzw. `VirtualMachineScaleSets` sein.
...
Add-On-Namen, die aktiviert werden sollen, wenn der Cluster erstellt wird.
...
### -DnsNamePrefix
Das Präfix für den DNS-Namen für den Cluster. Die Länge muss <= 9 sein, wenn Benutzer den Windows-Container hinzufügen möchten.
...
### -GenerateSshKey
Generieren Sie eine ssh-Schlüsseldatei an {HOME}/.ssh/id_rsa.
...
### -LoadBalancerSku
Die Load Balancer-Sku für den verwalteten Cluster.
...
### -Name
Verwalteter Clustername von Kubernetes.
...
ScaleSetEvictionPolicy zum Angeben der Räumungsrichtlinie für den Skalierungssatz für virtuelle Computer mit geringer Priorität. Standardmäßig löschen.
...
### -NodeSetPriority
ScaleSetPriority, das zum Angeben der Priorität des Skalierungssatzs für virtuelle Computer verwendet werden soll. Standardmäßig als "normal" festgelegt.
...
Die Client-ID und der Geheimtipp des Clients, die der AAD-Anwendung/dem Dienstprinzipal zugeordnet sind.
...
### -SshKeyValue
SSH-Schlüsseldateiwert oder Schlüsseldateipfad.
Standardmäßig ist {HOME}/.ssh/id_rsa.pub festgelegt.
...
### -Subnetzname
Subnetzname des VirtualNode-Addons.
...
Der Administratorname, der für Windows VMs verwendet werden soll.
...
Das administrator password to use for Windows VMs, its length must be least 12, containing least one lower case character, d. h. `[a-z]` , one and one special `[A-Z]` `[!@#$%^&*()]` character.
