...
### Beispiel 1: Beschreibung der Integrations-Laufzeit aktualisieren.
```
...
### Beispiel 2: Selbst gehostete Integrationslaufzeit freigeben.
```
PS C:\> Set-AzDataFactoryV2IntegrationRuntime -ResourceGroupName 'rg-test-dfv2' -DataFactoryName 'test-df-eu2' -Name 'test-selfhost-ir' `
...
Das Cmdlet fügt den ADF hinzu, um die freigegebene Integrationslaufzeit zu verwenden. Bei Verwendung `-SharedIntegrationRuntimeResourceId` des -Parameters `-Type` muss auch der enthalten sein. Beachten Sie, dass der Daten factory die Berechtigung zur Verwendung der Integrations-Runtime erteilt werden muss, bevor das Cmdlet ausgeführt wird.

### Beispiel 3: Konfigurieren Self-Hosted IR als Proxy für Azure-SSIS IR in ADF.
```
...
Das Cmdlet aktualisiert die Azure-SSIS-Integrations-Runtime, um die Selbst gehostete Integrations-Runtime als Datenproxy zu verwenden.
...
### -LicenseType
Der Lizenztyp, den Sie für die SSIS IR auswählen möchten. Es gibt zwei Typen: LicenseIncluded oder BasePrice. Wenn Sie für die Preise für Azure Hybrid Use Benefit (AHUB) qualifiziert sind, wählen Sie bitte Basispreis aus. Wenn nicht, wählen Sie LicenseIncluded aus.
...
### -Location
Der Speicherort für die Integrationslaufzeit.
...
### -VNetId
Die ID des VNets, dem die Integrations-Runtime beitritt.
