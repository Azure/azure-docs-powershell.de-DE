...
**HINWEIS:** Derzeit `New-AzDedicatedHsm` gibt es eine Einschränkung, die zurückgegeben wird, bevor das HSM vollständig in Azure bereitgestellt wird.
Fragen Sie daher nach dem Erstellen eines neuen HSM den Zustand nach ab, und stellen Sie sicher, dass er `Get-AzDedicatedHsm` `Provisioning State` vor der Verwendung `Succeeded` ist.
...
### -NetworkInterface
Gibt die Liste der Ressourcen-ID für die Netzwerkschnittstellen an, die dem dedizierten HSM zugeordnet sind.
Informationen zum Erstellen finden Sie im Abschnitt NOTIZEN zu DEN NETZWERKINTERFACE-Eigenschaften, und erstellen Sie eine Hashtabelle.
...
### -StampId
Dieses Feld wird verwendet, wenn RP Verfügbarkeitszonen nicht unterstützt.
