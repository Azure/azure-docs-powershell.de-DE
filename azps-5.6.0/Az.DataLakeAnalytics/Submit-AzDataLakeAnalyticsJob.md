...
U-SQL-Skriptparameter werden über dem Hauptskriptinhalt vorab festgelegt, z. B.: DECLARE @Department string = "Sales"; DECLARE @NumRecords int = 1000; DECLARE @StartDateTime DateTime = new DateTime(2017, 12, 6, 0, 0, 0, 0, 0);
...
### -AnalyticsUnits
Die analyseeinheiten, die für diesen Auftrag verwendet werden. In der Regel führen mehr Analyseeinheiten, die einem Skript gewidmet sind, zu einer schnelleren Ausführungszeit von Skripts.
...
### -CompileMode
Die Art der Kompilierung, die für diesen Auftrag zu erledigen ist. Gültige Werte: 
- Semantisch (führt nur semantische Überprüfungen und erforderliche Sanitätsprüfungen durch)
...
### -CompileOnly
Gibt an, dass die Übermittlung nur den Auftrag erstellen und nicht ausführen sollte, wenn auf "true" festgelegt ist.
...
### -PipelineId
Eine ID, die die Übermittlung dieses Auftrags angibt, ist Teil einer Reihe von wiederkehrenden Aufträgen und auch mit einer Auftragspipeline verknüpft.
...
### -PipelineName
Optionaler Anzeigename für die Pipeline, die diesem Auftrag zugeordnet ist.
...
### -PipelineUri
Ein optionaler URI, der mit dem ursprünglichen Dienst verknüpft ist, der dieser Pipeline zugeordnet ist.
...
### -Priorität
Die Priorität des Auftrags. Wenn nicht angegeben, ist die Priorität 1000. Eine niedrigere Zahl gibt eine höhere Auftragspriorität an.
...
### -Runtime
Legen Sie optional die Version der Laufzeit für den Auftrag ein. Wenn die Standardlaufzeit nicht festgelegt ist, wird die Standardlaufzeit verwendet.
...
### -ScriptParameter
Die Skriptparameter für diesen Auftrag als Wörterbuch mit Parameternamen (Zeichenfolge) zu Werten (beliebige Kombinationen aus Byte, Sbyte, Int, uint (oder uint32), long, ulong (oder uint64), float, double, decimal, short (oder int16), ushort (oder uint16), char, string, DateTime, bool, Guid oder byte[]).
...
### -ScriptPath
Pfad zur zu übermittelnde Skriptdatei.
