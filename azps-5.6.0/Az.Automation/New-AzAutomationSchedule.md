...
### Beispiel 1: Erstellen eines Einmalzeitplans in Ortszeit
```
...
Der zweite Befehl erstellt einen Zeitplan, der einmal am aktuellen Datum um 23:00 Uhr in der angegebenen Zeitzone ausgeführt wird.
...
Der erste Befehl erstellt ein Datumsobjekt mithilfe des **Get-Date-Cmdlets** und speichert das Objekt dann in $StartDate Variablen.
Geben Sie eine Uhrzeit an, die in Zukunft mindestens fünf Minuten beträgt.
Der zweite Befehl erstellt ein Datumsobjekt mithilfe des **Get-Date-Cmdlets** und speichert das Objekt dann in $EndDate Variablen.
Der Befehl gibt eine zukünftige Uhrzeit an.
Der letzte Befehl erstellt einen täglichen Zeitplan mit dem Namen "Schedule02", der zu der in $StartDate gespeicherten Uhrzeit beginnt und zu dem Zeitpunkt abläuft, der in $EndDate.
...
Der letzte Befehl erstellt einen täglichen Zeitplan namens Schedule03, der von Montag bis Freitag jede Woche um 13:00 Uhr ausgeführt wird. Der Zeitplan läuft nie ab.
...
Wenn Sie diesen Parameter nicht angeben und den *Parameter "OneTime"* nicht angeben, ist der Standardwert eins (1).
...
Sie können eine Zeichenfolge angeben, die in eine gültige **DateTimeOffset konvertiert werden kann.**
Wenn der *Parameter TimeZone* angegeben ist, wird der Offset ignoriert, und die angegebene Zeitzone wird verwendet.
