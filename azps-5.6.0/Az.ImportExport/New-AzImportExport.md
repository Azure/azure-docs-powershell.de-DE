...
Diese Cmdlets erstellen einen neuen ImportExportauftrag.
...
### -CancelRequested
Gibt an, ob eine Anforderung zum Kündigen des Auftrags übermittelt wurde.
...
### -ClientTenantId
Die Mandanten-ID des Clients, der die Anforderung vor sich hat.
...
### -DiagnosticsPath
Das virtuelle Blobverzeichnis, in dem die Kopie Protokolle und Sicherungen von Laufwerkmanifestdateien (sofern aktiviert) gespeichert wird.
...
### -DriveList
Liste von bis zu zehn Laufwerken, die den Auftrag umfassen.
Die Laufwerkliste ist ein erforderliches Element für einen Importauftrag. es wird nicht für Exportaufträge angegeben.
...
Der relative URI zum Blockblob, der die Liste der Blobpfade oder Blobpfadpräfixe enthält, wie oben definiert, beginnend mit dem Containernamen.
Wenn sich das Blob im Stammcontainer befindet, muss der URI mit der $root.
...
Ein Blobpfad, der auf einen Blockblob verweist, der eine Liste der Blobnamen enthält, die aufgrund des unzureichenden Laufwerkspeicherplatzes nicht exportiert wurden.
Wenn alle Blobs erfolgreich exportiert wurden, ist dieses Element nicht in der Antwort enthalten.
...
### -LogLevel
Standardwert ist Fehler.
Gibt an, ob die Fehlerprotokollierung oder ausführliche Protokollierung aktiviert wird.
...
### -PercentComplete
Gesamter Prozentsatz, der für den Auftrag abgeschlossen wurde.
...
DRIVELIST <IDriveStatus[]>: Liste von bis zu zehn Laufwerken, die den Auftrag umfassen. Die Laufwerkliste ist ein erforderliches Element für einen Importauftrag. es wird nicht für Exportaufträge angegeben.
