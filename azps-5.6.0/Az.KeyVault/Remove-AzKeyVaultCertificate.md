...
Mit diesem Befehl wird das Zertifikat "SelfSigned01" aus dem Schlüsseltresor namens ContosoKV01 entfernt.
Dieser Befehl gibt den Parameter *Kraft* an.
Daher werden Sie vom Cmdlet nicht zur Bestätigung aufgefordert.

### Beispiel 2: Endgültiges Löschen des gelöschten Zertifikats aus dem Schlüsseltresor
```powershell
PS C:\> Remove-AzKeyVaultCertificate -VaultName 'Contoso' -Name 'MyCert' -InRemovedState
```

Mit diesem Befehl wird das Zertifikat mit dem Namen "MyCert" endgültig aus dem Schlüsseltresor namens "Contoso" entfernt.
Für die Ausführung dieses Cmdlets ist die Berechtigung "Löschen" erforderlich, die zuvor und explizit dem Benutzer in diesem Schlüsseltresor erteilt worden sein muss.
...
### -InRemovedState
Wenn vorhanden, entfernt das zuvor gelöschte Zertifikat dauerhaft
...
Dieses Cmdlet erstellt den vollqualifizierten Domänennamen (Fully Qualified Domain Name, FQDN) eines Zertifikats basierend auf dem Namen, den dieser Parameter angibt, dem Namen des Schlüsseltresor und Ihrer aktuellen Umgebung.
...
### -VaultName
Gibt den Namen des Schlüsseltresor an, aus dem dieses Cmdlet ein Zertifikat entfernt.
...
[Rückgängig-AzKeyVaultCertificateRemoval](./Undo-AzKeyVaultCertificateRemoval.md)
