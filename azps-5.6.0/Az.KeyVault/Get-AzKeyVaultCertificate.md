...
## BESCHREIBUNG
Das **Get-AzKeyVaultCertificate-Cmdlet** ruft das angegebene Zertifikat oder die Versionen eines Zertifikats aus einem Schlüsseltresor im Azure Key Vault ab.
...
### Beispiel 2: Cert erhalten und als pfx speichern
Dieser Befehl ruft das Zertifikat mit dem Namen TestCert01 aus dem Schlüsseltresor namens ContosoKV01 ab. Führen Sie den folgenden Befehl aus, um das Zertifikat als pfx-Datei herunterzuladen. Diese Befehle greifen auf SecretId zu und speichern den Inhalt dann als pfx-Datei.
...
### Beispiel 3: Alle Zertifikate, die für diesen Schlüsseltresor gelöscht, aber nicht gelöscht wurden.
```powershell
...
### Beispiel 4: Ruft das Zertifikat MyCert ab, das für diesen Schlüsseltresor gelöscht, aber nicht gelöscht wurde.
```powershell
...
Dieser Befehl ruft das Zertifikat mit dem Namen "MyCert" ab, das zuvor gelöscht, aber nicht gelöscht wurde, im Schlüsseltresor contoso.
Dieser Befehl gibt Metadaten zurück, z. B. das Löschdatum und das geplante Löschdatum dieses gelöschten Zertifikats.
...
### -IncludeVersions
Gibt an, dass bei diesem Vorgang alle Versionen des Zertifikats angezeigt werden.
...
### -InRemovedState
Gibt an, ob zuvor gelöschte Zertifikate in die Ausgabe
...
### -Name
Gibt den Namen des zertifikats an, das sie erhalten soll.
...
[Rückgängig-AzKeyVaultSecretCertificate](./Undo-AzKeyVaultSecretCertificate.md)
