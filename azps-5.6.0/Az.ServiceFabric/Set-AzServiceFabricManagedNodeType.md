...
## BESCHREIBUNG
Legt Ressourceneigenschaften des Knotentyps fest, oder führen Sie Reimageaktionen für bestimmte Ndes des Knotentyps mit dem Parameter -Reimage aus. Beim Neuimgae-Vorgang werden die Service Fabric-Knoten deaktiviert, bevor sie die virtuellen Computer erneut vorverladen und wieder aktiviert werden, sobald sie wiederkommen. Wenn dies für primäre Knotentypen erfolgt, kann es eine Weile dauern, da möglicherweise nicht alle Knoten gleichzeitig neu nachbilden. Verwenden Sie -ForceReimage erzwingen den Vorgang, auch wenn service fabric die Knoten nicht deaktivieren kann, aber mit Bedacht vorgehen kann, da dies zu Datenverlust führen kann, wenn auf dem Knoten zustandsreiche Workloads ausgeführt werden.
...
Aktualisieren Sie die Instanzanzahl des Knotentyps.
...
Aktualisieren sie die Platzierungswebsites des Knotentyps. Dadurch werden ältere Platzierungs-Properites überschrieben, sofern vorhanden.
...
Den Knoten 0 und 3 für den Knotentyp neu abbilden.
...
Aktualisieren Sie die Instanzanzahl des Knotentyps mit Piping.
...
### -Kapazität
Kapazitätstags, die als Schlüssel-Wert-Paare auf die Knoten im Knotentyp angewendet werden, verwendet der Clusterressourcen-Manager diese Tags, um zu verstehen, wie viel Ressource ein Knoten hat. Wenn Sie dies aktualisieren, werden die aktuellen Werte außer Kraft setzen.
...
### -EphemeralStartPort
Kurzlebiger Startport einer Reihe von Ports.
...
Verwenden Sie mit Bedacht, da dies zu Datenverlust führen kann, wenn auf dem Knoten zustandsreiche Arbeitsauslastungen ausgeführt werden.
...
### -PlacementProperty
Platzierungstags, die auf Knoten im Knotentyp als Schlüssel-Wert-Paare angewendet werden, können verwendet werden, um anzugeben, wo bestimmte Dienste (Workload) ausgeführt werden sollen. Wenn Sie dies aktualisieren, werden die aktuellen Werte außer Kraft setzen.
...
### -Reimage
Geben Sie an, dass Knoten für den Knotentyp neu erstellt werden.
