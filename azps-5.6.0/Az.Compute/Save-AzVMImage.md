...
Bevor Sie ein Abbild eines virtuellen Computers erstellen, sysprepieren Sie den virtuellen Computer, und markieren Sie ihn dann mit dem cmdlet Set-AzVM als generalisiert.
Die Ausgabe dieses Cmdlets ist eine JavaScript Object Notation (JSON)-Vorlage.
Sie können virtuelle Computer aus ihrem aufgenommenen Bild bereitstellen.
...
Die **Ausgabeeigenschaft** gibt eine JSON-Vorlage zurück.
...
Gibt den Namen eines Containers innerhalb des "Systemcontainers" an, in dem Die Bilder enthalten sein möchten.
Wenn der Container nicht vorhanden ist, wird er für Sie erstellt.
Die virtuellen Festplatten (VHDs), die das VMImage bilden, befinden sich in dem Container, den dieser Parameter angibt.
...
Die URL des gespeicherten Bilds ist ähnlich wie: https:// \<storageAccountName\> .blob.core.windows.net/system/Microsoft.Compute/Images/ \<imagesContainer\> / \<vhdPrefix-osDisk\> .xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx.vhd.
...
### -Overwrite
Gibt an, dass mit diesem Cmdlet alle VHDs überschrieben werden, die das gleiche Präfix im Zielcontainer haben.
...
### -VHDNamePrefix
Gibt das Präfix im Namen der Blobs an, die das Speicherprofil der VMImage bilden.
Ein Präfix vhdPrefix für einen Betriebssystemdatenträger führt z. B. zu dem Namen vhdPrefix-osdisk. \<guid\> vhd.
