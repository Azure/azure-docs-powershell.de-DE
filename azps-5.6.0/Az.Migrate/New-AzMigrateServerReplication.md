...
## SYNOPSIS
Startet die Replikation für den angegebenen Server.
...
### Beispiel 1: Wenn nur ein Betriebssystemdatenträger vorhanden ist
```powershell
...
Dies gilt für das Szenario, in dem nur ein einziger Datenträger geschützt werden muss.

### Beispiel 2: Wenn mehrere Datenträger vorhanden sind
```powershell
...
Dies gilt für das Szenario, in dem mehrere Datenträger geschützt werden müssen.
...
### -DiskEncryptionSetID
Gibt den datenträgerencyption-Satz an, der verwendet werden soll.
...
### -DiskToInclude
Gibt die Datenträger auf dem Quellserver an, die für die Replikation einbezogen werden sollen.
Informationen zum Erstellen finden Sie im Abschnitt NOTIZEN für DISKTOINCLUDE-Eigenschaften, und erstellen Sie eine Hashtabelle.
...
### -DiskType
Gibt den Typ der Datenträger an, die für die Azure VM verwendet werden sollen.
...
### -InputObject
Gibt den erkannten Server an, der migriert werden soll.
Das Serverobjekt kann mithilfe des cmdlets Get-AzMigrateServer abgerufen werden.
...
### -LicenseType
Gibt an, ob der Azure Hybrid-Vorteil für die Migration des Quellservers gilt.
...
### -MachineId
Gibt die Computer-ID des erkannten Servers an, der migriert werden soll.
...
### -OSDiskID
Gibt den Betriebssystemdatenträger für den Quellserver an, der migriert werden soll.
...
### -PerformAutoResync
Gibt an, ob die Replikation automatisch repariert wird, falls die Änderungsnachverfolgung für den Quellserver bei der Replikation verloren geht.
...
Gibt den Verfügbarkeitssatz an, der für die Vm-Erstellung verwendet werden sollSpecififs the Availability Set to be used for VM creation.
...
### -TargetNetworkId
Gibt die Virtuelle Netzwerk-ID innerhalb des Azure-Zielabonnements an, zu dem der Server migriert werden muss.
...
Gibt die Ressourcengruppen-ID innerhalb des Azure-Zielabonnements an, zu dem der Server migriert werden muss.
...
### -TargetSubnetName
Gibt den Subnetznamen innerhalb des Ziels Virtual Netowk an, zu dem der Server migriert werden muss.
...
### -TargetVMName
Gibt den Namen der zu erstellenden Azure-VM an.
...
### -TargetVMSize
Gibt die SKU der zu erstellenden Azure VM an.
...
DISKTOINCLUDE <IVMwareCbtDiskInput[]>: Gibt die Datenträger auf dem Quellserver an, die für die Replikation einbezogen werden sollen.
  - `DiskId <String>`: Die Datenträger-ID.
  - `IsOSDisk <String>`: Ein Wert, der angibt, ob es sich bei dem Datenträger um den Betriebssystemdatenträger handelt.
  - `LogStorageAccountId <String>`: Das Protokollspeicherkonto ARM ID.
  - `LogStorageAccountSasSecretName <String>`: Der geheime Schlüsselname des Protokollspeicherkontos.
  - `[DiskEncryptionSetId <String>]`: Die DiskEncryptionSet-ARM-ID.
  - `[DiskType <DiskAccountType?>]`: Der Datenträgertyp.

INPUTOBJECT <IVMwareMachine> : Gibt den erkannten Server an, der migriert werden soll. Das Serverobjekt kann mithilfe des cmdlets Get-AzMigrateServer abgerufen werden.
  - `[GuestOSDetailOstype <String>]`: Typ des Betriebssystems.
