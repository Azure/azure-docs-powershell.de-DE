...
### Beispiel 2: Aktivieren der Verschlüsselung in einem Databricks-Arbeitsbereich
```powershell
...
Zum Aktivieren der Verschlüsselung in einem Databricks-Arbeitsbereich sind drei Schritte erforderlich:
1.
Aktualisieren Sie den Arbeitsbereich `-PrepareEncryption` mit (wenn er nicht erstellt wurde).
1.
Suchen `StorageAccountIdentityPrincipalId` Sie in der Ausgabe des letzten Schritts.
Erteilen Sie dem Prinzipal schlüsselberechtigungen.
1.
Aktualisieren Sie den Arbeitsbereich erneut, um Informationen zum Verschlüsselungsschlüssel einfüllen zu können:
    - `-EncryptionKeySource`
    - `-EncryptionKeyVaultUri`
    - `-EncryptionKeyName`
    - `-EncryptionKeyVersion`

### Beispiel 3: Deaktivieren der Verschlüsselung in einem Databricks-Arbeitsbereich
```powershell
...
Um die Verschlüsselung zu deaktivieren, legen Sie einfach `-EncryptionKeySource` auf . `'Default'`
...
### -EncryptionKeySource
Der VerschlüsselungsschlüsselSource (Anbieter).
Mögliche Werte (Groß-/Kleinschreibung): Standard, Microsoft.Keyvault
