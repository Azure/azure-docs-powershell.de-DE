...
## Beschreibung
Das Get-AzNetworkWatcherFlowLogStatus-Cmdlet ruft den Status der Fluss Protokollierung für eine Ressource ab. Der Status umfasst, ob die Fluss Protokollierung für die bereitgestellte Ressource aktiviert ist, das konfigurierte Speicherkonto zum Senden von Protokollen und die Aufbewahrungsrichtlinie für die Protokolle. Zurzeit werden Netzwerk Sicherheitsgruppen für die Fluss Protokollierung unterstützt. 

## Beispiele

### Beispiel 1: Abrufen des Fluss Protokollierungs Status für eine angegebene NSG
```
...
In diesem Beispiel wird der Fluss Protokollierungsstatus für eine Netzwerksicherheitsgruppe abgerufen. Bei der angegebenen NSG ist die Fluss Protokollierung aktiviert, das Standardformat und keine Aufbewahrungsrichtlinie festgelegt.

### Beispiel 2: Abrufen der Fluss Protokollierung und des Datenverkehrsanalyse Status für eine angegebene NSG
```
PS C:\> $NW = Get-AzNetworkWatcher -ResourceGroupName NetworkWatcherRg -Name NetworkWatcher_westcentralus
PS C:\> $nsg = Get-AzNetworkSecurityGroup -ResourceGroupName NSGRG -Name appNSG
...
In diesem Beispiel wird der Fluss Protokollierungs-und Datenverkehrsanalyse Status für eine Netzwerksicherheitsgruppe abgerufen. In der angegebenen NSG sind Fluss Protokollierung und Datenverkehrsanalyse aktiviert, Standardformat und keine Aufbewahrungsrichtlinie festgelegt.
