...
## Beschreibung
Mit den Cmdlets **New-AzContainerGroup** wird eine Containergruppe erstellt.
...
Mit diesen Befehlen wird eine Containergruppe mit dem neuesten nginx-Bild erstellt und eine öffentliche IP-Adresse mit dem Öffnen von Port 8000 angefordert.
...
Mit diesen Befehlen wird eine Containergruppe erstellt und ein benutzerdefiniertes Skript im Container ausgeführt.

### Beispiel 3: erstellt eine Containergruppe für die Fertigstellung.
```
...
Mit diesen Befehlen wird eine Containergruppe erstellt, die "Hallo" ausgibt und angehalten wird.

### Beispiel 4: Erstellen einer Containergruppe mithilfe von Image in der Azure-Container Registrierung
```
...
Mit diesen Befehlen wird eine Containergruppe mit einem nginx-Bild in der Azure-Container Registrierung erstellt.

### Beispiel 5: Erstellen einer Containergruppe mithilfe von Bild in der benutzerdefinierten Container Bildregistrierung
```
PS C:\> $secpasswd = ConvertTo-SecureString "PlainTextPassword" -AsPlainText -Force
...
Mit diesen Befehlen wird eine Containergruppe mithilfe eines benutzerdefinierten Bilds aus einer benutzerdefinierten Container Bildregistrierung erstellt.

### Beispiel 6: erstellt eine Containergruppe, die das Azure-Datei Volumen einhängt
```
PS C:\> $secpasswd = ConvertTo-SecureString "PlainTextPassword" -AsPlainText -Force
PS C:\> $mycred = New-Object System.Management.Automation.PSCredential ("username", $secpasswd)
...
Mit diesen Befehlen wird eine Containergruppe erstellt, in der die bereitgestellte Azure-Dateifreigabe bereitgestellt wird `/mnt/azfile` .
...
Mit diesen Befehlen wird eine Containergruppe mit dem System zugewiesene Identität mit dem neuesten nginx-Bild erstellt, und es wird eine öffentliche IP-Adresse mit dem Öffnen von Port 8000 angefordert.
...
Mit diesen Befehlen wird eine Containergruppe mit dem System zugewiesen und der Benutzer zugewiesene Identität mithilfe des neuesten nginx-Bilds erstellt, und es wird eine öffentliche IP-Adresse mit dem Öffnen von Port 8000 angefordert.
...
Die Speicherkonto Anmeldeinformationen der Azure-Dateifreigabe für die Bereitstellung, wobei der Benutzername der Name des speicherkontos ist und der Schlüssel der speicherkontoschlüssel ist.
...
Der Bereitstellungspfad für das Azure-Datei Volume.
...
Der Name der Azure-Dateifreigabe, die bereitgestellt werden soll.
...
### -Identitäts Kennung
Der Benutzer hat Identitäts-IDs zugewiesen
...
### -IdentityType
Der Typ der verwalteten Identität
...
### -Standort
Der Speicherort der Containergruppe.
...
### -OsType
Der Typ des Container-Betriebssystems.
Standard: Linux
...
### -Port
Der zu öffnende Port (s). Standard: [80]
...
### -RegistryCredential
Die Anmeldeinformationen für benutzerdefinierte Container-Registrierung.
...
Der benutzerdefinierte Container Registrierungs Anmeldeserver.
...
### -RestartPolicy
Die Richtlinie für den Neustart des Containers. Standard: immer
