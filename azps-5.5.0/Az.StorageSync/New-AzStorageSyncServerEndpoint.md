...
## BESCHREIBUNG
Mit diesem Befehl wird ein neuer Serverendpunkt auf einem registrierten Server erstellt. Dadurch kann der angegebene Pfad auf dem Server die Synchronisierung der Dateien mit anderen Endpunkten in der Synchronisierungsgruppe starten. Wenn es bereits Dateien auf anderen Endpunkten in der Synchronisierungsgruppe gibt und dieser neu hinzugefügte Speicherort auch Dateien enthält, versucht ein Abstimmungsprozess festzustellen, ob sich die Dateien tatsächlich in denselben Ordnern wie auf anderen Endpunkten befinden. Die Namespaces werden zusammengeführt, und die Abstimmung hilft, Konflikte in Dateien zu vermeiden. Wenn Dateien auf anderen Serverendpunkten vorhanden sind, ist es häufig besser, mit einem leeren Speicherort auf diesem Server zu beginnen, damit die Dateien aus der Cloud in einem automatischen Prozess, der als schnelle Notfallwiederherstellung bezeichnet wird, an den Server geleitet werden. Namespacemetadaten werden zuerst synchronisiert, dann wird der Datenstrom jeder Datei heruntergeladen. Wenn eine Datei von einem Benutzer oder einer Anwendung nicht in der Downloadreihenfolge angefordert wird, wird diese Datei mit Priorität zurückgerufen, um die Zugriffsanforderung zu erfüllen. Optional können Sie die Cloud tiering auf diesem Serverendpunkt verwenden, um festzustellen, ob dieser Endpunkt ein Cache der gesamten Gruppe von Dateien aus der Cloud werden soll. Wenn Cloud tiering verwendet wird, wird der Download von Dateiinhalten an dem Punkt beendet, der durch die Cloud tiering policies, die Sie festlegen können, definiert ist.
...
Mit diesem Befehl wird ein neuer Serverendpunkt auf einem registrierten Server erstellt und in eine Synchronisierungsgruppe eingefügt. Auf diese Weise ist es Teil einer Topologie anderer Endpunkte. Dateimetadaten und -inhalte werden sofort mit der Synchronisierung aller Speicherorte gestartet, auf die als Endpunkte in der Synchronisierungsgruppe verwiesen wird.
...
### -OfflineDataTransfer
Parameter für Daten mit Cloud-Startwert
...
### -ServerLocalPath
Parameter für lokalen Serverpfad
...
### -TierFilesOlderDays
Parameter 'Dateien stufen, die älter als Tage' sind
...
Parameter "Volume Free Space Percent"
