...
Maximale Wartezeit beim Erstellen der Bildvorlage.
Lassen Sie "0" weg, oder geben Sie "0" an, um den Standardwert (4 Stunden) zu verwenden.
...
### -Customize
Gibt die Eigenschaften an, die verwendet werden, um die Anpassungsschritte des Bilds zu beschreiben, z. B. Bildquelle usw. Informationen zum Erstellen finden Sie im Abschnitt NOTES zu #A0 und zum Erstellen einer Hashtabelle.
...
### -DefaultProfile
Region HideParameter Die Anmeldeinformationen, das Konto, den Mandanten und das Abonnement, die für die Kommunikation mit Azure verwendet werden.
...
### -Distribute
Die Verteilung zielt ab, an die die Bildausgabe gehen muss.
Informationen zum Erstellen finden Sie im Abschnitt "NOTES" für #A0 und zum Erstellen einer Hashtabelle.
...
Startzeit des letzten Durchlaufs (UTC).
...
Die ID der vom Benutzer zugewiesenen Identität.
...
Geben Sie "0" aus, um die standardmäßige Datenträgergröße des Betriebssystems Azure zu verwenden.
...
### -VMProfileVmSize
Die Größe des virtuellen Computers, der zum Erstellen, Anpassen und Erfassen von Bildern verwendet wird.
Lassen Sie eine leere Zeichenfolge weg, oder geben Sie sie an, um die Standardzeichenfolge (Standard_D1_v2) zu verwenden.
...
CUSTOMIZE <IImageTemplateCustomizer[]>: Gibt die Eigenschaften an, die verwendet werden, um die Anpassungsschritte des Bilds zu beschreiben, z. B. Bildquelle usw.
  - `Type <String>`: Der Typ des Anpassungstools, den Sie für das Bild verwenden möchten. "Shell" kann z. B. ein Shell-Customizer sein.
  - `[Name <String>]`: Anzeigename, der Kontext zu den Durchschritten dieses Anpassungsschritts bietet

DISTRIBUTE <IImageTemplateDistributor[]>: Die Verteilungsziele, an die die Bildausgabe übergeben werden muss.
  - `RunOutputName <String>`: Der Name, der für den zugeordneten RunOutput verwendet werden soll.
  - `Type <String>`: Verteilungsart
  - `[ArtifactTag <IImageTemplateDistributorArtifactTags>]`: Tags, die auf das Artefakt angewendet werden, nachdem es vom Distributor erstellt/aktualisiert wurde.
    - `[(Any) <String>]`: Dies gibt an, dass diesem Objekt beliebige Eigenschaften hinzugefügt werden können.

SOURCE : Beschreibt eine Bildquelle für einen virtuellen Computer zum <IImageTemplateSource> Erstellen, Anpassen und Verteilen.
