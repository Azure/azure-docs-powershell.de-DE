...
Verwenden Sie dieses Cmdlet, um die Replikation für ein asr-fähiges Element zu aktivieren.
...
Startet den replikationsgeschützten Elementerstellungsvorgang für das angegebene asr-geschützte Element und gibt den ASR-Auftrag zurück, mit dem der Vorgang nachverfolgt wird.
...
Startet den Vorgang zum Erstellen von replikationsgeschützten Element für das angegebene asR-geschützte Element und gibt den ASR-Auftrag zurück, mit dem der Vorgang (vmWare zu Azure-Szenario) nachverfolgt wird.
...
Startet den Vorgang zum Erstellen eines replikationsgeschützten Elements für das angegebene asR-geschützte Element und gibt den ASR-Auftrag zurück, mit dem der Vorgang nachverfolgt wird (Azure-zu-Azure-Szenario).
...
Startet den Vorgang zum Erstellen von replikationsgeschützten Element für das angegebene asR-geschützte Element, einschließlich selektiver Datenträger, und gibt den ASR-Auftrag zurück, mit dem der Vorgang (vmWare zu Azure-Szenario) mit ausgewählten Datenträgern nachverfolgt wird.
...
Startet den replikationsgeschützten Elementerstellungsvorgang für die angegebene VmId und gibt den ASR-Auftrag zurück, der zum Nachverfolgen des Vorgangs verwendet wird (Azure-zu-Azure-Szenario). Für die Failover-VM-Details, die zum Verschlüsseln im Cmdlet übergeben werden, werden verwendet.
...
Startet den replikationsgeschützten Elementerstellungsvorgang für einen virtuellen Computer in der Näherungsplatzierungsgruppe und gibt den ASR-Auftrag zurück, mit dem der Vorgang nachverfolgt wird (Azure-zu-Azure-Szenario).

## PARAMETERS

### -Konto
Das Konto "Ausführen als", das verwendet werden soll, um die Installation des Diensts Mobility bei Bedarf zu pushen. Must be one from the list of run as accounts in the ASR fabric.
...
### -AzureToAzure
Der Parameter "Switch" gibt an, dass das replizierte Element in Azure zu Azure erstellt wird.
...
Gibt die Datenträgerkonfiguration an, die für das Notfallwiederherstellungsszenario "Vm für Azure zu Azure" verwendet wird.
...
### -DiskEncryptionSetId
Gibt die Ressourcen-ID des Datenträgerverschlüsselungssets an, der für die Verschlüsselung der verwalteten Datenträger verwendet werden soll.
...
Gibt die geheime Speichertresor-ID (Azure-Datenträgerverschlüsselung) an, die nach dem Failover als Wiederherstellungs-VM verwendet werden soll.
...
### -HyperVToAzure
Parameter wechseln, um anzugeben, dass das replizierte Element ein virtueller Hyper-V-Computer ist, der in Azure repliziert wird.
...
Gibt die Datenträgerkonfigurationseingabe für vMWare-Datenträger-ID an, um vor bestimmten schützenden Elemente zu schützen.
...
### -LogStorageAccountId
Gibt die Protokoll- oder Cachespeicherkonto-ID an, die zum Speichern von Replikationsprotokollen verwendet werden soll.
...
### -Name
Gibt einen Namen für das asr-replikationsgeschützte Element an. Der Name muss innerhalb des Tresors eindeutig sein.
...
### -ProcessServer
Der Prozessserver, der zum Replizieren dieses Computers verwendet wird. Use the list of process servers in the ASR fabric corresponding to the Configuration server to specify one.
...
### -ProtectableItem
Gibt das schützende ASR-Elementobjekt an, für das Replikation aktiviert wird.
...
Gibt das ASR-Schutzcontainerzuordnungsobjekt an, das der für die Replikation zu verwendenden Replikationsrichtlinie entspricht.
...
Die ID des AvailabilitySets, auf das der Computer im Fall eines Failovers wiederhergestellt werden soll.
...
Die ID des virtuellen Azure-Netzwerks, auf das der Computer im Fall eines Failovers wiederhergestellt werden soll.
...
Das Subnetz innerhalb des wiederherstellungs virtuellen Azure-Netzwerks, an das der Fehler über den virtuellen Computer im Falle eines Failovers angefügt werden sollte.
...
Gibt die Ressourcen-ID des Wiederherstellungs-Clouddiensts an, zu dem dieser virtuelle Computer über einen Failover führen soll.
...
Geben Sie die id der Näherungsplatzierungsgruppe an, die von der Failover-VM im Zielwiederherstellungsbereich verwendet werden soll.
...
Gibt den ARM der Ressourcengruppe an, in der der virtuelle Computer im Fall eines Failovers erstellt wird.
...
Gibt den Namen der Replikationsgruppe an, mit dem einheitliche Wiederherstellungspunkte für mehrere VM erstellt werden. Standardmäßig wird jedes replikationsgeschützte Element in einer eigenen Gruppe erstellt, und konsistente Wiederherstellungspunkte für mehrere VM werden nicht generiert. Verwenden Sie diese Option nur, wenn Sie einheitliche Wiederherstellungspunkte für mehrere VM für eine Gruppe von Computern erstellen müssen, indem Sie alle Computer mit derselben Replikationsgruppe schützen. 
...
### -UseManagedDisk
Gibt an, ob der im Failover erstellte virtuelle Azure-Computer verwaltete Datenträger verwenden soll. Er akzeptiert entweder "Wahr" oder "Falsch".
...
### -VmmToVmm
Wechseln Sie den Parameter, um anzugeben, dass das replizierte Element ein virtueller Hyper-V-Computer ist, der zwischen VMM-verwalteten Hyper-V-Websites repliziert wird.
...
### -VMwareToAzure
Wechseln Sie den Parameter, um anzugeben, dass es sich bei dem replizierten Element um einen virtuellen VMware-Computer oder physischen Server handelt, der in Azure repliziert wird.
...
### -Confirm
Fordert vor dem Starten des Vorgangs zur Bestätigung auf.
