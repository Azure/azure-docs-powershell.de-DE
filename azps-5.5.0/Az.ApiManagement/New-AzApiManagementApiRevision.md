...
Das **Cmdlet "New-AzApiManagementApiRevision"** erstellt eine API-Revision für eine vorhandene API im API-Verwaltungskontext.

## BEISPIELE

### Beispiel 1: Erstellen einer leeren API-Überarbeitung für eine API
```powershell
...
Mit diesem Befehl wird eine API-Überarbeitung `2` der `echo-api` API erstellt.

### Beispiel 2: Erstellen einer API-Überarbeitung aus einer vorhandenen API und Kopieren aller Vorgänge, Tags und Richtlinien
```powershell
...
Mit diesem Befehl wird eine API-Überarbeitung `5` der `echo-api` API erstellt.

## PARAMETERS

### -ApiId
Bezeichner für API, deren Revision erstellt werden soll.
...
### -ApiRevisionDescription
Api-Revisionsbeschreibung. Dieser Parameter ist optional.
...
### -ServiceUrl
Eine URL des Webdiensts, der die API im Back-End-Dienst verfügbar gibt. Diese URL wird nur von Azure API Management verwendet und nicht öffentlich gemacht. Muss 1 bis 2.000 Zeichen lang sein. Dieser Parameter ist erforderlich.
...
### -SourceApiRevision
Api-Revisions-ID der Quell-API. Dieser Parameter ist optional.
