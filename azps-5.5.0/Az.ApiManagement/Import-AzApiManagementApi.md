...
## BESCHREIBUNG
Das **Cmdlet "Import-AzApiManagementApi"** importiert eine Azure API-Verwaltungs-API aus einer Datei oder url im Web Application Description Language (WADL), Web Services Description Language (WSDL) oder Swagger-Format.
...
Mit diesem Befehl wird eine API aus der angegebenen DATEI WADL importiert.

### Beispiel 2: Importieren einer API aus einer Swaggerdatei
```powershell
PS C:\>$ApiMgmtContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Mit diesem Befehl wird eine API aus der angegebenen Datei "Swagger" importiert.

### Beispiel 3: Importieren einer API aus einer WADL-Verknüpfung
```powershell
PS C:\>$ApiMgmtContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Mit diesem Befehl wird eine API aus der angegebenen WADL-Verknüpfung importiert.

### Beispiel 4: Importieren einer API aus einem Open Api Link
```powershell
...
Mit diesem Befehl wird eine API aus der angegebenen Open 3.0-Spezifikationsverknüpfung importiert.

### Beispiel 5: Importieren einer API aus einem Open Api Link in einen ApiVersionssatz
...
### -ApiRevision
Bezeichner der API-Überarbeitung. Dieser Parameter ist optional. Wenn keine Angabe erfolgt, erfolgt der Import in die derzeit aktive Überarbeitung oder eine neue API.
...
### -ApiType
Dieser Parameter ist mit dem Standardwert "Http" optional. Die Option "Soap" gilt nur beim Importieren von WSDL und erstellt eine SOAP-Passthrough-API.
...
### -ApiVersion
Die API-Version der zu erstellende API. Dieser Parameter ist optional.
...
### -Protocol
Web-API-Protokolle (http, https). Protokolle, über die API verfügbar gemacht wird. Dieser Parameter ist optional. Sofern angegeben, werden die im Spezifikationsdokument angegebenen Protokolle überschrieben.
...
### -ServiceUrl
Eine URL des Webdiensts, der die API verfügbar machen soll. Diese URL wird nur von Azure API Management verwendet und nicht öffentlich gemacht. Dieser Parameter ist optional. Sofern angegeben, setzt sie die im Dokument "Spezifikationen" angegebene "ServiceUrl" außer Kraft.
...
### -SpecificationFormat
Gibt das Spezifikationsformat an.
psdx_paramvalues Wadl, Wsdl und Swagger
...
### -SpecificationPath
Gibt den Dateipfad der Spezifikation an.
