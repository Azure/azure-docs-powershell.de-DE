...
Die Tasten werden nicht in die Ergebnisdetails eingeschlossen. Zum Erhalten von Schlüsseln verwenden **Sie "Get-AzApiManagementSubscriptionKey".**

## BEISPIELE

### Beispiel 1: Alle Abonnements erhalten
```powershell
...
### Beispiel 2: Erhalten eines Abonnements mit einer angegebenen ID
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-East-US" -ServiceName "contoso"
...
### Beispiel 3: Alle Abonnements für einen Benutzer erhalten
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-East-US" -ServiceName "contoso"
...
### Beispiel 4: Alle Abonnements für ein Produkt erhalten
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-East-US" -ServiceName "contoso"
...
### Beispiel 5: Alle Abonnements für einen Bereich erhalten
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-East-US" -ServiceName "contoso"
...
### Beispiel 6: Alle Abonnements für ein Produkt und einen Benutzerbereich erhalten
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-East-US" -ServiceName "contoso"
...
### -ProductId
Gibt einen Produktbezeichner an.
Falls angegeben, findet dieses Cmdlet alle Abonnements nach dem Produktbezeichner.
...
### -Scope
Bereichsbezeichner. Der Umfang des Abonnements, unabhängig davon, ob es sich um den Api-Bereich /apis/{apiId} oder den Produktbereich /products/{productId} oder den globalen API-Bereich /apis oder den globalen Bereich /handelt.
...
### -UserId
Gibt einen Benutzerbezeichner an.
Falls angegeben, findet dieses Cmdlet alle Abonnements nach der Benutzer-ID.
