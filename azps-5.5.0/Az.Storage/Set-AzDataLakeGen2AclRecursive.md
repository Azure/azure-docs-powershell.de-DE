...
## BESCHREIBUNG
Das **Cmdlet "Set-AzDataLakeGen2AclRecursive"** legt den ACL rekursiv für den angegebenen Pfad fest. Die Eingabe-ACL ersetzt die ursprüngliche ACL vollständig.

## BEISPIELE

### Beispiel 1: Rekursives Festlegen von ACL für ein Verzeichnis
```
...
Mit diesem Befehl wird zuerst ein ACL-Objekt mit drei acl-Einträgen erstellt und dann rekursiv für ein Verzeichnis bestimmt.

### Beispiel 2: Rekursives Festlegen von ACL für ein Stammverzeichnis des Dateisystems
```
...
Dieser Befehl legt ACL zuerst rekursiv auf ein Stammverzeichnis fest und ist fehlgeschlagen. Anschließend wird es mit ContinuationToken fortgesetzt, nachdem der Benutzer die fehlgeschlagene Datei korrigiert hat.

### Beispiel 3: Festlegen eines rekursiven ACL-Abschnitts nach Abschnitt
```
$token = $null
...
Dieses Skript legt den #A0 rescursiv für verzeichnisblöckeweise mit der Blockgröße "BatchSize * MaxBatchCount" fest. In diesem Skript ist die Blockgröße 200.

### Beispiel 4: Richten Sie den ACL rekursiv für ein Verzeichnis und ContinueOnFailure ein, und setzen Sie den ACL dann nach dem anderen fort.
```
...
Dieser Befehl legt ACL zuerst rekursiv auf ein Verzeichnis mit ContinueOnFailure fest. Bei einigen Elementen ist ein Fehler auftritt, und dann werden die fehlgeschlagenen Elemente wieder eins zu eins fortgesetzt.

## PARAMETERS

### -Acl
Die POSIX-Zugriffssteuerungsliste, um rekursiv für die Datei oder das Verzeichnis festgelegt zu werden.
...
### -MaxBatchCount
Maximale Anzahl von Batches, für die ein einzelner Änderungsvorgang der Zugriffssteuerung ausgeführt werden kann. Wenn die Größe des Datasets die Multipliziergröße von "BatchSize" von MaxBatchCount überschreitet, wird das Fortsetzungstoken zurückgeben.
