...
### Beispiel 3: Erstellt eine Containergruppe für die Ausführung bis zum Abschluss.
```
...
Mit diesen Befehlen wird eine Containergruppe erstellt, in der "Hallo" und Stopps gedruckt werden.

### Beispiel 4: Erstellt eine Containergruppe mithilfe eines Bilds in der Azure Container Registry.
```
...
Mit diesen Befehlen wird eine Containergruppe mithilfe eines nginx-Bilds in der Azure Container Registry erstellt.

### Beispiel 5: Erstellt eine Containergruppe mithilfe eines Bilds in der benutzerdefinierten Containerbildregistrierung.
```
PS C:\> $secpasswd = ConvertTo-SecureString "PlainTextPassword" -AsPlainText -Force
...
Mit diesen Befehlen wird eine Containergruppe erstellt, indem ein benutzerdefiniertes Bild aus einer benutzerdefinierten Containerbildregistrierung verwendet wird.

### Beispiel 6: Erstellt eine Containergruppe, die das Azure -Dateivolume einbeigt.
```
PS C:\> $secpasswd = ConvertTo-SecureString "PlainTextPassword" -AsPlainText -Force
PS C:\> $mycred = New-Object System.Management.Automation.PSCredential ("username", $secpasswd)
...
Mit diesen Befehlen wird eine Containergruppe erstellt, in der die bereitgestellte Azure-Dateifreigabe bereitgestellt `/mnt/azfile` wird.
...
Mit diesen Befehlen wird eine Containergruppe mit einer vom System zugewiesenen Identität erstellt, die das neueste nginx-Bild verwendet, und es wird eine öffentliche IP-Adresse mit Öffnen von Port 8000 anfordert.
...
Mit diesen Befehlen wird eine Containergruppe mit dem System zugewiesenen Identität und der vom Benutzer zugewiesenen Identität mithilfe des neuesten nginx-Bilds erstellt, und es wird eine öffentliche IP-Adresse mit Öffnen von Port 8000 anfordert.

## PARAMETERS

### -AssignIdentity
Aktivieren der vom System zugewiesenen Identität
...
Die Anmeldeinformationen für das Speicherkonto der Azure-Dateifreigabe, um bereitgestellt zu werden, wobei der Benutzername der Name des Speicherkontos und der Schlüssel der Speicherkontoschlüssel ist.
...
Der Bereitstellungspfad für das Azure-Dateivolume.
...
### -DnsNameLabel
Die Namenbezeichnung des DNS für die IP-Adresse.
...
### -MemoryInGB
Der erforderliche Speicher in GB.
Standard: 1,5
...
### -Port
Die zu öffnende(n) Port(n). Standard: [80]
...
### -RegistryCredential
Die Anmeldeinformationen für die benutzerdefinierte Containerregistrierung.
...
Der Anmeldeserver der benutzerdefinierten Containerregistrierung.
...
### -RestartPolicy
Die Richtlinie für den Containerneustart. Standard: Immer
