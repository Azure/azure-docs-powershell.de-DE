...
## BESCHREIBUNG
Das Set-AzDataFactoryV2IntegrationRuntime aktualisiert eine Integrationslaufzeit mit bestimmten Parametern.

## BEISPIELE

### Beispiel 1: Beschreibung der Updateintegrations-Laufzeit.
```
...
Das Cmdlet aktualisiert die Beschreibung der Integrationslaufzeit namens "test-selfhost-ir".

### Beispiel 2: Selbst gehostete Integrationslaufzeit teilen.
```
PS C:\> Set-AzDataFactoryV2IntegrationRuntime -ResourceGroupName 'rg-test-dfv2' -DataFactoryName 'test-df-eu2' -Name 'test-selfhost-ir' `
...
Das Cmdlet fügt adF hinzu, um die Laufzeit der gemeinsam genutzten Integration zu verwenden. Bei Verwendung `-SharedIntegrationRuntimeResourceId` von `-Type` Parametern muss auch das eingeschlossen werden. Beachten Sie, dass der Daten factory die Berechtigung zur Verwendung der Integrationslaufzeit erteilt werden muss, bevor das Cmdlet ausgeführt wird.

### Beispiel 3: Konfigurieren Self-Hosted A0 als Proxy für Azure-SSIS IR in ADF.
```
...
Das Cmdlet aktualisiert die Azure-SSIS-Integrationslaufzeit, um die selbst gehostete Integrationslaufzeit als Datenproxy zu verwenden.
...
### -DataFlowComputeType
Der Rechentyp des Datenflussclusters, der den Datenflussauftrag ausführen soll.
...
### -DataFlowCoreCount
Die Kernanzahl des Datenflussclusters, der den Datenflussauftrag ausführen soll.
...
### -DataFlowTimeToLive
Die Zeit zum Leben (in Minuten) des Datenflussclusters, der den Datenflussauftrag ausführen soll.
...
Der Self-Hosted Integration Runtime-Name, der als Proxy verwendet wird
...
Der Name des verknüpften Azure Blob Storage-Diensts, der auf den Stagingdatenspeicher verweist, der beim Verschieben von Daten zwischen Self-Hosted und Azure-SSIS Integration Runtime verwendet wird
...
### -DataProxyStagingPath
Der Pfad im Stagingdatenspeicher, der beim Verschieben von Daten zwischen Self-Hosted und Azure-SSIS-Integrations-Runtimes verwendet wird, wird ein Standardcontainer verwendet, wenn keine Daten angegeben sind.
...
### -Edition
Die Edition für die SSIS-Integrationslaufzeit, die "Standard" oder "Enterprise" sein kann, ist "Standard", wenn sie nicht angegeben ist.
...
### -LicenseType
Der Lizenztyp, den Sie für SSIS IR auswählen möchten. Es gibt zwei Typen: "LicenseIncluded" oder "BasePrice". Wenn Sie für die Preise von Azure Hybrid Use Benefit (AHUB) qualifiziert sind, wählen Sie "BasePrice" aus. Wenn nicht, wählen Sie "LicenseIncluded" aus.
...
### -VNetId
Die ID des VNets, das von der Integrationslaufzeit gemeinsam verwendet wird.
