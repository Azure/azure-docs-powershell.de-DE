...
### Beispiel 2: Hinzufügen einer neuen Erweiterung mit Erweiterungsparametern, die über die Pipeline angegeben werden
```powershell
...
Dadurch wird eine neue Erweiterung mit den Erweiterungsparametern erstellt, die von dem über die Pipeline übergebenen Objekt bereitgestellt werden.
Dies ist ideal, wenn Sie die Parameter eines Computers übernehmen und auf einen anderen Computer anwenden möchten.

### Beispiel 3: Hinzufügen einer neuen Erweiterung mit über die Pipeline angegebenen Speicherort
```powershell
...
Dadurch wird eine neue Computererweiterung erstellt, indem die über die Pipeline bereitgestellte Identität verwendet wird.
Wahrscheinlich werden Sie dies nicht tun, aber es ist möglich.
...
Dadurch wird eine neue Computererweiterung erstellt, indem die über die Pipeline bereitgestellte Identität und die Erweiterungsdetails des übergebenen Erweiterungsobjekts verwendet werden.
Wahrscheinlich werden Sie dies nicht tun, aber es ist möglich.
...
### -Location
Der geo-Standort, an dem sich die Ressource befindet
...
### -ProtectedSetting
Die Erweiterung kann entweder geschützteSettings oder protectedSettingsFromKeyVault oder überhaupt keine geschützten Einstellungen enthalten.
...
<IMachineExtension>EXTENSIONPARAMETER: Beschreibt eine Computererweiterung.
  ...
  - `[MachineExtensionType <String>]`: Gibt den Typ der Erweiterung an. Beispiel: "CustomScriptExtension".
  - `[ProtectedSetting <IMachineExtensionPropertiesProtectedSettings>]`: Die Erweiterung kann entweder geschützteSettings oder protectedSettingsFromKeyVault oder überhaupt keine geschützten Einstellungen enthalten.
