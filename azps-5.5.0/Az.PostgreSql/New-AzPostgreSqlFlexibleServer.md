...
Dieses Cmdlet erstellt einen flexiblen PostgreSql-Server mit Standardwerten, stellt den Server in einem neuen virtuellen Netzwerk zur Verfügung, und es wird ein Subnetz an den Server delegiert.
Die Standardwerte für den Speicherort sind "USA 2", "SKU" ist Standard_D2s_v3, "SKU-Ebene" ist "GeneralPurpose", und die Speichergröße beträgt 128GiB.
...
(Beispiel: $server. SecuredPassword | ConvertFrom-SecureString -AsPlainText)
...
Dieses Cmdlet erstellt einen flexiblen PostgreSql-Server mit vnet-ID oder vnet-Namen, die von einem Benutzer bereitgestellt werden.
...
Dieses Cmdlet erstellt einen postgreSql-flexiblen Server mit vnet-Namen, Subnetznamen, vnetpräfix und Subnetzpräfix.
...
Dieses Cmdlet erstellt einen flexiblen PostgreSql-Server, der mit angegebenen IP-Adressen geöffnet wird.
...
Geben Sie eine einzelne oder einen Bereich von IP-Adressen ein, die in die Liste zulässiger IPs aufgenommen werden sollen.
Die IP-Adressbereiche müssen durch Bindestriche getrennt sein und dürfen keine Leerzeichen enthalten.
Die Angabe von 0.0.0.0 ermöglicht den öffentlichen Zugriff von allen ressourcen, die in Azure bereitgestellt werden, den Zugriff auf Ihren Server.
Wenn Sie keine IP-Adresse angeben, wird der Server im öffentlichen Zugriffsmodus festgelegt, aber keine Firewallregel erstellt.
...
### -SKU
Der Name der SKU, normalerweise Stufen + Familie + Kerne, z. B. Standard_B1ms, Standard_D2ds_v4.
...
### -Subnet
Der Name oder die ID eines vorhandenen Subnetzes oder eines neuen Subnetzes, das erstellt werden soll.
...
### -SubnetPrefix
Das Subnetz-IP-Adresspräfix, das beim Erstellen eines neuen Vnets im CIDR-Format verwendet wird.
...
### -Vnet
Der Name oder die ID eines vorhandenen virtuellen Netzwerks oder eines neuen zu erstellende neuen Netzwerks.
...
### -VnetPrefix
Das IP-Adresspräfix, das beim Erstellen eines neuen Vnets im CIDR-Format verwendet wird.
