...
Listen Sie alle Themen des Ereignisrasters in der Ressourcengruppe \` "MyResourceGroupName" \` ohne Seitenumrisse auf.
...
Listen Sie die ersten 10 Themen des Ereignisrasters (sofern vorhanden) in der Ressourcengruppe \` "MyResourceGroupName" auf, die den Anforderungen $odataFilter \` entsprechen. Wenn weitere Ergebnisse verfügbar sind, wird $result. "NextLink" wird nicht $null. Um die nächste Seite(n) mit Themen zu erhalten, wird erwartet, dass der Benutzer die Seite(n) erneut aufruft Get-AzEventGridTopic ergebnisse verwendet. NextLink aus dem vorherigen Anruf. Der Anrufer sollte beendet werden, wenn das Ergebnis vor sich geht. "NextLink" wird $null.
