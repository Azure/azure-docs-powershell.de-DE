...
## SYNOPSIS
Aktualisiert eine Datensammlungsregel (vollständigen Ersatz).
...
Um den Parameter "-RuleFile" zu verwenden, erstellen Sie eine json-Datei, die drei Eigenschaften enthält: DataSources, Ziele, Datenflüsse (siehe Beispiel für #1).

Die Schemadetails finden Sie [hier.](https://docs.microsoft.com/en-us/rest/api/monitor/datacollectionrules/create)

Die Ausgabe eines DCR, der mit dem Cmdlet ConvertTo-Json serialisiert wurde, wird ebenfalls unterstützt (Beispiel #2).
...
### Beispiel 2: Aktualisieren der Datensammlungsregel JSON aus PSDataCollectionRuleResource
```powershell
PS C:\>Set-AzDataCollectionRule -Location 'East US 2 EUAP'
...
### Beispiel 3: Aktualisieren der Datensammlungsregel von einem Objekt
```powershell
