---
Module Name: Az.Monitor
Module Guid: 698c387c-bd6b-41c6-82ce-721f1ef39548
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.monitor
Help Version: 4.0.4.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Monitor/Monitor/help/Az.Monitor.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Monitor/Monitor/help/Az.Monitor.md
ms.openlocfilehash: ce60ca11184537254fa899317241e69849eb0b1c
ms.sourcegitcommit: c05d3d669b5631e526841f47b22513d78495350b
ms.translationtype: MT
ms.contentlocale: de-DE
ms.lasthandoff: 02/09/2021
ms.locfileid: "100170285"
---
# Az.Monitor-Modul
## Beschreibung
In diesem Thema werden Hilfethemen für die Azure Insights-Cmdlets angezeigt.

## Az.Monitor-Cmdlets
### [Add-AzAutoscaleSetting](Add-AzAutoscaleSetting.md)
Erstellt eine Einstellung für die Automatische Skala.

### [Add-AzLogProfile](Add-AzLogProfile.md)
Erstellt ein neues Aktivitätsprotokollprofil. Dieses Profil wird verwendet, um das Aktivitätsprotokoll entweder in einem Azure -Speicherkonto zu archivieren oder es in einen Azure-Ereignishub im selben Abonnement zu streamen. 

### [Add-AzMetricAlertRule](Add-AzMetricAlertRule.md)
Fügt eine metrisch-basierte Warnungsregel hinzu oder aktualisiert diese.

### [Add-AzMetricAlertRuleV2](Add-AzMetricAlertRuleV2.md)
Fügt eine auf V2 basierende (nicht klassische) metrische Warnungsregel hinzu oder aktualisiert diese.

### [Add-AzWebtestAlertRule](Add-AzWebtestAlertRule.md)
Fügt eine Webtestwarnungsregel hinzu oder aktualisiert diese.

### [Disable-AzActivityLogAlert](Disable-AzActivityLogAlert.md)
Deaktiviert eine Aktivitätsprotokollbenachrichtigung und legt deren Tags fest.

### [Enable-AzActivityLogAlert](Enable-AzActivityLogAlert.md)
Aktiviert eine Aktivitätsprotokollbenachrichtigung und legt deren Tags fest.

### [Get-AzActionGroup](Get-AzActionGroup.md)
Ruft Aktionsgruppe(n) ab.

### [Get-AzActivityLog](Get-AzActivityLog.md)
Rufen Sie Aktivitätsprotokollereignisse ab.

### [Get-AzActivityLogAlert](Get-AzActivityLogAlert.md)
Ruft mindestens eine Aktivitätsprotokollbenachrichtigungsressourcen ab.

### [Get-AzAlertHistory](Get-AzAlertHistory.md)
Ruft den Verlauf von Benachrichtigungen ab.

### [Get-AzAlertRule](Get-AzAlertRule.md)
Ruft Benachrichtigungsregeln ab.

### [Get-AzAutoscaleHistory](Get-AzAutoscaleHistory.md)
Ruft den Verlauf der Automatischen Skala ab.

### [Get-AzAutoscaleSetting](Get-AzAutoscaleSetting.md)
Ruft die Einstellungen für die Automatische Skalierung ab.

### [Get-AzDiagnosticSetting](Get-AzDiagnosticSetting.md)
Ruft die protokollierten Kategorien und Zeitkörner ab.

### [Get-AzDiagnosticSettingCategory](Get-AzDiagnosticSettingCategory.md)
Erhalten oder Auflisten der unterstützten Diagnoseeinstellungskategorie für die Azure-Ressource.

### [Get-AzInsightsPrivateLinkScope](Get-AzInsightsPrivateLinkScope.md)
Bereich für private Verknüpfungen erhalten

### [Get-AzInsightsPrivateLinkScopedResource](Get-AzInsightsPrivateLinkScopedResource.md)
Get for private link scoped resource

### [Get-AzLogProfile](Get-AzLogProfile.md)
Ruft ein Protokollprofil ab.

### [Get-AzMetric](Get-AzMetric.md)
Ruft die metrischen Werte einer Ressource ab.

### [Get-AzMetricAlertRuleV2](Get-AzMetricAlertRuleV2.md)
Ruft metrische Warnungsregeln für V2 (nicht klassisch) ab

### [Get-AzMetricDefinition](Get-AzMetricDefinition.md)
Ruft Metrikdefinitionen ab.

### [Get-AzScheduledQueryRule](Get-AzScheduledQueryRule.md)
Ruft geplante Abfrageressourcen ab

### [New-AzActionGroup](New-AzActionGroup.md)
Erstellt ein ActionGroup-Referenzobjekt im Arbeitsspeicher.

### [New-AzActionGroupReceiver](New-AzActionGroupReceiver.md)
Erstellt einen neuen Empfänger für eine Aktionsgruppe.

### [New-AzActivityLogAlertCondition](New-AzActivityLogAlertCondition.md)
Erstellt ein neues Benachrichtigungsobjekt für das Aktivitätsprotokoll im Arbeitsspeicher.

### [New-AzAlertRuleEmail](New-AzAlertRuleEmail.md)
Erstellt eine E-Mail-Aktion für eine Warnungsregel.

### [New-AzAlertRuleWebhook](New-AzAlertRuleWebhook.md)
Erstellt einen Warnungsregel-Webhook.

### [New-AzAutoscaleNotification](New-AzAutoscaleNotification.md)
Erstellt eine E-Mail-Benachrichtigung für die automatische Skalierung.

### [New-AzAutoscaleProfile](New-AzAutoscaleProfile.md)
Erstellt ein Profil für die Automatische Skala.

### [New-AzAutoscaleRule](New-AzAutoscaleRule.md)
Erstellt eine Autoskalenregel.

### [New-AzAutoscaleWebhook](New-AzAutoscaleWebhook.md)
Erstellt einen Autoskalen-Webhook.

### [New-AzDiagnosticDetailSetting](New-AzDiagnosticDetailSetting.md)
Erstellen Sie das PSDiagnosticDetailSetting-Objekt. Der Typ kann eine Metrik oder ein Protokoll sein.

### [New-AzDiagnosticSetting](New-AzDiagnosticSetting.md)
Erstellen Sie das PSServiceDiagnosticSettings-Objekt.

### [New-AzInsightsPrivateLinkScope](New-AzInsightsPrivateLinkScope.md)
Erstellen eines privaten Verknüpfungsbereichs

### [New-AzInsightsPrivateLinkScopedResource](New-AzInsightsPrivateLinkScopedResource.md)
Erstellen für Ressource mit Bereich "Private Verknüpfung"

### [New-AzMetricAlertRuleV2Criteria](New-AzMetricAlertRuleV2Criteria.md)
Erstellt ein lokales Kriterienobjekt, das zum Erstellen einer neuen Metrikwarnung verwendet werden kann.

### [New-AzMetricAlertRuleV2DimensionSelection](New-AzMetricAlertRuleV2DimensionSelection.md)
Erstellt ein lokales Dimensionsauswahlobjekt, das zum Erstellen eines metrischen Warnungskriterien verwendet werden kann.

### [New-AzMetricFilter](New-AzMetricFilter.md)
Erstellt einen metrischen Dimensionsfilter, der zum Abfragen von Metriken verwendet werden kann.

### [New-AzScheduledQueryRule](New-AzScheduledQueryRule.md)
Erstellt eine Protokollwarnungsregel (Typ der geplanten Abfrageregel)

### [New-AzScheduledQueryRuleAlertingAction](New-AzScheduledQueryRuleAlertingAction.md)
Erstellt ein Objekt vom Typ "Warnungsaktion"

### [New-AzScheduledQueryRuleAznsActionGroup](New-AzScheduledQueryRuleAznsActionGroup.md)
Erstellt ein Objekt vom Typ "Azns-Aktionsgruppe".

### [New-AzScheduledQueryRuleLogMetricTrigger](New-AzScheduledQueryRuleLogMetricTrigger.md)
Erstellt ein Objekt vom Typ "Protokollmetrischer Trigger".

### [New-AzScheduledQueryRuleSchedule](New-AzScheduledQueryRuleSchedule.md)
Erstellt ein Objekt vom Typ "Zeitplan".

### [New-AzScheduledQueryRuleSource](New-AzScheduledQueryRuleSource.md)
Erstellt ein Objekt vom Typ "Quelle".

### [New-AzScheduledQueryRuleTriggerCondition](New-AzScheduledQueryRuleTriggerCondition.md)
Erstellt ein Objekt vom Typ "Triggerbedingung".

### [Remove-AzActionGroup](Remove-AzActionGroup.md)
Entfernt eine Aktionsgruppe.

### [Remove-AzActivityLogAlert](Remove-AzActivityLogAlert.md)
Entfernt eine Aktivitätsprotokollbenachrichtigung.

### [Remove-AzAlertRule](Remove-AzAlertRule.md)
Entfernt eine Warnungsregel.

### [Remove-AzAutoscaleSetting](Remove-AzAutoscaleSetting.md)
Entfernt eine Einstellung für die Automatische Skala.

### [Remove-AzDiagnosticSetting](Remove-AzDiagnosticSetting.md)
Entfernen Sie eine Diagnoseeinstellung für eine Ressource.

### [Remove-AzInsightsPrivateLinkScope](Remove-AzInsightsPrivateLinkScope.md)
Bereich für private Verknüpfungen löschen

### [Remove-AzInsightsPrivateLinkScopedResource](Remove-AzInsightsPrivateLinkScopedResource.md)
Delete for private link scoped resource

### [Remove-AzLogProfile](Remove-AzLogProfile.md)
Entfernt ein Protokollprofil.

### [Remove-AzMetricAlertRuleV2](Remove-AzMetricAlertRuleV2.md)
Entfernt eine metrische Warnungsregel aus V2 (nicht der klassischen Version).

### [Remove-AzScheduledQueryRule](Remove-AzScheduledQueryRule.md)
Entfernt eine Protokollwarnungsregel

### [Set-AzActionGroup](Set-AzActionGroup.md)
Erstellt eine neue Aktionsgruppe oder aktualisiert eine vorhandene Aktionsgruppe.

### [Set-AzActivityLogAlert](Set-AzActivityLogAlert.md)
Erstellt eine neue Benachrichtigung oder legt eine vorhandene Aktivitätsprotokollbenachrichtigung fest.

### [Set-AzDiagnosticSetting](Set-AzDiagnosticSetting.md)
Legt die Protokoll- und Metrikeinstellungen für die Ressource fest.

### [Set-AzScheduledQueryRule](Set-AzScheduledQueryRule.md)
Aktualisiert eine Protokollwarnungsregel

### [Update-AzInsightsPrivateLinkScope](Update-AzInsightsPrivateLinkScope.md)
Update für privaten Verknüpfungsbereich

### [Update-AzScheduledQueryRule](Update-AzScheduledQueryRule.md)
Aktualisiert eine Protokollbenachrichtigungsregel

