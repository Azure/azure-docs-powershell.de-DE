...
### Beispiel 1: Erstellen eines Datenarbeitsbereichs
```powershell
...
Mit diesem Befehl wird ein Arbeitsbereich "Databungs" erstellt.

### Beispiel 2: Erstellen eines Arbeitsbereichs mit einem benutzerdefinierten virtuellen Netzwerk
```powershell
...
Mit diesem Befehl wird ein Databungsarbeitsbereich mit benutzerdefiniertem virtuellem Netzwerk in einer Ressourcengruppe erstellt.

### Beispiel 3: Erstellen eines Datenarbeitsbereichs mit Aktivierter Verschlüsselung
```powershell
...
Dieser Befehl erstellt einen Arbeitsbereich "Databungs" und legt ihn so fest, dass er für die Verschlüsselung vorbereitet wird.
Weitere Einstellungen zur Verschlüsselung finden Sie in Update-AzDatabricksWorkspace Beispielen für die Verschlüsselung.
...
### -PublicSubnetName
Der Name eines öffentlichen Subnetzes innerhalb des virtuellen Netzwerks.
...
Ein boolescher Wert, der angibt, ob das DBFS-Stammdateisystem mit sekundärer Verschlüsselungsebene mit plattform verwalteten Schlüsseln für ruhede Daten aktiviert wird.
...
### -VirtualNetworkId
Die ID eines virtuellen Netzwerks, in dem dieser Datenknotencluster erstellt werden soll.
