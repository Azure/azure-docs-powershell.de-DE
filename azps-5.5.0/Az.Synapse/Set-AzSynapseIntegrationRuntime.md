...
Das Cmdlet fügt den Arbeitsbereich zur Verwendung der Laufzeit für die gemeinsame Integration hinzu. Bei Verwendung `-SharedIntegrationRuntimeResourceId` von `-Type` Parametern muss auch das eingeschlossen werden. Beachten Sie, dass dem Arbeitsbereich die Berechtigung zur Verwendung der Integrationslaufzeit erteilt werden muss, bevor das Cmdlet ausgeführt wird.
...
Der Pfad im Stagingdatenspeicher, der beim Verschieben von Daten zwischen Self-Hosted und Azure-SSIS-Integrations-Runtimes verwendet wird, wird ein Standardcontainer verwendet, wenn keine Daten angegeben werden.
...
### -ExpressCustomSetup
Die express benutzerdefinierte Einrichtung für die SSIS-Integrationslaufzeit, die zum Einrichten von Konfigurationen und Komponenten von Drittanbietern ohne benutzerdefiniertes Setupskript verwendet werden kann.
...
### -LicenseType
Der Lizenztyp, den Sie für SSIS IR auswählen möchten.
Es gibt zwei Typen: "LicenseIncluded" oder "BasePrice".
Wenn Sie für die Preise von Azure Hybrid Use Benefit (AHUB) qualifiziert sind, wählen Sie "BasePrice" aus.
...
### -VNetId
Die ID des VNets, dem die Integrationslaufzeit hinzugefügt wird.
