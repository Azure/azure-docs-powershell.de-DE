...
## BESCHREIBUNG
Das **Cmdlet "Start-AzAutomationDscCompilationJob"** kompiliert eine Konfiguration mit dem Status "APS Desired State Configuration (DSC)" in der Azure-Automatisierung.
...
Der zweite Befehl kompiliert die DSC-Konfiguration namens "Config01".
Der Befehl enthält die Werte in $Params für DSC-Konfigurationsparameter.

### Beispiel 2: Kompilieren einer Azure -DSC-Konfiguration in der Automatisierung mit einer neuen Buildversion der Knotenkonfiguration.
```
PS C:\>$Params = @{"StringParam"="Hello World";"IntegerParam"=32}
...
Ähnlich wie im ersten Beispiel erstellt der erste Befehl ein Wörterbuch mit Parametern und speichert sie in der $Params Variable.
Der zweite Befehl kompiliert die DSC-Konfiguration namens "Config01".
Der Befehl enthält die Werte in $Params für DSC-Konfigurationsparameter.
Die zuvor vorhandene Knotenkonfiguration wird nicht überschrieben, indem eine neue Knotenkonfiguration mit dem Namen "Config01[<2>] erstellt <NodeName> wird. Die Versionsnummer wird basierend auf der vorhandenen Versionsnummer, die bereits vorhanden ist, erhöht.
