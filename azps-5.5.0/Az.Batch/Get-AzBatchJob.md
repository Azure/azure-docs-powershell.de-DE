...
## BESCHREIBUNG
Das **Cmdlet "Get-AzBatchJob"** ruft die Azure -Batchaufträge für das Vom Parameter *"BatchAccountContext"* angegebene Batchkonto ab.
Sie können den Parameter *"Id"* verwenden, um einen einzelnen Auftrag zu erhalten.
Sie können den Parameter *"Filter"* verwenden, um die Aufträge zu erhalten, die einem Open Data Protocol (OData)-Filter entsprechen.
Wenn Sie eine Auftragsplan-ID oder eine **PSCloudJobSchedule-Instanz** liefern, gibt dieses Cmdlet nur die Aufträge für diesen Stellenplan zurück.

## BEISPIELE

### Beispiel 1: Erhalten eines Stapelauftrags nach ID
```
...
Dieser Befehl ruft den Auftrag ab, der die ID "Job01" hat.
Verwenden Sie das Get-AzBatchAccountKey-Cmdlet, um der Variablen einen Kontext $Context zuzuordnen.

### Beispiel 2: Alle aktiven Aufträge für einen Stellenplan erhalten
```
...
Dieser Befehl ruft die aktiven Aufträge für den Auftragszeitplan ab, der die ID "JobSchedule27" hat.
...
Mit diesem Befehl wird der Auftragszeitplan mit der ID "JobSchedule27" mithilfe des cmdlets Get-AzBatchJobSchedule erhalten.
Der Befehl übergibt diesen Auftragszeitplan mithilfe des Pipelineoperators an das aktuelle Cmdlet.
Der Befehl ruft alle Aufträge für diesen Stellenplan ab.
...
Verwenden Sie zum Abrufen **eines PSCloudJobSchedule-Objekts** das Get-AzBatchJobSchedule Cmdlet.
...
### -JobScheduleId
Gibt die ID des Auftragsplans an, der die Aufträge enthält.
...
### -MaxCount
Gibt die maximale Anzahl von Aufträgen an, die zurückgeben werden sollen.
