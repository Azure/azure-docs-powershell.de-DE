...
## BESCHREIBUNG
Das **Cmdlet "Set-AzVMCustomScriptExtension"** fügt einem virtuellen Computer ein benutzerdefiniertes Skript hinzu.
...
Dieser Befehl fügt dem virtuellen Computer namens "VirtualMachine07" ein benutzerdefiniertes Skript hinzu.
...
### -Argument
Gibt die Argumente an, die von der Skripterweiterung an das Skript übergeben werden.
...
### -FileName
Gibt den Namen der Skriptdatei an. Wenn die Datei im Azure-BLOB-Speicher gespeichert ist, wird beim Dateinamenwert die Zwischenschreibung beachtet. Bei Den Dateinamen von Dateien, die im Azure File Storage gespeichert sind, wird die Zwischen- und Kleinschreibung nicht beachtet.
...
### -SecureExecution
Zeigt an, dass mit diesem Cmdlet sichergestellt wird, dass der Wert des Parameters *"Ausführen"* nicht auf dem Server protokolliert oder mithilfe der GET-Erweiterungs-API an den Benutzer zurückgegeben wird.
Der Wert von *"Ausführen"* enthält möglicherweise geheime Daten oder Kennwörter, die sicher an die Skriptdatei übergeben werden sollen.
...
Führen Sie zum Abrufen der Version das cmdlet Get-AzVMExtensionImage mit dem Wert "Microsoft.Compute" für den Parameter *"PublisherName"* und "CustomScriptExtension" für den Parameter *"Type"* aus.
