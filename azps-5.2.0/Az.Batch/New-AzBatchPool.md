...
## BESCHREIBUNG
Das **Cmdlet "New-AzBatchPool"** erstellt einen Pool im Azure Batch-Dienst unter dem Konto, das vom Parameter *"BatchContext" angegeben* wird.

## BEISPIELE

### Beispiel 1: Erstellen eines neuen Pools mit dem Parameter "TargetDedicated" mit "CloudServiceConfiguration"
```
...
Der Pool ist für die Verwendung STANDARD_D1_V2 virtuellen Computer mit der Betriebssystemversion von Familie 4 konfiguriert.

### Beispiel 2: Erstellen eines neuen Pools mit dem Parameter "TargetDedicated" mit "VirtualMachineConfiguration"
```
...
Mit diesem Befehl wird ein neuer Pool mit ID MyPool erstellt, indem der Parameter "TargetDedicated" verwendet wird.
Die Zielzuordnung besteht aus drei Rechenknoten.
Der Pool ist für die Verwendung STANDARD_D1_V2 virtuellen Computer mit dem Betriebssystemabbild "Windows 2016-Datacenter" konfiguriert.
...
Mit diesem Befehl wird ein neuer Pool mit ID AutoScalePool erstellt, indem der Parametersatz "AutoScale" verwendet wird.
Der Pool ist für die Verwendung STANDARD_D1_V2 virtuellen Computern mit dem Betriebssystemabbild des Windows-2016-Rechenzentrums konfiguriert, und die Zielanzahl der Berechnungsknoten wird durch die Autoskalenformel bestimmt.
...
### -ApplicationLicenses
Die Liste der Anwendungslizenzen, die der Batchdienst für jeden Rechenknoten im Pool zur Verfügung stellen wird.
...
Gibt die Zeitspanne in Minuten an, die verstrichen ist, bevor die Poolgröße automatisch entsprechend der Formel für die AutoSkala angepasst wird.
...
Gibt die dem Pool zugeordneten Zertifikate an.
Der Batchdienst installiert die Referenzzertifikate für jeden Rechenknoten des Pools.
...
Gibt Konfigurationseinstellungen für einen Pool basierend auf der Azure Cloud Service Platform an.
...
Gibt an, dass dieses Cmdlet den Pool für die direkte Kommunikation zwischen dedizierten Rechenknoten ein richtet.
...
Gibt die maximale Anzahl von Aufgaben an, die auf einem einzelnen Rechenknoten ausgeführt werden können.
...
### -MountConfiguration
Eine Liste der Dateisysteme, die auf jedem Knoten im Pool angezeigt werden. Dies unterstützt Azure-Dateien, NFS, CIFS/SMB und Blobfuse.
...
### -ResizeTimeout
Gibt das Zeit out für das Zuordnen von Berechnungsknoten zum Pool an.
...
### -StartTask
Gibt die Spezifikation für den Startaufgabe für den Pool an.
Die Startaufgabe wird ausgeführt, wenn ein Rechenknoten dem Pool beitritt, der Rechenknoten neu gestartet oder neu abbildet wird.
...
Gibt die Zielnummer der dedizierten Rechenknoten an, die dem Pool zugewiesen werden.
...
Gibt die Zielnummer der Rechenknoten mit niedriger Priorität an, die dem Pool zugewiesen werden.
...
Gibt die Aufgabenplanungsrichtlinie an, z. B. ComputeNodeFillType.
