---
Module Name: Az.Websites
Module Guid: cc69c625-e961-43f4-8b50-0061eba6e4b6
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.websites
Help Version: 1.0.0.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Websites/Websites/help/Az.Websites.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Websites/Websites/help/Az.Websites.md
ms.openlocfilehash: e21d0de8ee0d0fc407208e7d5ef899147670a0aa
ms.sourcegitcommit: 04221336bc9eed46c05ed1e828a6811534d4b4ab
ms.translationtype: MT
ms.contentlocale: de-DE
ms.lasthandoff: 12/08/2020
ms.locfileid: "98307576"
---
# Az.Websites-Modul
## Beschreibung
ARM (Azure-Ressourcen-Manager) Web App- und App-Dienstplanbefehle.

## Az.Websites-Cmdlets
### [Add-AzWebAppAccessRestrictionRule](Add-AzWebAppAccessRestrictionRule.md)
Fügt einer Azure Web App eine Access-Resticationsregel hinzu.

### [Add-AzWebAppTrafficRouting](Add-AzWebAppTrafficRouting.md)
Fügen Sie dem Slot eine Routingregel hinzu.

### [Edit-AzWebAppBackupConfiguration](Edit-AzWebAppBackupConfiguration.md)


### [Enter-AzWebAppContainerPSSession](Enter-AzWebAppContainerPSSession.md)
Öffnet eine Remote-PowerShell-Sitzung in dem Windows-Container, der auf einer bestimmten Website oder einem bestimmten Slot und in einer bestimmten Ressourcengruppe angegeben ist.

### [Get-AzAppServicePlan](Get-AzAppServicePlan.md)
Ruft einen Azure App Service Plan in der angegebenen Ressourcengruppe ab.

### [Get-AzDeletedWebApp](Get-AzDeletedWebApp.md)
Ruft gelöschte Web Apps im Abonnement ab.

### [Get-AzWebApp](Get-AzWebApp.md)
Ruft Azure Web Apps in der angegebenen Ressourcengruppe ab.

### [Get-AzWebAppAccessRestrictionConfig](Get-AzWebAppAccessRestrictionConfig.md)
Ruft die Access-Restiction-Konfiguration für eine Azure Web App ab.

### [Get-AzWebAppBackup](Get-AzWebAppBackup.md)


### [Get-AzWebAppBackupConfiguration](Get-AzWebAppBackupConfiguration.md)


### [Get-AzWebAppBackupList](Get-AzWebAppBackupList.md)


### [Get-AzWebAppCertificate](Get-AzWebAppCertificate.md)
Ruft ein Azure Web App-Zertifikat ab.

### [Get-AzWebAppContainerContinuousDeploymentUrl](Get-AzWebAppContainerContinuousDeploymentUrl.md)
Get-AzWebAppContainerContinuousDeploymentUrl gibt die fortlaufende Bereitstellungs-URL des Containers zurück.

### [Get-AzWebAppPublishingProfile](Get-AzWebAppPublishingProfile.md)
Ruft ein Azure Web App-Veröffentlichungsprofil ab.

### [Get-AzWebAppSlot](Get-AzWebAppSlot.md)
Ruft einen Azure Web App-Slot ab.

### [Get-AzWebAppSlotConfigName](Get-AzWebAppSlotConfigName.md)
Liste der Web App-Slot-Konfigurationsnamen

### [Get-AzWebAppSlotPublishingProfile](Get-AzWebAppSlotPublishingProfile.md)
Ruft ein Azure Web App-Slot-Veröffentlichungsprofil ab.

### [Get-AzWebAppSnapshot](Get-AzWebAppSnapshot.md)
Ruft die Momentaufnahmen für eine Web App ab.

### [Get-AzWebAppSSLBinding](Get-AzWebAppSSLBinding.md)
Ruft eine Azure Web App-Zertifikat-SSL-Bindung ab.

### [Get-AzWebAppTrafficRouting](Get-AzWebAppTrafficRouting.md)
Erhalten Sie eine Routingregel für den angegebenen Slotnamen.

### [New-AzAppServicePlan](New-AzAppServicePlan.md)
Erstellt einen Azure App Service Plan an einem bestimmten Geo-Standort.

### [New-AzWebApp](New-AzWebApp.md)
Erstellt eine Azure Web App.

### [New-AzWebAppAzureStoragePath](New-AzWebAppAzureStoragePath.md)
Erstellt ein Objekt, das einen Azure -Speicherpfad darstellt, der in einer Web App bereitgestellt werden soll. Er soll als Parameter (-AzureStoragePath) zum Set-AzWebApp und Set-AzWebAppSlot

### [New-AzWebAppBackup](New-AzWebAppBackup.md)


### [New-AzWebAppContainerPSSession](New-AzWebAppContainerPSSession.md)
New-AzWebAppContainerPSSession wird eine neue Remote-PowerShell-Sitzung im Windows-Container erstellt, der auf einer bestimmten Website oder einem bestimmten Slot und in einer bestimmten Ressourcengruppe angegeben ist.

### [New-AzWebAppDatabaseBackupSetting](New-AzWebAppDatabaseBackupSetting.md)


### [New-AzWebAppSlot](New-AzWebAppSlot.md)
Erstellt einen Azure Web App-Slot.

### [New-AzWebAppSSLBinding](New-AzWebAppSSLBinding.md)
Erstellt eine SSL-Zertifikatbindung für eine Azure Web App.

### [Publish-AzWebApp](Publish-AzWebApp.md)
Stellt mithilfe der Zipploy eine Azure Web App aus einer ZIP-, JAR- oder WAR-Datei bereit. 

### [Remove-AzAppServicePlan](Remove-AzAppServicePlan.md)
Entfernt einen Azure-App-Dienstplan.

### [Remove-AzWebApp](Remove-AzWebApp.md)
Entfernt eine Azure Web App.

### [Remove-AzWebAppAccessRestrictionRule](Remove-AzWebAppAccessRestrictionRule.md)
Entfernt eine Zugriffseinschränkungsregel aus einer Azure Web App.

### [Remove-AzWebAppBackup](Remove-AzWebAppBackup.md)


### [Remove-AzWebAppSlot](Remove-AzWebAppSlot.md)


### [Remove-AzWebAppSSLBinding](Remove-AzWebAppSSLBinding.md)
Entfernt eine SSL-Bindung aus einem hochgeladenen Zertifikat.

### [Remove-AzWebAppTrafficRouting](Remove-AzWebAppTrafficRouting.md)
Entfernen Sie eine Routingregel aus dem Slot.

### [Reset-AzWebAppPublishingProfile](Reset-AzWebAppPublishingProfile.md)


### [Reset-AzWebAppSlotPublishingProfile](Reset-AzWebAppSlotPublishingProfile.md)


### [Restart-AzWebApp](Restart-AzWebApp.md)
Startet eine Azure Web App neu.

### [Restart-AzWebAppSlot](Restart-AzWebAppSlot.md)


### [Restore-AzDeletedWebApp](Restore-AzDeletedWebApp.md)
Stellt eine gelöschte Web App in einer neuen oder vorhandenen Web App wieder zur Verfügung.

### [Restore-AzWebAppBackup](Restore-AzWebAppBackup.md)


### [Restore-AzWebAppSnapshot](Restore-AzWebAppSnapshot.md)
Stellt eine Web-App-Momentaufnahme wieder zur Verfügung.

### [Set-AzAppServicePlan](Set-AzAppServicePlan.md)
Legt einen Azure-App-Dienstplan fest.

### [Set-AzWebApp](Set-AzWebApp.md)
Ändert eine Azure Web App.

### [Set-AzWebAppSlot](Set-AzWebAppSlot.md)
Ändert einen Azure Web App-Slot.

### [Set-AzWebAppSlotConfigName](Set-AzWebAppSlotConfigName.md)
Festlegen von Web App-Slot-Konfigurationsnamen

### [Start-AzWebApp](Start-AzWebApp.md)
Startet eine Azure Web App.

### [Start-AzWebAppSlot](Start-AzWebAppSlot.md)
Startet einen Azure Web App-Slot.

### [Stop-AzWebApp](Stop-AzWebApp.md)
Beendet eine Azure Web App.

### [Stop-AzWebAppSlot](Stop-AzWebAppSlot.md)
Beendet ein Azure Web App-Slot.

### [Switch-AzWebAppSlot](Switch-AzWebAppSlot.md)
Tauschen von zwei Slots mit einer Web App

### [Update-AzWebAppAccessRestrictionConfig](Update-AzWebAppAccessRestrictionConfig.md)
Aktualisiert die Vererbung der Aktualisierungskonfiguration für den Hauptzugriff auf die SCM-Website für eine Azure Web App.

### [Update-AzWebAppTrafficRouting](Update-AzWebAppTrafficRouting.md)
Aktualisieren Sie eine Routingregel für das Slot.

