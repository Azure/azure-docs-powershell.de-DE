...
## BESCHREIBUNG
Dieses Cmdlet ermöglicht das Erstellen zustandsloser oder zustandsfreier Dienste unter der angegebenen Anwendung. Der Dienst sollte im Anwendungsmanifest beendet werden, und der Typ sollte mit dem typ im Manifest identisch sein. Der Anwendungsname sollte ein Präfix des Dienstnamens sein.
...
In diesem Beispiel wird der neue zustandslose Dienst "testApp~testService1" mit der Instanzenanzahl -1 (für alle Knoten) erstellt.
...
In diesem Beispiel wird der neue zustandsreiche Dienst "testApp~testService2" mit einem Ziel von 5 Knoten erstellt.
...
### -DefaultMoveCost
Geben Sie die Standardkosten für einen Verschiebezug an.
Höhere Kosten machen es weniger wahrscheinlich, dass der Clusterressourcen-Manager die Replikate verschiebt, wenn versucht wird, den Cluster zu ausgewogener
...
### -InstanceCount
Angeben der Instanzenanzahl für den Dienst
...
### -MinReplicaSetSize
Angeben der Mindestgröße des Replikats für den Dienst
...
Gibt an, dass der Dienst das benannte Partitionsschema verwendet.
Dienste, die dieses Modell verwenden, enthalten in der Regel Daten, die innerhalb eines gebundenen Sets in bucketed werden können.
Einige häufige Beispiele für Datenfelder, die als benannte Partitionsschlüssel verwendet werden, sind Regionen, Postleitzahlen, Kundengruppen oder andere Geschäftsgrenzen.
...
Gibt an, dass der Dienst das Partitionsschema des Singletons verwendet.
Singletonpartitionen werden in der Regel verwendet, wenn für den Dienst kein zusätzliches Routing erforderlich ist.
...
Gibt an, dass der Dienst das Partitionsschema "UniformInt64" verwendet.
Dies bedeutet, dass jede Partition über einen Bereich von int64-Schlüsseln besitzt.
...
### -VorzeichenLossWaitDuration
Angeben der Wartezeitdauer für den Verlust für den Dienst
...
Angeben der Dauer für den Neustart des Replikats für den Dienst
...
Angeben der Dauer des Ständers nach Replikat für den Dienst
...
### -Stateful
Verwenden für zustandsreiche Dienste
...
### -Stateless
Verwenden für zustandslose Dienste
...
Angeben der Größe des Zielreplikates für den Dienst
...
### -Type
Geben Sie den Diensttypnamen der Anwendung an, der im Anwendungsmanifest vorhanden sein sollte.
