...
### Beispiel 1: Erhalten eines BLOB nach BLOB-Namen
```
PS C:\>Get-AzStorageBlob -Container "ContainerName" -Blob blob*
```

Dieser Befehl verwendet einen BLOB-Namen und einen Platzhalter, um ein BLOB zu erhalten.

### Beispiel 2: Erhalten von Blobs in einem Container mithilfe der Pipeline
```
...
Dieser Befehl verwendet die Pipeline, um alle Blobs (blobs im Status "Gelöscht" enthalten) in einem Container zu erhalten.

### Beispiel 3: Blobs nach Namenspräfix erhalten
```
PS C:\>Get-AzStorageBlob -Container "ContainerName" -Prefix "blob"
```

Dieser Befehl verwendet ein Namenspräfix, um BLOBs zu erhalten.
...
In diesem Beispiel werden die *Parameter "MaxCount"* und *"ContinuationToken"* verwendet, um Azure Storage-BLOBs in mehreren Batches auflisten.
Die ersten vier Befehle weisen Variablen Werte zu, die im Beispiel verwendet werden sollen.
Der fünfte Befehl gibt eine **Do-While-Anweisung** an, die das **Cmdlet "Get-AzStorageBlob"** verwendet, um BLOBS zu erhalten.
...
Weitere Informationen erhalten Sie, wenn Sie " `Get-Help About_Do` eingeben" aus.
Im letzten Befehl wird der Befehl **"Echo"** zum Anzeigen der Summe verwendet.

### Beispiel 5: Alle Blobs in einem Container erhalten, enthalten eine BLOB-Version.
```
...
Mit diesem Befehl werden alle BLOB-BloBs in einem Container mit einer BLOB-Version bekommt.

### Beispiel 6: Erhalten einer einzelnen BLOB-Version
```
...
Dieser Befehl ruft einen einzelnen BLOBS Verion mit VersionId ab.

### Beispiel 7: Erhalten einer einzelnen BLOB-Momentaufnahme
```
...
### -BLOB
Gibt ein Namens- oder Namensmuster an, das für eine Platzhaltersuche verwendet werden kann.
Wenn kein BLOB-Name angegeben ist, werden im Cmdlet alle BLOBS im angegebenen Container aufgeführt.
Wenn für diesen Parameter ein Wert angegeben ist, werden im Cmdlet alle BLOBs mit Namen aufgeführt, die diesem Parameter entsprechen. Dieser Parameter unterstützt Platzhalter an beliebiger Stelle in der Zeichenfolge.
...
### -Context
Gibt das Azure-Speicherkonto an, aus dem Sie eine Liste der BLOBS erhalten möchten.
Mit dem cmdlet New-AzStorageContext können Sie einen Speicherkontext erstellen.
...
Verwenden Sie diesen Parameter und den *Parameter "MaxCount",* um BLOBs in mehreren Batches auflisten.
...
### -IncludeDeleted
Deleted Blob include, default get blob won't include deleted blob.
...
### -IncludeVersion
Blob-Versionen werden nur aufgelistet, wenn dieser Parameter vorhanden ist, standardmäßig enthält "get blob" keine BLOB-Versionen.
...
### -Prefix
Gibt ein Präfix für die BLOB-Namen an, die Sie erhalten möchten.
Dieser Parameter unterstützt nicht die Verwendung regulärer Ausdrücke oder Platzhalterzeichen für die Suche.
Dies bedeutet: Wenn der Container nur blobs mit den Namen "My", "MyBlob1" und "MyBlob2" enthält und Sie "-Prefix My*" angeben, gibt das Cmdlet keine BLOBs zurück.
