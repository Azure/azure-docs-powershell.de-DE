...
## BESCHREIBUNG
Das **Cmdlet "New-AzApiManagementApi"** erstellt eine Azure API Management API.
...
### Beispiel 2: Erstellen einer API durch Kopieren aller Operation, Tags, Produkte und Richtlinien aus der Echo-API in eine ApiVersionSet
```powershell
...
Dieser Befehl erstellt eine API in ApiVersionSet und kopiert alle Operation, Tags und `echoapiv3` Richtlinien aus der `xmsVersionSet` Quell-API. `echo-api` Sie setzt "SubscriptionRequired", "ServiceUrl", "Path", "Protocols" außer Kraft
...
### -AuthorizationScope
Gibt den Bereich der OAuth-Vorgänge an.
...
### -Path
Gibt den Web-API-Pfad an, bei dem es sich um den letzten Teil der öffentlichen URL der API handelt und der dem Suffixfeld für die Web-API-URL im Verwaltungsportal entspricht.
Diese URL wird von Api-Kunden verwendet, um Anforderungen an den Webdienst zu senden, und sie muss ein bis 400 Zeichen lang sein.
Der Standardwert ist $Null.
...
### -SourceApiId
Der API-Bezeichner der Quell-API. Dieser Parameter ist optional.
...
Gibt den Parameternamen der Abfragezeichenfolge des Abonnementschlüssels an.
Der Standardwert ist $Null.
