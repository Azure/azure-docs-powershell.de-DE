...
## BESCHREIBUNG
Das **Cmdlet "New-AzKeyVaultCertificatePolicy"** erstellt ein Speicherzertifikatrichtlinienobjekt für den Azure Key Vault.
...
Mit diesem Befehl wird eine Zertifikatrichtlinie erstellt, die sechs Monate lang gültig ist, und der Schlüssel zum Erneuern des Zertifikats erneut verwendet.

### Beispiel 2

Erstellt ein Speicherzertifikatrichtlinienobjekt. (automatisch generiert)
...
### Beispiel 3: Erstellen eines Zertifikats vom Typennamen (Subject Alternate Name, SAN)
...
In diesem Beispiel wird ein SAN-Zertifikat mit drei DNS-Namen erstellt.

## PARAMETERS

### -CertificateTransparency
Gibt an, ob für dieses Zertifikat/den Aussteller Zertifikattransparenz aktiviert ist. wenn nicht angegeben, ist der Standardwert "true".
...
### -Curve
Gibt den auslassungspunktenden Kurvennamen des Schlüssels des Zertifikats an.
...
### -DnsName
Gibt die DNS-Namen im Zertifikat an. Subject Alternative Names (SANs) können als DNS-Namen angegeben werden.
...
### -EKUS
Gibt die erweiterten Schlüsselverwendungen (ENHANCEDUs) im Zertifikat an.
...
### -EmailAtNumberOfDaysBeforeExpiry
Gibt an, wie viele Tage vor Ablauf der automatische Benachrichtigungsprozess beginnt.
...
### -KeySize
Gibt die Schlüsselgröße des Zertifikats an.
Die zulässigen Werte für diesen Parameter sind:
- 2048
- 3072
- 4096
...
### -RenewAtNumberOfDaysBeforeExpiry
Gibt die Anzahl von Tagen vor dem Ablauf an, nach der der automatische Prozess für die Zertifikaterneuerung beginnt.
...
### -SecretContentType
Gibt den Inhaltstyp des neuen Schlüsseltresorschlüssels an.
Die zulässigen Werte für diesen Parameter sind:
...
> [!NOTE]
> Wenn Sie innerhalb einer Eigenschaft im Parameter ein Komma (,) oder einen Komma (.) verwenden müssen, müssen Sie das Eigenschaftsfeld `SubjectName` in Anführungszeichen setzen. Sie können z. B. O="Contoso, Ltd." verwenden. im Feld "Organisationsname".
...
### -ValidityInMonths
Gibt die Anzahl der Monate an, für die das Zertifikat gültig ist.
