...
## BESCHREIBUNG
Verwenden Sie `Remove-AzKeyVaultRoleAssignment` das Cmdlet, um den Zugriff auf einen Beliebigen Prinzipal in einem bestimmten Bereich und einer bestimmten Rolle zu widerrufen. Das Objekt der Abtretung, d. h. der Prinzipal MUSS angegeben werden. Der Prinzipal kann ein Benutzer (Verwenden von SignInName- oder ObjectId-Parametern zum Identifizieren eines Benutzers), eine Sicherheitsgruppe (mit dem ObjectId-Parameter zum Identifizieren einer Gruppe) oder ein Dienstprinzipal (ApplicationId- oder ObjectId-Parameter verwenden, um einen "ServicePrincipal" zu identifizieren) sein. Die Rolle, der der Prinzipal zugewiesen ist, muss mit dem Parameter "RoleDefinitionName" oder "RoleDefinitionId" angegeben werden.
...
In diesem Beispiel werden alle Rollen von " user1@microsoft.com in allen Gültigkeitsbereich widerrufen.
