...
## BESCHREIBUNG
Vollständige Sicherung eines verwalteten HSM in einem Speicherkonto.
Wird `Restore-AzKeyVault` verwendet, um die Sicherung wiederherzustellen.
...
Das Cmdlet erstellt einen Ordner (normalerweise mit dem Namen) im Speichercontainer, speichern die Sicherung in diesem Ordner und `mhsm-{name}-{timestamp}` gibt den Ordner-URI aus.
