...
## BESCHREIBUNG
Das Set-AzEventHubNamespace cmdlet aktualisiert die Eigenschaften des angegebenen Ereignishub-Namespaces.
...
Aktualisiert die Tags für den Namespace \` "MyNamespaceName" \` auf "Created".
...
Aktualisiert den Zustand des \` Namespaces "MyNamespaceName" mit aktivierter \` Funktion "AutoInflate =" und "MaximumThroughputUnits = 10".

### Beispiel 3

Aktualisiert den angegebenen Ereignishub-Namespace. (automatisch generiert)

```powershell <!-- Aladdin Generated Example --> 
Set-AzEventHubNamespace -Location 'WestUS' -Name MyNamespaceName -ResourceGroupName MyResourceGroupName -SkuName Basic
```

### Beispiel 5: Erstellen und Aktualisieren von Namespaces mit "Identität verwalten" in einem Cluster 
```powershell
...
### -IdentityUserDefined
Benutzerdefinierte Identität oder Keine
...
### -KeyProperty
Liste der Schlüsseleigenschaften, @(@(KeyName,KeyVaultUri,Keyversion),@(KeyName,KeyVaultUri,Keyversion))
...
### -State
Deaktivieren/Aktivieren des Namespaces
...
### -Tag
Hashtables, die das Ressourcentag darstellt.
