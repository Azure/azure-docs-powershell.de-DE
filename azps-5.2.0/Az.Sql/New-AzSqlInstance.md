...
## BESCHREIBUNG
Das **Cmdlet "New-AzSqlInstance"** erstellt eine Azure SQL-Datenbank-verwaltete Instanz.
...
Dieser Befehl erstellt mithilfe des Parameters "SkuName" eine neue Instanz.
...
Dieser Befehl erstellt mithilfe der Parameter "Edition" und "ComputeGeneration" eine neue Instanz.

### Beispiel 3: Erstellen einer neuen Instanz in einem Instanzpool mithilfe eines Instanzpoolobjekts
```powershell
...
Mit diesem Befehl wird mithilfe eines Instanzpoolobjekts eine neue Instanz in einem Instanzpool erstellt.

### Beispiel 4: Erstellen einer neuen Instanz in einem Instanzpool mithilfe eines Ressourcenbezeichners für den Instanzpool
```powershell
...
Mit diesem Befehl wird mithilfe des Ressourcenbezeichners des Instanzpools eine neue Instanz in einem Instanzpool erstellt.

### Beispiel 5: Erstellen einer neuen Instanz in einem Instanzpool
```powershell
...
Mit diesem Befehl wird eine neue Instanz in einem Instanzpool mit Name instancePool0 erstellt.
...
### -AssignIdentity
Generieren und zuweisen Sie eine Azure Active Directory-Identität für diese verwaltete Instanz zur Verwendung mit Schlüsselverwaltungsdiensten wie Azure KeyVault.
...
Die Sicherungsspeicherredundanz, die zum Speichern von Sicherungen für die sql Azure Managed Instance verwendet wird. Optionen sind: Lokal, Zone und Geo
...
### -Collation
Die Sortierung der zu verwendende Azure SQL verwaltete Instanz.
...
### -DnsZonePartner
Die Ressourcen-ID des vom Partner verwalteten Servers, von dem die Eigenschaft "DnsZone" für die Erstellung verwalteter Instanzen geerbt werden soll
...
### -InstancePool
Das übergeordnete Objekt des Instanzpools.
...
### -InstancePoolName
Der Instanzpool, in dem diese Instanz zu platzieren ist.
...
Die Ressourcen-ID des Instanzpools.
...
### -Location
Der Speicherort, an dem die Instanz erstellt werden soll
...
### -SkuName
Der Name der SKU für die Instanz, z. B. "GP_Gen4", "BC_Gen4".
...
### -SubnetId
Die Subnetz-ID, die für die Instanzerstellung verwendet werden soll
...
### -TimezoneId
Die Zeitzonen-ID für die zu festlegende Instanz. Eine Liste der Zeitzonen-IDs wird in der sys.time_zone_info (Transact-SQL) verfügbar gemacht.
