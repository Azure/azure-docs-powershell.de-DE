...
## Beschreibung
Das Cmdlet " **Set-AzApiManagementPolicy** " legt die angegebene Bereichs Richtlinie für die API-Verwaltung fest.
...
### -Format
Gibt das Format der Richtlinie an. Bei `application/vnd.ms-az-apim.policy+xml` der Verwendung müssen Ausdrücke, die in der Richtlinie enthalten sind, mit XML-Escapezeichen versehen sein. Bei Verwendung `application/vnd.ms-az-apim.policy.raw+xml` ist es **nicht** erforderlich, dass die Richtlinie XML-escaped ist.
Der Standardwert ist `application/vnd.ms-az-apim.policy+xml` .
